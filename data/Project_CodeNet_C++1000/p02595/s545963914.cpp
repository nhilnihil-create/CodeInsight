...
long double distance_from_origin(ll Xi, ll Yi){
    return (long double)pow((Xi*Xi)+(Yi*Yi), 0.5);
}

void solve(){
    ll N,D,Xi,Yi,count_less_than_D;
    cin>>N>>D;
    ...
    for(int i=0;i<N;i++){
        cin>>Xi>>Yi;
        if(distance_from_origin(Xi,Yi)<=D){
            count_less_than_D ++;
        }
    }

    cout<<count_less_than_D<<endl;
}
