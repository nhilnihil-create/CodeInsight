...
signed main() {
  ...
  int searchPos = -1;
  while(true){
  	int nextPos = s.find("AC",searchPos+1);
    if(nextPos!=string::npos){
      for(int i=searchPos+1;i<nextPos;i++){
        posCount[i+1] = posCount[i];
      }
      posCount[nextPos+1] = posCount[nextPos]+1;
      searchPos = nextPos;
    }
    else{
      for(int i=searchPos+1;i<n;i++){
        posCount[i+1] = posCount[i];
      }
      break;
    }
  ...
  */
  ...
  for(int i=0;i<q;i++){
    cin >> l >> r;
    cout << posCount[r-1]-posCount[l-1] << endl;
  }
