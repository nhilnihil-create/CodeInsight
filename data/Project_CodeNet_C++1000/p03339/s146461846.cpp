...
ll E_ruiseki[300300], W_ruiseki[300300];
ll N; string S;

int main(void) {
  cin >> N; cin >> S;
  reps(i,N){
    E_ruiseki[i] = E_ruiseki[i-1];
    W_ruiseki[i] = W_ruiseki[i-1];
    if (S[i-1]=='E') E_ruiseki[i]++;
    if (S[i-1]=='W') W_ruiseki[i]++;
  }
  ll res = INF;
  reps(i,N){
    chmin(res,W_ruiseki[i-1]-W_ruiseki[0]+E_ruiseki[N]-E_ruiseki[i]);
  }
  cout << res << "\n";
  return 0;
}
