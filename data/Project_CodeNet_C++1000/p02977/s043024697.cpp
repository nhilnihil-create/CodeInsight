...
signed main() {
	...
	if (n % 4 == 0) {
		...
		update(n - 1);
		update(x);
		FOR(i, 1, n - 1) if (!set.count(i)) update(i);
		ans.emplace_back(n, n - 1);
		ans.emplace_back(x, n + n);
	}
	else if(n % 4 == 1) {
		FOR(i, 2, n + 1) if (!set.count(i)) update(i);
		...
		update(3);
		update(2);
		FOR(i, 4, n - 1) if (!set.count(i)) update(i);
		ans.emplace_back(n, n - 1);
		ans.emplace_back(3, n + n);
		ans.emplace_back(1, 2);
	...
	REP(i, (int)v.size() - 1) {
		...
		ans.emplace_back(v[i] + n, v[i + 1] + n);
	}
	ans.emplace_back(v.back(), v.front() + n);
	OUT("Yes")BR;
	REP(i, ans.size()) {
		OUT(ans[i].first)SP OUT(ans[i].second)BR;
	}
