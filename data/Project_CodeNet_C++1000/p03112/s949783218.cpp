...
void solve(int a, int b, int q, vi s, vi t, vi x) {
  each(i, x) {
    auto s_up = upper_bound(all(s), i);
    int sl = s_up == begin(s) ? -INF : *(s_up - 1);
    int sr = s_up == end(s) ? INF : *s_up;
    auto t_up = upper_bound(all(t), i);
    int tl = t_up == begin(t) ? -INF : *(t_up - 1);
    int tr = t_up == end(t) ? INF : *t_up;
    int tmp = min({max(i - sl, i - tl), max(sr - i, tr - i),
        min(i - sl, tr - i) + i - sl + tr - i,
        min(i - tl, sr - i) + i - tl + sr - i});
    COUT(tmp);
  }
}
