...
int main()
{
  ...
  FOR(i,A){ ll a; cin >> a; s[i+1] = a; }
  FOR(i,B){ ll b; cin >> b; t[i+1] = b; }
  s[0] = -INF; t[0] = -INF; s[A+1] = INF; t[B+1] = INF;
  ...
  FOR(i,Q){
    ...
    //神社/寺→東。
    {
      ...
      ll xs = abs(*itrs-x), xt = abs(*itrt-x);
      //cout << xs << " " << xt << endl;
      ll tmp = max(xs,xt);
      ans = min(ans,tmp);
    }
    ...
    {
      auto itrs = lower_bound(all(s),x);
      auto itrt = prev(lower_bound(all(t),x));
      ll xs = abs(*itrs-x), xt = abs(*itrt-x);
      //cout << xs << " " << xt << endl;
      ll tmp = min(2*xs+xt,2*xt+xs);
      ans = min(ans,tmp);
    }
    //神社→西/寺→東。
    {
      auto itrs = prev(lower_bound(all(s),x));
      auto itrt = lower_bound(all(t),x);
      ll xs = abs(*itrs-x), xt = abs(*itrt-x);
      //cout << xs << " " << xt << endl;
      ll tmp = min(2*xs+xt,2*xt+xs);
