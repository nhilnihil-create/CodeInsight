...
int main(){
    ...
    rep(i,Q){
        ...
        ll ans = min({abs(x - *s1) + abs(*s1 - *t1), abs(x - *t1) + abs(*t1 - *s1), abs(x - *s1) + abs(*s1 - *t2), abs(x - *t2) + abs(*t2 - *s1),
                    abs(x - *s2) + abs(*s2 - *t1), abs(x - *t1) + abs(*t1 - *s2), abs(x - *s2) + abs(*s2 - *t2), abs(x - *t2) + abs(*t2 - *s2)});
