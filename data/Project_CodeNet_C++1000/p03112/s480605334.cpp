...
void solve(){
  ...
  V<ll> sm(a), tm(b);
  FOR(i,0,a) sm[i] = -s[a-i-1];
  FOR(i,0,b) tm[i] = -t[b-i-1];
  FOR(i,0,q){
    ll x; cin >> x;
    ll ans = LINF;
    auto sr = lb(ALL(s),x);
    ...
    auto sl = lb(ALL(sm),-x);
    auto tl = lb(ALL(tm),-x);
    if(sr != s.end() && tr != t.end()) ans = min<ll>(ans, max<ll>(*sr,*tr)-x);
    if(sl != sm.end() && tl != tm.end()) ans = min<ll>(ans, x + max<ll>(*sl,*tl) );
    if(sr != s.end() && tl != tm.end() ) ans = min(ans,min<ll>( (*sr-x) + (*sr - (-(*tl))) , (x-(-(*tl)))+(*sr - (-(*tl))  )     )   );
    if(sl != sm.end() && tr != t.end() ) ans = min(ans,min<ll>( (*tr-x) + (*tr - (-(*sl))) , (x-(-(*sl)))+(*tr - (-(*sl))  )     )   );
    ncoutn( ans );
  }
}
