...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        cout << min({ min(abs(qs-z[oka]),abs(qs - z[nga])) + min(abs(qs - x[okb]),abs(qs - x[ngb]))+min(min(abs(qs - z[oka]),abs(qs - z[nga])),min(abs(qs - x[okb]),abs(qs - x[ngb]))),max(abs(qs - z[oka]),abs(qs - x[okb])), max(abs(qs - z[nga]) , abs(qs - x[ngb])) }) << endl;
    }
}