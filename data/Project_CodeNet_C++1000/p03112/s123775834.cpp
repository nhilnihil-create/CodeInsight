...
int main() {
    ...
    vector<LL> s, t;
    for (int i = 0; i < 10; i++) s.push_back(INF), t.push_back(INF), s.push_back(-INF), t.push_back(-INF);
    for (int i = 0; i < A; i++) {
        LL x;
        cin >> x;
        s.push_back(x);
    ...
    while (Q--) {
        ...
        for (int i = -3; i <= 3; i++) {
            for (int j = -3; j <= 3; j++) {
                auto itss = its + i;
                auto ittt = itt + j;
                auto minV = min(*itss, *ittt);
                auto maxV = max(*itss, *ittt);
                if (maxV <= x) ans = min(ans, abs(x - minV));
                else if (x <= minV) ans = min(ans, abs(x - maxV));
                else {
                    auto d1 = abs(x - minV);
                    auto d2 = abs(x - maxV);
                    ans = min(ans, d1 * 2 + d2);
