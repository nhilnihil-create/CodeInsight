...
int main()
{
	...
	rep(i,q){
		ll ans=INF;
		vll ss(2),tt(2);
		ss[0]=s[lower_bound(all(s),x[i])-s.begin()-1];
		ss[1]=s[lower_bound(all(s),x[i])-s.begin()];
		tt[0]=t[lower_bound(all(t),x[i])-t.begin()-1];
		tt[1]=t[lower_bound(all(t),x[i])-t.begin()];
		for(auto&& e:ss){
			for(auto&& f:tt) chmin(ans,abs(x[i]-e)+abs(e-f));
		}
		for(auto&& e:tt){
			for(auto&& f:ss) chmin(ans,abs(x[i]-e)+abs(e-f));
		}
		cout << ans << endl;
	}
	return 0;
