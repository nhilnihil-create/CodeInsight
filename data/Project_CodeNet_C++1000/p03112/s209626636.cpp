...
int main(){
  ...
  rep(i,q){
    int64_t x;
    cin >> x;
    auto sritr = lower_bound(s.begin(),s.end(),x);
    auto slitr = sritr;
    int64_t sl,sr;
    if(sritr == s.end()){
      sr = 100000000000000;
    ...
    //printf("%ld,%ld\n",sl,sr);
    auto tritr = lower_bound(t.begin(),t.end(),x);
    auto tlitr = tritr;
    int64_t tl,tr;
    if(tritr == t.end()){
      tr = 100000000000000;
    ...
    //printf("%ld,%ld\n",tl,tr);
    vector<int64_t> dist(8);
    dist[0] = abs(sl - x) + abs(sl - tl);
    dist[1] = abs(tl - x) + abs(tl - sl);
    dist[2] = abs(sr - x) + abs(sr - tr);
    dist[3] = abs(tr - x) + abs(tr - sr);
    dist[4] = abs(sl - x) + abs(sl - tr);
    dist[5] = abs(tl - x) + abs(tl - sr);
    dist[6] = abs(sr - x) + abs(sr - tl);
    dist[7] = abs(tr - x) + abs(tr - sl); 
