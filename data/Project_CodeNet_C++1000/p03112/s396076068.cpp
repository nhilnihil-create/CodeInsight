...
int main()
{
	...
	rep(i,q){
		...
		ll rs=s[lower_bound(all(s),x)-s.begin()];
		ll lt=t[lower_bound(all(t),x)-t.begin()-1];
		ll rt=t[lower_bound(all(t),x)-t.begin()];
		ll ans=INF;
		chmin(ans,abs(x-ls)+abs(ls-lt));
		chmin(ans,abs(x-ls)+abs(ls-rt));
		chmin(ans,abs(x-rs)+abs(rs-lt));
		chmin(ans,abs(x-rs)+abs(rs-rt));
		chmin(ans,abs(x-lt)+abs(lt-ls));
		chmin(ans,abs(x-lt)+abs(lt-rs));
		chmin(ans,abs(x-rt)+abs(rt-ls));
		chmin(ans,abs(x-rt)+abs(rt-rs));
		cout << ans << endl;
	}
	return 0;
}