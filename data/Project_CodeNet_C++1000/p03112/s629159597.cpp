...
int main()
{
        ...
        rep(i, 0, Q) {
                ll x;
                cin >> x;
                ll rap = lower_bound(all(s), x) - s.begin();
                ll rbp = lower_bound(all(t), x) - t.begin();
                ll lap = rap - 1;
                ll lbp = rbp - 1;
                ...
                ll c1 = abs(ra - lb) + min(abs(x - ra), abs(x - lb));
                ll c2 = abs(rb - la) + min(abs(x - rb), abs(x - la));
                ll c3 = abs(max(ra, rb) - x);
                ll c4 = abs(x - min(la, lb));
