...
signed main() {
  ...
  for(ll i=0;i<q;i++){
    ...
    auto sPrevIter = sIter; sPrevIter--;
    ...
    auto tPrevIter = tIter; tPrevIter--;
    if(tIter == t.end()){
      tIter = t.end()-1;
      tPrevIter = t.end()-1;
    }
    ...
    ll minMove = 100000000000000;
    for(int j=0;j<2;j++){
      for(int k=0;k<2;k++){
        ll move = 0;
        if( ((x-sC[j])/abs(x-sC[j]))*((x-tC[k])/abs(x-tC[k])) > 0 ){
          move = max(abs(x-sC[j]),abs(x-tC[k]));
        }
        else{
          move = 2*min(abs(x-sC[j]),abs(x-tC[k])) + max(abs(x-sC[j]),abs(x-tC[k]));
        }
        //cout << x << ":" << move << endl;
