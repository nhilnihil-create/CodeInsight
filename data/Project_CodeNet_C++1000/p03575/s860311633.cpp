...
{
	ll N, M; cin >> N >> M;
	vt<pll> edge(M); each(i, e, edge) cin >> e.first >> e.second;
	each(i, e, edge) --e.first, --e.second;

	ll ans = 0;
	rep(i, 0, M)
	{
		DS_UnionFind uf(N + 1);
		rep(j, 0, i) uf.Union(edge[j].first, edge[j].second);
		rep(j, i + 1, Size(edge)) uf.Union(edge[j].first, edge[j].second);
		ll test = uf.Find(0);
		rep(j, 0, N)
		{
			if (test != uf.Find(j))
