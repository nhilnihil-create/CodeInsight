...
bool is_bridge(ll N, vector<vector<ll>> &v, ll a, ll b) {
    // cout << "##############" << a << "-" << b << endl;
    ...
    while(s.size() > 0) {
        ...
        // cout << now << endl;
        seen[now] = true;
        for(auto next_v: v[now]) {
            if(now == a && next_v == b) continue;
            if(now == b && next_v == a) continue;
...
int main() {
    ...
    ll ret = 0;
    for(ll i = 0; i < M; i++) {
        if(is_bridge(N, v, va[i], vb[i])) ret++;
    }
