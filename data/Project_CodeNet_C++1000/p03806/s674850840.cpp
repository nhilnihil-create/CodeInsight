...
int main(void)
{
  ...
  vector<vector<ll>> dp(411, vector<ll>(411, INT_MAX));
  vector<vector<ll>> dpNew(411, vector<ll>(411, INT_MAX));
  dp[0][0] = 0;
  dpNew[0][0] = 0;
  rep(k, n)
  {

    rep(i, 411) rep(j, 411) dpNew[i][j] = dp[i][j];
    ...
    rep(i, 401) rep(j, 401)
    {
      ...
      if (x < INT_MAX)
      {
        auto idxY = i + A[k];
        auto idxX = j + B[k];
        dpNew[idxY][idxX] = min(dp[idxY][idxX], dp[i][j] + C[k]);
      }
    }
    rep(i, 411) rep(j, 411) dp[i][j] = dpNew[i][j];
  }
