...
signed main(){
	ic(n)
	ic(m1) ic(m2)
	rep(0,i,n)cin>>a[i]>>b[i]>>c[i];
	rep(0,i,n)rep(0,j,444)rep(0,l,444)dp[i][j][l]=inf;
	dp[0][0][0]=0;
	rep(1,i,n+1){
		rep(0,j,444){
			rep(0,l,444){
				if(j>=a[i-1]&&l>=b[i-1]){
					dp[i][j][l]=min(dp[i-1][j][l],dp[i-1][j-a[i-1]][l-b[i-1]]+c[i-1]);
				}
	...
	int ans=inf;
	rep(1,i,444){
		if(i*m1<444&&i*m2<444){
			ans=min(ans,dp[n][i*m1][i*m2]);
		}
	}
	if(ans==inf)c(-1)
	else c(ans)
