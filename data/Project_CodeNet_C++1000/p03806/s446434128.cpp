...
int main()
{
  ...
  vector<ll> A(N), B(N), C(N); FOR(i,N){ cin >> A[i] >> B[i] >> C[i]; }
  ll offset = 2000, sz = 2*offset+1, INF = 1e18;
  vector<ll> cost1(sz,INF), cost2(sz,INF);
  
  for(int i=1; i<power(2,(N+1)/2); i++){
    ll index = 0, cost = 0;
    ...
    cost1[offset+index] = min(cost1[offset+index],cost);
  }
  for(int i=1; i<power(2,N/2); i++){
    ll index = 0, cost = 0;
    for(int j=0; j<(N/2); j++){
      if((i>>j)%2){
        index += (MB*A[(N+1)/2+j]-MA*B[(N+1)/2+j]);
        cost += C[(N+1)/2+j];
      }
    }
    cost2[offset+index] = min(cost2[offset+index],cost);
  }

  //main
  ll ans = min(cost1[offset],cost2[offset]);
  for(int i=0; i<sz; i++){
    ans = min(ans,cost1[i]+cost2[sz-1-i]);
  }
