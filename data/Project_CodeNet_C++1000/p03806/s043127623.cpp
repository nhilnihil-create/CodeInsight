...
signed main()
{
    ...
    b.resize(n);
    c.resize(n);
    rep(n) cin >> a.at(i) >> b.at(i) >> c.at(i);
    rep(50) rep(k, 500) rep(j, 500) table[i][j][k] = IINF;
    table[0][0][0] = 0;
    rep(n)
    {
        rep(ca, 400)
            rep(cb, 400)
        {
            if (table[i][ca][cb] == IINF)
                continue;
            table[i + 1][ca][cb] = min(table[i + 1][ca][cb], table[i][ca][cb]);
            table[i + 1][ca + a[i]][cb + b[i]] = min(table[i + 1][ca + a[i]][cb + b[i]], table[i][ca][cb] + c[i]);
        }
    }
