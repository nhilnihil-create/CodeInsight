...
void Dufaux()
{
    ...
    ll firstay=b;
    ll seconday=b+w;
    if((firstay>=firstax && firstay<=secondax) || (seconday>=firstax && seconday<=secondax)){
        cout<<"0"<<endl;
    }
    else{
        ll aa=abs(firstax-firstay);
        ll bb=abs(firstax-seconday);
        ll cc=abs(secondax-firstay);
        ll dd=abs(secondax-seconday);
