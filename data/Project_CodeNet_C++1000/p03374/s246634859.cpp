...
int main(){
  ...
  rep1(i,n){
    lli tx, tv;
    cin >> tx >> tv;
    vec[i]=make_pair(tx, tv);
  }
  vec.emplace_back(c,0);

  vector<lli> ml(n+1,0),mr(n+1,0),li(n+1,0), ri(n+1,0), lii(n+1,0), rii(n+1,0);
  rep1(i,n){
    li[i]= - vec[i].first + vec[i-1].first + vec[i].second + li[i-1];
    ri[i]= - vec[n-i+2].first + vec[n-i+1].first + vec[n-i+1].second + ri[i-1];
    lii[i]= 2*(- vec[i].first + vec[i-1].first) + vec[i].second + lii[i-1];
    rii[i]= 2*(- vec[n-i+2].first + vec[n-i+1].first) + vec[n-i+1].second + rii[i-1];
  }
  rep1(i,n){
    ml[i]=max(lii[i],ml[i-1]);
    mr[i]=max(rii[i],mr[i-1]);
  }
  lli al=0, ar=0;
  rep1(i,n) if(ri[i]+ml[n-i] > ar)ar=ri[i]+ml[n-i];
  rep1(i,n) if(li[i]+mr[n-i] > al)al=li[i]+mr[n-i];
  cout << max(al,ar);
}
