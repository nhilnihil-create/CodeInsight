...
ll solve(int N, ll K, vi& a, vi& f) {
    sort(ALL(a)); reverse(ALL(a));
    sort(ALL(f)); reverse(ALL(f));
    ll max_a = *max_element(ALL(a)), max_f = *max_element(ALL(f));
    ll lo = -1, hi = max_a * max_f;
    while (lo+1 < hi) {
        ll mi = (lo+hi) / 2;
        vll okline(N);
        rep(i,N){
        ...
        sort(ALL(okline)); reverse(ALL(okline));
        ll cost = 0;
        rep(i,N){
            if ((ll)a[i] > okline[i]) cost += (ll)a[i] - okline[i];
        }
        bool possible = (cost <= K);
        if (possible) {
            hi = mi;
        } else {
            lo = mi;
