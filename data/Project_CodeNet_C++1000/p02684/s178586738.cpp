...
int main() {
    ...
    for (int i = 1; i <= N + 1; i++) // N回繰り返す
    {   
        // cout << "a,"<< a << ",A[a], " << A[a] << ",";
        ROOP[A[a]] = a;
        // ROOP.insert(make_pair(A[a], a)); // A[a]の場所に訪れたa
        ...
        // 終了条件
        int b = A[A[a]];
        // cout <<  "A[b],"<< b<<",次のRoopのkey: "<< ROOP[A[b]] <<endl;


        if(ROOP[b] == A[a]){
            // cout << "Roop[A[b]],"<<ROOP[A[b]]<<",A[a]"<< A[a] <<",";
            // cout << "|" << endl;
            m = A[b];
            s = Time[b];
            e = i;
        ...
        Time[A[a]] = i; // aを訪れた時の繰り返す回数;
        // cout << ",Time," << Time[A[a]]  <<endl;
