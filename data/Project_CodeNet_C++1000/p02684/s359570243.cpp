...
int main()
{
	...
	cin >> n;
	cin >> k;
	vector<pair<int32_t, int64_t>> v(n + 1);
	...
	for (i = 1; i <= k; i++) {
		if (v[next_position].second) { break; }
		v[next_position].second = i;
		next_position = v[next_position].first;
	}

	int64_t div = i - v[next_position].second;
	int64_t remain = (k - i + 1) % div;
