...
int main(int argc, char const *argv[])
{
    ...
    Vector record;  // 出てきた頂点の記録（順番に）
    set<ll> appeared;   // 出てきた頂点の記録
    Vector tele_times(N, 0);    // 各場所に最初に訪れた時に何回テレポートしていたか
    ll now = 0, next;
    record.push_back(now);
    appeared.insert(now);
    ll sum_times = 0, until_loop, loop_times;
    FOR_EQ(times, 1, K){
        next = A[now];
        sum_times++;
        if(appeared.find(next) == appeared.end()){  // 訪れたことがあるかの確認
            appeared.insert(next);
            record.push_back(next);
            tele_times[next] = sum_times;
        }else{  // 訪れたことがある。循環の検出
            until_loop = tele_times[next];
            loop_times = sum_times - until_loop;
            break;
        }
        now = next;
        if(times == K){ // 循環に入る前に終わった
            cout << now + 1 << endl;
            return 0;
        }
    }
    K -= until_loop;
    K %= loop_times;
    cout << record[until_loop + K] + 1 << endl;
