...
int main(){
    ...
    //doubling[i][j]:iから2^j回でどこにいるか
    ll D_MAX=65; //2^D_MAXで辿り着ける
    vector<vector<ll> > doubling(N+1,vector<ll>(D_MAX+1,0));
    
    //[i][0]は入力から作る
    rep1(i,N) doubling[i][0]=A[i];
    
    //前処理
    rep1(j,D_MAX)rep1(i,N){
    ...
    //逆辿り
    now=1;
    rrep(i,sz(v)){if(v[i]==1) now=doubling[now][i];
    }
    cout<<now<<endl;
}
