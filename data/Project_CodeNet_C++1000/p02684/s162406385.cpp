...
// double 引数 x 以下の整数で最大の整数値
// ex) -3.30303 ⇒ -4 / 3.30303 ⇒ 3
#define _floor(x) floor(x)
...
// ■スプリット
// auto elem = split(s, '+'); で+～+間の文字列コンテナ戻し
...
int main(void)
{
	...
	int32_t until_loop = distance(result.begin(), find(result.begin(), result.end(), temp));
	int32_t loop_count = result.size() - until_loop;

	if (k < until_loop) {
		cout << result[(int32_t)k] << endl;
		return 0;
	}

	k -= until_loop;
	k %= loop_count;
	cout << result[(int32_t)(until_loop + k)] << endl;
	return 0;
}
