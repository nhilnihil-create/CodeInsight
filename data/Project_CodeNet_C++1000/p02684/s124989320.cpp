...
int main(){
  ...
  vector<vector<int>> table(n+1,vector<int>(61));
  for(int i=1;i<=n;i++){
    table.at(i).at(0) = a.at(i);
  }
  for(int i=1;i<=60;i++){
    for(int j=1;j<=n;j++){
      table.at(j).at(i) = table.at(table.at(j).at(i-1)).at(i-1);
    }
  }
  int ans = 1;
  for(long long i=0;i<=60;i++){
    if(i<=30){
    if(k & (1<<i)){
      ans = table.at(ans).at(i);
    }
    }
    else{
      long long newi1 = (1<<30);
      long long newi2 = (1<<(i-30));
      if(k&(newi1*newi2)){
        ans = table.at(ans).at(i);
      }
    }
