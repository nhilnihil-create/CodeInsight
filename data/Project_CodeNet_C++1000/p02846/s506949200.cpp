...
int main(void) {
    ...
    auto naive = [&] {
        ...
        if(p + q < 0) return (int128)0;
        if(p + q == 0) return (int128)-1;
    ...
    auto solve = [&] {
        ...
        if((a1-b1)*t1 + (a2-b2)*t2 == 0) return (int128)-1;

        int128 lb = 1, ub = 1e22;
        if(check(ub)) return (int128)-1;
        while(ub-lb > 1) {
            int128 mid = (lb+ub)/2;
            if(check(mid)) lb = mid;
            else ub = mid;
    ...
    ll ret = solve();
    if(ret == -1) cout << "infinity" << endl;
    else cout << ret << endl;

    // REP(loop, 100000) {
    //     t1 = rand() % 100 + 1, t2 = rand() % 100 + 1;
    //     a1 = rand() % 100 + 1, a2 = rand() % 100 + 1;
    //     b1 = rand() % 100 + 1, b2 = rand() % 100 + 1;
