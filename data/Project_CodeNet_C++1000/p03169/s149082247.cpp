...
int N;
double memo(int i, int j, int k, vector<VVD> &dp){
  if(i==0&&j==0&&k==0) return 0;
  if(dp.at(i).at(j).at(k) > -1) return dp.at(i).at(j).at(k);
  double res = 1;
  if(i>0) res += memo(i-1,j,k,dp)*i/N;
  if(j>0) res += memo(i+1,j-1,k,dp)*j/N;
  if(k>0) res += memo(i,j+1,k-1,dp)*k/N;
  res *= (double)N/(double)(i+j+k);
...
int main() {
  ...
  vector<VVD> dp(N+1,VVD(N+1,VD(N+1,-1)));

  dp.at(0).at(0).at(0) = 0;

  double ans;
  ans = memo(p.at(0),p.at(1),p.at(2),dp);
