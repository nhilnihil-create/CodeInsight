...
int main()
{
  ...
  ll N; cin >> N; 
  vector<ll> A(N); vector<ll> cnt(4,0);
  FOR(i,N){ cin >> A[i]; cnt[A[i]] ++; }
  vector<vector<vector<double>>> dp(N+1,vector<vector<double>>(N+1,vector<double>(N+1,0)));
  vector<vector<vector<bool>>> isSearched(N+1,vector<vector<bool>>(N+1,vector<bool>(N+1,false)));
  isSearched[0][0][0] = true;
  
  //ループの回し方が非自明→メモ化再帰
  function<double(ll,ll,ll)> rec = [&](ll i, ll j, ll k){
    if(isSearched[i][j][k]){
      return dp[i][j][k]; 
    }
    
    double ret = 1;
    if(i >= 1) ret += rec(i-1,j,k)*((double)i/(double)N);//1→0
    if(j >= 1) ret += rec(i+1,j-1,k)*((double)j/(double)N);//2→1
    if(k >= 1) ret += rec(i,j+1,k-1)*((double)k/(double)N);//3→2
    ret /= (double)(i+j+k)/(double)N;//i==j==k==0→isSearchedがtrue
    
    isSearched[i][j][k] = true;
    return dp[i][j][k] = ret;
  };
