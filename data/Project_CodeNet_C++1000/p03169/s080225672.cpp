...
//vector<vr> g;
vector<vector<vector<dd> > > dp;
vector<ll> in;
dd n;
dd fun(dd one,dd two,dd three)
{
	...
	if(one>0)
	{
		ans+=(1 + fun(one-1,two,three))*one;
	}
	if(two>0)
	{
		ans+=(1 + fun(one+1,two-1,three))*two;
	}
	if(three>0)
	{
		ans+=(1 + fun(one,two+1,three-1))*three;
	}
	ans+=(n-(one + two + three));
	ans=ans/(one+two+three);
...
signed main()
{
	...
	while(t--)
	{
			...
			dp.resize(301,vector<vector<dd> >(301,vector<dd>(301,-1)));
			fun(f[1],f[2],f[3]);
			co(fixed<<setprecision(9)<<dp[f[1]][f[2]][f[3]]);
	}
