...
ldb solve(int i,int j,int k)
{
    ...
    if(dp[i][j][k]!= -1)
        return dp[i][j][k];
    dp[i][j][k]=0;
    dp[i][j][k]=(ldb)n/(i+j+k);
    dp[i][j][k]=dp[i][j][k]+(ldb)(i*solve(i-1,j,k))/(i+j+k);
    dp[i][j][k]=dp[i][j][k]+(ldb)(j*solve(i+1,j-1,k))/(i+j+k);
    dp[i][j][k]=dp[i][j][k]+(ldb)(k*solve(i,j+1,k-1))/(i+j+k);
    return dp[i][j][k];
}
