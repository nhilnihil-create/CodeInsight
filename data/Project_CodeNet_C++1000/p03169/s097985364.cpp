...
ll n ;
ld solve( ll b , ll c , ll d)
{
  // cout<<b<<c<<d<<endl;
  if(dp[b][c][d] !=-1) return dp[b][c][d];
  ll sum = n;
  ll eta = b+c+d;
  ld pr = sum*1.0 / eta;
  ld ans = 0;
  if(b)ans += (1.0 * b)/eta * (solve(b-1,c,d));
  if(c)ans += (1.0 * c)/eta * (solve(b+1,c-1,d));
  if(d)ans += (1.0 * d)/eta * (solve(b,c+1,d-1));
  // cout<<ans<<endl;
  ans += pr;
  // cout<<ans<<endl;
  return dp[b][c][d] = ans;
...
{ 
  ...
  std::setprecision(18);
  f(i,0,305) f(j,0,305) f(k,0,305)  dp[i][j][k] = -1;
  dp[0][0][0] = 0;
  cout<<setprecision(18)<<solve(b,c,d);
}
