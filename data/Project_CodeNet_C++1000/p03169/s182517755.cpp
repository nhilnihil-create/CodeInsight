...
ld solve(int one, int two, int three)
{
    int alive = one + two + three; //trace3(one,two,three);
    if (min(one, min(two, three)) < 0 || !alive)
        return 0;
    if (dp[one][two][three])
        return dp[one][two][three];
    return dp[one][two][three] = (n * 1.0) / alive + (one * solve(one - 1, two, three) + two * solve(one + 1, two - 1, three) + three * solve(one, two + 1, three - 1)) / (alive * 1.0);
}
...
int32_t main()
{
    ...
    while (tests--)
    {
        ...
        f(i, 0, n) cin >> e, one += e == 1, two += e == 2, three += e == 3;
        cout << prec(10) << solve(one, two, three); //trace3(dp[3][0][0],dp[2][0][0],dp[1][0][0]);
