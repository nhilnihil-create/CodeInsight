...
ld dp[305][305][305];
ld n;
ld expected_moves(int ones,int twos,int threes){
...
//  ld P1=ones/n,P2=twos/n,P3=threes/n,P0=(n-(ones+twos+threes))/n;
 dp[ones][twos][threes]=(n+ones*expected_moves(ones-1,twos,threes)+twos*expected_moves(ones+1,twos-1,threes)+threes*expected_moves(ones,twos+1,threes-1));
...
int main(){
 ...
 for(auto &x:a)
  cin>>x;
 /* It is necessary to understand that the dp depends only on the number
    of plates with 1, 2 and 3 sushis at that instance, and not on the 
    order in which the plates with diff. number of sushi occur, for example-
    Answer for {1,1,2,0,3,2} will be the same as answer for {2,1,3,0,1,2}, 
    or any other arrangement with the same count of each type of dish.

    So, let's define F(x,y,z) as a state of dp, where-
    F(x,y,z) is the expected number of moves when there are x plates with 
    1 sushi, y plates with 2 sushi, and z plates with 3 sushi.
    
    Now, int this state, the probability of choosing a dish with 1 shushi is-
    P1=(x/N),
    Similarly, P2=(y/N), and P3=(z/N), where N is the total no. of dishes.
    Also, probability of choosing a dish with 0 sushi would be- P0=(N-(x+y+z))/N.
    Now, we have- 
    F(x,y,z)=1+P0*F(x,y,z)+P1*F(x-1,y,z)+P2*F(x+1,y-1,z)+P3*F(x,y+1,z-1); 
    (Here the added 1 represents the current move)
    Rearranging-
   --> (1-P0)*F(x,y,z)=1 + P1*F(x-1,y,z) + P2*F(x+1,y-1,z) + P3*F(x,y+1,z-1);
   --> F(x,y,z) = (1 + P1*F(x-1,y,z) + P2*F(x+1,y-1,z) + P3*F(x,y+1,z-1))/(1 - P0);
    Simplifying further by putting values of P1, P2, P3 and P0-

   --> F(x,y,z) =( n + x*F(x-1,y,z) + y*F(x+1,y-1,z) + z*F(x,y+1,z-1) )/(x + y + z); **Required DP**
 ...
 for(auto v:a)
  freq[v]++;
 ones=freq[1];twos=freq[2];threes=freq[3];
 cout<<setprecision(16)<<expected_moves(ones,twos,threes)<<"\n";
