...
signed main() {
  ...
  int n,m;
  cin >> n >> m;
  graph going(n),coming(n),hanaoka(n);
  int qw,er,ty;
  Vi a(m),b(m),c(m);
  rep(i,0,m){
    cin >> a[i] >> b[i] >> c[i];
    a[i]--,b[i]--;
    going.add_edge(a[i],b[i],1);
    coming.add_edge(b[i],a[i],1);
  }
  ...
  rep(i,0,n){
    if(going.dist[i] <= INF/2 && coming.dist[i] <= INF/2) {
      can.insert(i);
    }
  }
  
  rep(i,0,m){
    if(can.count(a[i]) && can.count(b[i])) {
      hanaoka.add_edge(a[i],b[i],c[i]*(-1));
    }
  }
  
  bool ok = hanaoka.bellman_ford(0);
  
  if(ok) cout << "inf" << endl;
  else {
    cout << hanaoka.dist[n-1]*(-1) << endl;
  }
