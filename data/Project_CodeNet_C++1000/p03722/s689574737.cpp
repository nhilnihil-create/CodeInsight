...
template <typename T>
vector<T> bellman_ford(vector<edge<T>> &edges, int V, int s, const T &INF) {
  vector<T> dist(V, INF);
  dist[s] = 0;
  for (int i = 0; i < V - 1; i++) {
    for (auto &e : edges) {
  ...
  rep(_, V) {
    for (auto &e : edges) {
      if (dist[e.src] == INF) continue;
      if (dist[e.src] == -INF || dist[e.to] > dist[e.src] + e.cost) dist[e.to] = -INF;
    }
  }
  return dist;
}
...
void solve(int N, int M, vi a, vi b, vi c) {
  vector<edge<int>> es;
  rep(i, M)es.emplace_back(a[i], b[i], -c[i]);
  auto bf = bellman_ford(es, N, 0, INF);
  bf[N - 1] == -INF ? COUT("inf") : COUT(-bf[N - 1]);
}
