...
template<typename T>
bool bellman_ford(const vector<vector<pair<int,T>>>& path,vector<T>& dis,vector<bool>& negative,int start){
    ...
    for(int i=1;i<=n;i++){
        rep(j,n)for(auto e:path[j]){
            if(dis[j]==is_null) continue;
            tie(to,cost)=e;
            if(dis[to]>dis[j]+cost){
                dis[to]=dis[j]+cost;
                negative[to]=true;
            }
            if(negative[j]) negative[to]=true;
        }
    }
    return true;
}
...
int main(){
    ...
    bellman_ford<ll>(path,dis,negative,0);
