...
int main()
{
	...
	vector<vector<pii>> r(n), r2(n);
	rep(i, m){
		int a, b, c; cin >>a >>b >>c;
		r[a-1].emplace_back(b-1, -c);
		r2[b-1].emplace_back(a-1, c);
	}
	const ll inf = 1LL<<60;
	VI used(n);
	auto f = [&](auto f, int s, int z) ->void{
		auto& rcc = z==1? r: r2;
		if(used[s] & z) return;
		used[s] |= z;
		for(auto v:rcc[s]) f(f, v.first, z);
	};
	...
	auto bellman = [&](int s){
		...
		for(int i =0, nxt =1; i<n+3 && nxt>0 ; i++){
			nxt = 0;
			rep(j, n) for(auto z: r[j]) if(used[z.first]>=3){
