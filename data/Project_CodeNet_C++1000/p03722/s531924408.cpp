...
int main() {
    ...
    vector<bool> reachToN(n,false);

    auto dfs = [&](auto dfs,ll v)->void{
        if(reachFrom0[v]) return;
        reachFrom0[v] = true;
        Each(nx,g[v]){
            dfs(dfs, nx.fi);
        }
    ...
    auto rdfs = [&](auto rdfs,ll v)->void{
        if(reachToN[v])return;
        reachToN[v] = true;
        Each(nx,rg[v]){
            rdfs(rdfs, nx.fi);
        }
    ...
    while(update){
        ...
        rep(i,m){
            pair<ll,P> &e = edge[i];
            if(!ok[e.fi])continue;
            if(!ok[e.se.fi])continue;
            if(chmin(d[e.se.fi],d[e.fi]+e.se.se)){
                update = true;
            }
        }
        cnt ++;
