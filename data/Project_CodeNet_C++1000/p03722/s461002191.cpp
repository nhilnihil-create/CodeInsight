...
int main() {
	int n, m;
	cin >> n >> m;
	vector<vector<pair<int, llint>>>nodes(n);
	vllint costs(n, ll_max);
	costs[0] = 0;
	rep(i, m) {
		int a, b, c;
		cin >> a >> b >> c;
	...
	rep(i, n - 1) {
		rep(j, n) {
			if (costs[j] == ll_max)continue;
			for (auto k : nodes[j]) {
				if (costs[k.first] == ll_max) {
					costs[k.first] = costs[j] + k.second;
				}
				else {
					costs[k.first] = max(costs[j] + k.second, costs[k.first]);
				}
			}
		}
	}
