...
int main() {
	...
	vector<vector<Pll>> data(N);
	vector<SINT64> road(N,-INF64);
	
	rep(i,0,M) {
		SINT64 a,b,c;
		cin >> a >> b >> c;
		a--; b--;
		data[a].emplace_back(Pll(b,c));
	}
	...
	while(q.size() != 0) {
		SINT64 buf = q.front();
		q.pop();
		rep(i,0,data[buf].size()) {
			if (road[buf]+data[buf][i].S > road[data[buf][i].F]) {
				road[data[buf][i].F] = road[buf]+data[buf][i].S;
				q.push(data[buf][i].F);
			}
		}
