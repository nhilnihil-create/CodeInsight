...
int main(){
 ll w,h;
 cin>>h>>w;
 ll a[h+10][w+10],sum=0;
 rep(i,0,h)rep(j,0,w)cin>>a[i][j],sum+=a[i][j];
 ...
 rep(i,0,h){
  if(i%2==0){
   rep(j,0,w-1){
    if(a[i][j]%2==1){
     a[i][j]--,a[i][j+1]++;
     ans.pb(i+1),ans.pb(j+1),ans.pb(i+1),ans.pb(j+2);
    }
   }
   if(a[i][w-1]%2==1&&i<h-1){
    a[i][w-1]--,a[i+1][w-1]++;
   ans.pb(i+1),ans.pb(w),ans.pb(i+2),ans.pb(w);
   }
  }
  else {
   rrp(j,w,1){
    if(a[i][j]%2==1){
     a[i][j]--,a[i][j-1]++; 
     ans.pb(i+1),ans.pb(j+1),ans.pb(i+1),ans.pb(j);
    }
   }
   if(a[i][0]%2==1&&i<h-1){
    a[i][0]--,a[i+1][0]++;
    ans.pb(i+1),ans.pb(1),ans.pb(i+2),ans.pb(1);
   }
  }
 }
 pr(ans.size()/4);
