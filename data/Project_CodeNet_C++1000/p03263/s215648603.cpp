...
int main() {
	...
	vector<vector<SINT64>> data(N,vector<SINT64>(M));
	SINT64 ans = 0;
	rep(i,0,N) {
		rep(j,0,M) {
			cin >> data[i][j];
	...
	rep(i,0,N) {
		rep(j,0,M-1) {
			if ((data[i][j] % 2) != 0) {
				...
				ans3.emplace_back(i+1);
				ans4.emplace_back(j+2);
				ans++;
			}
		}
	}
	...
	put(ans);
	rep(i,0,ans) {
		puts(ans1[i]);puts(ans2[i]);puts(ans3[i]);put(ans4[i]);
	}
