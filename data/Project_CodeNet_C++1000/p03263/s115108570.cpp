...
int main(void) {
    ...
    REP(i, h) {
        if(i % 2 == 0) {
            REP(j, w) op.push_back(make_pair(i, j));
        } else {
            for(int j = w-1; j >= 0; j--) op.push_back(make_pair(i, j));
        }
    }
    vpprint(op);
    int cnt = 0;
    ...
    REP(i, op.size()-1) {
        ...
        if(b[sy][sx] == 1) {
            if(b[ty][tx] == 0) {
                swap(b[sy][sx], b[ty][tx]);
            } else {
                b[sy][sx] = 0;
                b[ty][tx] = 0;
            }
            anss.push_back(make_pair(sy, sx));
            anst.push_back(make_pair(ty, tx));
            cnt++;
        }
    }
    cout << cnt << endl;
    REP(i, cnt) {
        cout << anss[i].first+1 << " " << anss[i].second+1 << " " << anst[i].first+1 << " " << anst[i].second+1 << endl;
    }
}