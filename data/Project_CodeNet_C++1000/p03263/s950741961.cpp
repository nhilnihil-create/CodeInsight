...
void solve4(){
  ...
  V< pair< pair<int,int>, pair<int,int> > > cf;
  
  FOR(i,0,h){
    if(i%2==0){
      FOR(j,0,w-1){
        if(a[i][j]%2) {a[i][j]--, a[i][j+1]++; cf.eb( pair<int,int>(i+1,j+1), pair<int,int>(i+1,j+2) );}
      }
      if(a[i][w-1]%2){
        if(i+1<h){
          a[i][w-1]--, a[i+1][w-1]++;
          cf.eb( pair<int,int>(i+1,w), pair<int,int>(i+2,w) );
        }
      }
    }
    else{
      IFOR(j,0,w-1){
        if(a[i][j+1]%2) {a[i][j+1]--, a[i][j]++; cf.eb( pair<int,int>(i+1,j+2), pair<int,int>(i+1,j+1) );}
      }
      if(a[i][0]%2){
        if(i+1<h){
          a[i][0]--, a[i+1][0]++;
          cf.eb( pair<int,int>(i+1,1), pair<int,int>(i+2,1) );
        }
      }
    }
  }
  
  //FOR(i,0,h) {FOR(j,0,w) couts(a[i][j]); coutn("");}
  
  coutn( (int)cf.size() );
  FOR(i,0,cf.size()) {auto &x = cf[i]; cout<< x.fs.fs << " " << x.fs.sd << " " << x.sd.fs << " " << x.sd.sd << endl;}
}
