...
int main () {
  ...
  dq_max.push_back(dq.front()); dq.pop_front();
  while (!dq.empty()) {
    dq_max.push_front(dq.back()); dq.pop_back();
    if (dq.empty()) break;
    dq_max.push_back(dq.back()); dq.pop_back();
    if (dq.empty()) break;
    dq_max.push_front(dq.front()); dq.pop_front();
    if (dq.empty()) break;
    dq_max.push_back(dq.front()); dq.pop_front();
    if (dq.empty()) break;
  }
  ...
  ll tot = 0;
  rep(i,n) {
    if (i) tot += abs(dq_max[i] - dq_max[i-1]);
  }
  chmax(ans, tot);

  deque<int> dp_max2, dq2;
  sort(a.rbegin(), a.rend());
  rep(i,n) dq2.push_back(a[i]);
  dp_max2.push_back(dq2.front()); dq2.pop_front();
  while (!dq2.empty()) {
    dp_max2.push_front(dq2.back()); dq2.pop_back();
    if (dq2.empty()) break;
    dp_max2.push_back(dq2.back()); dq2.pop_back();
    if (dq2.empty()) break;
    dp_max2.push_front(dq2.front()); dq2.pop_front();
    if (dq2.empty()) break;
    dp_max2.push_back(dq2.front()); dq2.pop_front();
    if (dq2.empty()) break;
  }
  ...
  tot = 0;
  rep(i,n) {
    if (i) tot += abs(dp_max2[i] - dp_max2[i-1]);
  }
  chmax(ans, tot);
