...
int n,l,r,ta,ta2,ta3;
vector<int> ans,a,b,c,ans2,ans3;
signed main() {
    cin>>n;
    rep(i,n){
        int k;
    ...
    rep(i,n-1){
        ta+=abs(ans[i+1]-ans[i]);
    }
    sort(b.begin(),b.end());
    ans2.push_back(b[n/2-1]);
    ...
    rep(i,n-1){
        ta2+=abs(ans2[i+1]-ans2[i]);
    }
    sort(c.begin(),c.end());
    ans3.push_back(c[n/2+(n&1)]);
    c.erase(c.begin()+n/2+(n&1));
    r=n-2;
    l=0;
    rep(i,n-1){
        if (i&1) {
    ...
    rep(i,n-1){
        ta3+=abs(ans3[i+1]-ans3[i]);
    }
    cout<<max({ta,ta2,ta3})<<endl;
