...
const int N = 4000;
double a, b, h, m;
P<double, double> f(double l, double t){
  t = t/180*acos(-1);
  return mp(l*cos(t), l*sin(t));
}
main(){
  cin.tie(0);
  ios::sync_with_stdio(false);
  cin >> a >> b >> h >> m;
  P<double, double> c = f(a, h*30+m*0.5);
  P<double, double> d = f(b, m*6);
  double ans = sqrt((c.fi-d.fi)*(c.fi-d.fi) + (c.se-d.se)*(c.se-d.se));
  printf("%.12f\n", ans);
}