...
int main(){
    ...
    cin>>hcm>>mcm>>h>>m;
    vll hvc(2), mvc(2);
    double hangel = 0, mangel = 0;
    mangel = m * 6.0;
    hangel = (double) h * 30.0 + 30.0 * ((double)m/60.0);
    hangel *= M_PI/180.0;
    mangel *= M_PI/180.0;
    double hx = cos(hangel) * hcm;
    double hy = sin(hangel) * hcm;
    
    double mx = cos(mangel) * mcm;
    double my = sin(mangel) * mcm;
