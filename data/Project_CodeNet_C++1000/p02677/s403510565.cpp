...
signed main(){
  ...
  cin>>A>>B>>H>>M;
  if(H%12*30<M*5.5){
    if(M*5.5-H%12*30>=180)c=360-(M*5.5-H%12*30);
    else c=M*5.5-H%12*30;
  }
  else{
    if(H%12*30-M*5.5>=180)c=360-(H%12*30-M*5.5);
    else c=H%12*30-M*5.5;
  }
  cout<<fixed<<setprecision(30)<<sqrt(A*A+B*B-2*A*B*cos(PI/2*c/90))<<endl;
}