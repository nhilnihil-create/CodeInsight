...
void ino(vector<vector<int> > &tree,int now)
{
	...
	if(tree[now][1] == minimum && tree[now][2] == minimum)
	{
		cout << " " << tree[now][0];
	}
	else
	{
		if(tree[now][1]!=minimum)
		{
			ino(tree,tree[now][1]);
		}
		cout << " " << tree[now][0];
		if(tree[now][2]!=minimum)
		{
			ino(tree,tree[now][2]);
		}
	}
...
void insert(vector<vector<int> > &tree,int key,int now)
{
	...
	else
	{
		...
		{
			if(tree[now][1] == minimum)	//child none
			{
				tree[now][1] = count;
				tree[count][0] = key;
			}
		...
		{
			if(tree[now][2] == minimum)	//child none
			{
				tree[now][2] = count;
				tree[count][0] = key;
			}
...
void find(vector<vector<int> > &tree, int key,int now)
{
	if(tree[now][0]==key)
	{
		cout << "yes" << endl;
...
int main(void)
{
	...
	string command;
	cin >> n;
    tree = vector<vector<int> >(n,vector<int>(3,minimum));
