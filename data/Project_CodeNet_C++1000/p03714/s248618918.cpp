...
int main() {
  ...
  io.read(a);
  set<Pll> mn, mx;
  vector<ll> mns(3 * N), mxs(3 * N);
  REP(i, N) mn.insert(Pll(a[i], i));
  ll acc = accumulate(a.begin(), a.begin() + N, 0LL);
  FOR(i, N - 1, 2 * N) {
    mns[i] = acc;
    mn.insert(Pll(a[i + 1], i + 1));
    acc += a[i + 1] - mn.begin()->first;
    mn.erase(mn.begin());
  }
  for (int i = 3 * N - 1; i >= 2 * N; --i) mx.insert(Pll(-a[i], i));
  acc = accumulate(a.begin() + 2 * N, a.begin() + 3 * N, 0LL);
  for (int i = 2 * N; i >= N; --i) {
    mxs[i] = acc;
    mx.insert(Pll(-a[i - 1], i - 1));
    acc += a[i - 1] + mx.begin()->first;
    mx.erase(mx.begin());
  }
  ll ans = -LLINF;
  FOR(i, N - 1, 2 * N) { ans = max(ans, mns[i] - mxs[i + 1]); }
  io.writeln(ans);
  return 0;
}