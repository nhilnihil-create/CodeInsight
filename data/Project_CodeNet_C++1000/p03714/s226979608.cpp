...
bool enough(ll T, ll A, ll B, ll N){
  ll cnt = 0;
  rep(i, N){
    cnt += ceil((double)max(h[i]-B*T,0ll)/(A-B));
  }
  return cnt<=T;
...
int main() {
  ...
  ll N;cin>>N;
  vint a(3*N);rep(i,3*N)cin>>a[i];

  priority_queue<ll> q1,q2;
  vint s1(N+1),s2(N+1);
  s1[0] = 0;
  rep(i,N){q1.push(-a[i]);s1[0]+=a[i];}
  repf(i,N,2*N){
    if(-q1.top()<a[i]){
      s1[i-N+1] = s1[i-N]+q1.top()+a[i];
      q1.pop();q1.push(-a[i]);
    }else{
      s1[i-N+1] = s1[i-N];
    }
  ...
  s2[N] = 0;
  rep(i,N){q2.push(a[i+2*N]);s2[N]-=a[i+2*N];}
  for(ll i=2*N-1;i>=N;i--){
    if(q2.top()>a[i]){
      s2[i-N] = s2[i-N+1]+q2.top()-a[i];
      q2.pop();q2.push(a[i]);
    }else{
      s2[i-N] = s2[i-N+1];
    }
