...
ll deep(Matrix &a,ll num) {
	ll max = -1;
	Array pat(a[num].size());
	if (a[num].size() == 0)return 0;

	REP(i, a[num].size()) pat[i]= deep(a, a[num][i]);
	sort(all(pat),greater<ll>());
	REP(i, pat.size()) 	chmax(max, pat[i] + i + 1);
	return max;
}
