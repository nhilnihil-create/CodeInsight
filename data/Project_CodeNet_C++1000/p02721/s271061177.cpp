...
signed main()
{
    ...
    n += n;
    s = "0" + s + "0";
    vector<int> finR(n + 2), finL(n + 2);
    for (int i = c; i <= n; i++)
    {
        finR[i] = finR[i - 1];
        if (s[i - c + 1] == 'o') finR[i] = max(finR[i], finR[i - c] + 1);
    }
    for (int i = n; i >= 1; i--)
    {
        finL[i] = finL[i + 1];
        if (s[i] == 'o') finL[i] = max(finL[i], (i + c <= n + 1 ? finL[i + c] : 0) + 1);
    }
    for (int i = 1; i <= n / 2; i++)
    {
        int x = max(finR[i + c - 2] + finL[i + c], finR[i - 1] + finL[i + 1]);
        if (s[i] == 'o' && x < k && finR[i - 1] + finL[i + c] + 1 >= k) cout << i << endl;
    }
}
