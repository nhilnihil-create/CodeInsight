...
namespace NyaGadget
{
	...
	struct DS_NyaaCumulativeSum
	{
		std::vector<long long> sum;
		DS_NyaaCumulativeSum(std::vector<long long>& v)
		{
		...
		@note
		 ...
		 範囲外のsやtに対しては0を返す。
		**/
		long long Range(long long s, long long t)
		{
			if (s < 0) s = 0;
			if ((long long)sum.size() <= t) t = (long long)sum.size() - 1;
...
int main(void)
{
	...
	if (std::count(all(lans), 0) != N - K) return 0;

	each(i, e, lans)
	{
		if (e == 0) continue;
		if (e == rans[i]) cout << i+1 << endl;
	}
