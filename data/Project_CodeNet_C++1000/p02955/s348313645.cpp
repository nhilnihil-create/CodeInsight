...
namespace NyaGadget
{
	...
	**/
	void NT_DivisorTable(long long n, vector<long long>& ans)
	{
...
int main(void)
{
	...
	vt<ll> divisor;
	NT_DivisorTable(Sum(all(A)), divisor);

	ll ans = 1;
	each(i, ed, divisor)
	{
		vt<ll> modA = A;
		each(j, ea, modA) ea %= ed;
		sort(all(modA));

		each(j, ea, modA)
		{
			ll suml = Sum(modA.begin(), modA.begin() + j);
			ll sumr = ed * (Size(modA) - j) - Sum(modA.begin() + j, modA.end());
			if (suml != sumr) continue;
			if (suml <= K) ans = max(ans, ed);
		}
	}
