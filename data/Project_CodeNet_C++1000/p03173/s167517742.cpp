...
/*
    ...
    Title : DP_N_Modified.cpp
    Date : 03/09/2020   >> 13:15:37
*/
...
ll solve(vl& a, vl& pre, int i, int j) {
    if (i >= j)
        return 0;
    ...
    ll mn = INF(ll);
    rep(k, i, j) {
        mn = min(mn, solve(a, pre, i, k) + solve(a, pre, k + 1, j) + pre[j] - ((i) ? pre[i - 1] : 0));    
    }    
