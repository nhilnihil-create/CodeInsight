...
{ 
  ...
  // read(k)
  readarr(a,n)
  f(i,0,405) f(j,0,405) dp[i][j] = INF;
  // f(i,1,100005) dp[i][n] = 0;
  ll pre[405];
  pre[0] = 0;
  pre[1] = a[0];
  f(i,2,n+1) pre[i] = pre[i-1]+a[i-1];
  f(i,1,n+1) dp[i][i]= 0;
  f(len, 2, n+1)
  {
    ...
    {
      f(k,st,st+len-1)
      dp[st][st+len-1] = min(dp[st][st+len-1] , pre[st+len-1]-pre[st-1] + dp[st][k]+dp[k+1][st+len-1]);
    }
  }
  cout<<dp[1][n]<<endl;
}
