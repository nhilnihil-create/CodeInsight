...
signed main()
{
	...
	while(t--)
	{
		...
		in.resize(n+1);
		cn(in,1,n);
		dp.resize(n+1,vr(n+1,0));
		dp2.resize(n+1,vr(n+1,0));
		f(1,n)
		{
			dp2[i][i]=in[i];
		}
		f(2,n)
		{
			...
			while(y<=n)
			{
				...
				for(ll k =x;k<y;k++)
				{
					if((dp[x][k]+dp[k+1][y]) +dp2[x][k] +dp2[k+1][y] < ans)
					{
							ans=min(ans,(dp[x][k]+dp[k+1][y]) +dp2[x][k] + dp2[k+1][y]);
							index=k;
					}
				}
				dp[x][y]=ans;
				dp2[x][y]=dp2[x][index]+dp2[index+1][y];
				x++;
				y++;
			}
			//print2D(dp);
