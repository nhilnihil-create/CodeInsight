...
void solve(){
	int n; cin>>n;
	vl v(n+1,0); fore(i,0,n)cin>>v[i];
	vvl dp(n,vl(n,-1));
	vl sum(n+1);
	sum[0]=v[0];
	fore(i,1,n+1)sum[i]=sum[i-1]+v[i];
	auto f=[&](auto f, int l, int r)->lli{
		if(l==r)return 0;
		if(dp[l][r]!=-1)return dp[l][r];
		dp[l][r]=INF;
		fore(i,l,r)dp[l][r]=min(dp[l][r],f(f,l,i)+f(f,i+1,r)+sum[r]-(l?sum[l-1]:0));
		return dp[l][r];
	};
	cout<<f(f,0,n-1)<<ENDL;
}
