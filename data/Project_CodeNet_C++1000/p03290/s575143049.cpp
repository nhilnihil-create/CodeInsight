...
int main()
{
    ...
    int d, g, ans{1000000}, x, totalpoints, totalprobrems, p;
    cin >> d >> g;
    VI not_complete;
    VVI problems(d, VI(2));
    repr(i, d)
    ...
    rep(k, 1 << d)
    {
        ...
        rep(j, d)
        {
            if (x % 2)
            {
                totalpoints += (100 * (d - j) * problems[j][0]) + problems[j][1];
                totalprobrems += problems[j][0];
            }
            else
            {
                not_complete.emplace_back(j);
            }
            x /= 2;
        }
        ...
        if (totalpoints >= g)
        {
            ans = min(ans, totalprobrems);
        }
        else
        {
            for (int i : not_complete) if (g > totalpoints)
            {
                p = min(problems[i][0], (g - totalpoints + (d - i - 1) * 100) / ((d - i) * 100));
                totalprobrems += p;
                totalpoints += p * (100 * (d - i));
            }
        }
        ans = min(ans, totalprobrems);
    }
