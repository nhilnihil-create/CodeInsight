...
int main(void){
  ...
  if( n%2==0 ){
    ...
    ll sum=n+1;
    for(ll i=1;i<=n/2;i++){
      vec.push_back( make_pair(i,sum-i) );
    }
    ...
    while(!vec.empty()){
      ...
      vec.pop_back();
      for(auto pp:vec){
        cout << p.first << " " << pp.first << endl;
        cout << p.first << " " << pp.second<< endl;
        cout << p.second << " " << pp.first<< endl;
        cout << p.second << " " << pp.second<< endl;
      }
    ...
    //cout << vec.size()*(vec.size()+1) << endl;
    cout << (n-1)*(n-3)/2 + 2*vec.size() << endl;
    for(auto pp:vec){
      cout << sum << " "<< pp.first << endl;
      cout << sum << " "<< pp.second << endl;
    }
