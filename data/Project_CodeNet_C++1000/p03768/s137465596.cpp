...
void dfs(int v, int t, int dep){
    if(dp[v][dep] != -1) return;
    dp[v][dep] = t;
    if(dep == 0) return;

    for(auto& e : G.adj[v]) dfs(e.to, t, dep-1);
    return;
}

void Main(){
    int N,M; cin >> N >> M; G.resize(N); dp.resize(N,VI(11,-1));
    REP(i,M){
        int a,b; cin >> a >> b; a--; b--;
