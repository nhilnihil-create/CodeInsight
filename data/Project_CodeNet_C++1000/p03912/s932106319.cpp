...
signed main() {
    ...
    REP(i, N) mo[X[i] % M]++;
    REP(i, N) same[X[i]]++;
    REP(i, MA) { snum[i % M] += same[i] / 2; }
    ll ans = 0;
    ans += mo[0] / 2;
    if(M % 2 == 0) ans += mo[M / 2] / 2;
    FOR(i, 1, (M + 1) / 2) {
        ll tmp = 0;
        REP(j, min(mo[i], mo[M - i]) + 1) {
            ll srem = mo[i] - j, lrem = mo[M - i] - j;
            tmp = max(tmp, j + min(srem / 2, snum[i]) + min(lrem / 2, snum[M - i]));
        }
        ans += tmp;
    }
    cout << ans << "\n";
