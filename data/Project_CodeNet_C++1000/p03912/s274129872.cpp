...
int main() {
	...
	vector<SINT64> cnt1(M);
	vector<SINT64> cnt2(M);
...
*/
	...
	rep(i,1,M) {
		...
		if (i == j) {
			...
			if (cnt1[i] == cnt1[j]) {
				ans += cnt1[i];
				ans += cnt2[i]/2 + cnt2[j]/2;
			} else if (cnt1[i] > cnt1[j]) {
				SINT64 a = MIN(cnt1[i], cnt1[j]+cnt2[j]);
				ans += a;
				ans += cnt2[i]/2;
				ans += (cnt2[j]-(a-cnt1[j]))/2;
			} else {
				SINT64 a = MIN(cnt1[i]+cnt2[i], cnt1[j]);
				ans += a;
				ans += cnt2[j]/2;
				ans += (cnt2[i]-(a-cnt1[i]))/2;				
			}
		}
	}
	put(ans);
