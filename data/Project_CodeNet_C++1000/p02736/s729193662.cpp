...
void solve(){
    ...
    bool f = false; 
    for(i = 0; i < n; ++i){
        ret = (ret + ((ara[i] % 2) * lucas(n - 1 , i)) % 2) % 2;
        if(ara[i] == 1) f = true; 
    }
    if(ret == 1){ pi(1), nl; return; }
    if(f){ pi(0), nl; return; }
    for(i = 0; i < n; ++i) ret = (ret + (((ara[i] >> 1) % 2) * lucas(n - 1 , i)) % 2) % 2;
    if(ret == 1){ pi(2), nl; return; }
    pi(0), nl; return; 
}
int main(){
    ...
    int i, j; 
    si(n), ss(str); 
    for(i = 0; i < n; ++i) ara[i] = str[i] - '0', ara[i]--;  
    solve();
}
