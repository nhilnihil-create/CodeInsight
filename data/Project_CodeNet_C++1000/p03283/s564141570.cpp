...
int main(){
  ...
  // 二次元累積和
  // 都市Li をh, 都市Ri をwと見立てて、(Li, Ri) をマス上にとる
  // すると、Li, Ri ともに h=pi~qi, w=pi~qi の範囲内にある(Li, Ri)の組（＝列車）を都市の数を二次元累積和の差し引きで求められる
  // それがそのまま答えになる
  // li=0, ri=0 は番兵
  vector<vector<ll> > cumsum2d(N+1, vector<ll>(N+1, 0));
  ...
  REP(i, 1, N+1) REP(j, 1, N+1){
    cumsum2d[i][j] += cumsum2d[i-1][j] + cumsum2d[i][j-1] - cumsum2d[i-1][j-1];
  }
  ...
  rep(i, Q){
    ll p = il();
    ll q = il();
    cout << cumsum2d[q][q] - cumsum2d[q][p-1] - cumsum2d[p-1][q] + cumsum2d[p-1][p-1] << endl;
  }
