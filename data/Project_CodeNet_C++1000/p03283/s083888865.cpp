...
bool isOK(char a, char b){
  ...
  if(a == '@'){
    if(b == 'a' || b == 't' || b == 'c' || b == 'o' || b == 'd' || b == 'e' || b == 'r' || b == '@' ){
      return true;
    }
  }
  if(b == '@'){
    if(a == 'a' || a == 't' || a == 'c' || a == 'o' || a == 'd' || a == 'e' || a == 'r' || a == '@' ){
      return true;
    }
  }
  return false;
...
int main() {
  ...
  sort(ALL(a),[](sf x, sf y){
    if(x.r == y.r) return x.q < y.q;
    return x.r < y.r;
  });
  ...
  REP(i,M+Q){
    if(a.at(i).q == 0){
      ...
      int ans=0;
      FOR(j,a.at(i).l,a.at(i).r+1){
        ans += c.at(j);
      }
      d.PB(MP(a.at(i).n,ans));
    }
  }
