...
namespace NyaGadget
{
	...
	struct DS_Imos2D
	{
		...
		DS_Imos2D(long long w, long long h)
		{
			grid.resize(h + 1);
			for (auto&& e : grid) e.resize(w + 1);
		}

		void Add(long long hmin, long long hmax, long long wmin, long long wmax, long long v)
		{
			grid[hmin][wmin] += v;
			grid[hmin][wmax + 1] -= v;
			grid[hmax + 1][wmin] -= v;
		...
		void Update(void)
		{
			for (auto i = 0LL; i < (long long)grid.size(); ++i)
			{
				for (auto j = 1LL; j < (long long)grid[i].size(); ++j) grid[i][j] += grid[i][j - 1];
			}
			for (auto i = 1LL; i < (long long)grid.size(); ++i)
			{
				for (auto j = 0LL; j < (long long)grid[i].size(); ++j) grid[i][j] += grid[i - 1][j];
			}
		}
	};
}
...
{
	ll N, M, Q; cin >> N >> M >> Q;
	vt<pll> LR(M); each(i, e, LR) cin >> e.first >> e.second;
	vt<pll> pq(Q); each(i, e, pq) cin >> e.first >> e.second;

	DS_Imos2D imos(N+1, N+1);
	each(i, e, LR) imos.Add(1, e.first, e.second, N, 1);
	imos.Update();

	each(i, e, pq) cout << imos.grid[e.first][e.second] << endl;
	return 0;
}
