...
int main () {
  ...
  // ans[p][q] = x[p][p] + x[p][p+1] + ... + x[p][q] + x[p+1][p] + x[p+1][p+1] + ... x[q][q]

  vector<vector<int>> csum(n+1, vector<int>(n+1));
  rep(i,n)rep(j,n) csum[i+1][j+1] = csum[i][j+1] + csum[i+1][j] - csum[i][j] + x[i][j];

  rep(qi,Q) {
    int p, q;
    cin >> p >> q;
    p--, q--;
    int ans = csum[q+1][q+1] - csum[p][q+1] - csum[q+1][p] + csum[p][p];
    cout << ans << endl;
  }
