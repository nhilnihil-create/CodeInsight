...
int main(){
  ...
  ll ans = LINF;
  REP(i, 'a', 'z'+1){
    char tg = i; // 全ての文字をtgにするよう試みる
    string tmps = s; // これをいじっていく

    // 文字が1つもなければそもそも不可能
    if (s.find(tg) == string::npos) continue;

    ll nowlen = len;
    while(nowlen > 1){
      nowlen--;
      rep(i, nowlen){
        if (tmps[i]==tg || tmps[i+1]==tg) tmps[i] = tg;
        else tmps[i] = 'X';
      }
      if (tmps.substr(0, nowlen)==string(nowlen, tg)) break;
    }

    chmin(ans, len - nowlen);
