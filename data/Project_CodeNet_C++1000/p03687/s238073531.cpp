...
void Main() {
    string s = gs;
    map<char,vec<int>> chars;
    REP(i,s.size()) {
        chars[s[i]].add(i);
    }
    int minRes = INT_MAX;
    for(auto charCount: chars) {
        //cout << charCount.first << endl;
        vec<int> count = charCount.second;
        //print(count);
        int gapMax = 0;
        chmax(gapMax, count[0]);
        REP(i, count.size()-1) {
            chmax(gapMax, count[i+1] - count[i] - 1);
        }
        chmax(gapMax, (int)s.size() - *count.rbegin() - 1);
        chmin(minRes, gapMax);
    }
    print(minRes);
}
