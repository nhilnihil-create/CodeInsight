...
int main() {
	...
	for(int i = 0; i < N; ++i){
		int l = (i-1+N)%N;
		int r = (i+1)%N;
		if(B[l]<B[i]&&B[i]>B[r]) pq.push({B[i],i});
	}
	ll ans = 0;
	while(pq.size()){
		auto p = pq.top();
		...
		B[c] -= d;
		corner(B[c]<A[c],-1);
		ll ng = -1,ok = (B[c]-A[c])/d,md;
		while(ok-ng>1){
			md = (ok+ng)/2;
			ll diff = md*d;
			(B[c]-diff<=B[l]&&B[c]-diff<=B[r]?ok:ng)=md;
		}
		B[c] -= ok*d;
		ans += ok;

		if(B[c]<B[r]&&B[r]>B[(r+1)%N]&&B[r]!=A[r]) pq.push({B[r],r});
		if(B[(l-1+N)%N]<B[l]&&B[l]>B[c]&&B[l]!=A[l]) pq.push({B[l],l});
		// print(p,ok);
		// print(B);
		// print(A);
	}
