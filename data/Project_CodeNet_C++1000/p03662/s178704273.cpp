...
signed main(){
  ...
  vector<pair<ll,ll>>dist(n,{0,0});
  dist[0].fi=0;
  // fennec
  queue<pair<ll,ll>> que;
  for(auto x:adj[0])que.emplace(x,1);
  while(!que.empty()){
    ll pos,cos;
    tie(pos,cos)=que.front();
    que.pop();
    dist[pos].fi=cos;
    for(auto x:adj[pos]){
      if(dist[x].fi==0)que.emplace(x,cos+1);
    }
  }
  // snuke
  dist[n-1].se=0;
  for(auto x:adj[n-1])que.emplace(x,1);
  while(!que.empty()){
    ll pos,cos;
    tie(pos,cos)=que.front();
    que.pop();
    dist[pos].se=cos;
    for(auto x:adj[pos]){
      if(dist[x].se==0)que.emplace(x,cos+1);
    }
  }
  ll fnc=0,snk=0;
  rep(i,n){
    if(dist[i].fi>dist[i].se)snk++;
    else fnc++;
  }
  if(fnc>snk)out("Fennec");
  else out("Snuke");
