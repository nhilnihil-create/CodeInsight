...
int main(){
    ...
    vector<int> dist_from_black(N,inf),dist_from_white(N,inf);
    dist_from_black[0] = 0;
    dist_from_white[N - 1] = 0;
    queue<int> que;
    que.push(0);
    while(!que.empty()){
        int n = que.front();
        que.pop();
        for(auto x : G[n]){
            if(dist_from_black[x] == inf){
                que.push(x);
                dist_from_black[x] = dist_from_black[n] + 1;
            }
        }
    }
    que.push(N - 1);
    while(!que.empty()){
        ...
        que.pop();
        for(auto x : G[n]){
            if(dist_from_white[x] == inf){
                que.push(x);
                dist_from_white[x] = dist_from_white[n] + 1;
            }
        }
    }
    int black = 0,white = 0;
    rep(i,N){
        if(dist_from_black[i] <= dist_from_white[i])black++;
        else white++;
    }
    if(black <= white)cout << "Snuke\n";
    else cout << "Fennec\n";
}