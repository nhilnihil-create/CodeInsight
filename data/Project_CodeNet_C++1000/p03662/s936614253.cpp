...
int main(void)
{
	...
	for(i=1 ; i<=n ; i++) dist_1[i]=1e9;
	for(i=1 ; i<=n ; i++) dist_n[i]=1e9;
	dist_1[1]=dist_n[n]=0;
	Q.push(1);
	while(!Q.empty())
	{
		...
		{
			int nxt=edge[x][i];
			if(dist_1[nxt]>dist_1[x]+1)
				dist_1[nxt]=dist_1[x]+1, Q.push(nxt);
		}
	}
	Q.push(n);
	while(!Q.empty())
	{
		...
		{
			int nxt=edge[x][i];
			if(dist_n[nxt]>dist_n[x]+1)
				dist_n[nxt]=dist_n[x]+1, Q.push(nxt);
		}
	}
	for(i=1 ; i<=n ; i++) if(dist_1[i]<=dist_n[i]) a++;
	if(a>n-a) cout<<"Fennec"; 
	else cout<<"Snuke"; return 0;
}
