...
int main() {
    ...
    cin >> t;
    while (t--) {
        LL cans, cost, line, supply;
        cin >> cans >> cost >> line >> supply;
        // 第一天就壞
        if (cans < cost) {
            printf("No\n");
            continue;
        ...
        if (cost <= line) {
            ...
            // line < cost
            // 若有個時刻 cost > cans > line 
            LL gcd = Number::gcd(cost, supply);
            // printf("%lld %lld %lld %lld\n", cans, cost, line, supply);
            // printf("gcd = %Lf\n", gcd);
            // 是否有 k 使得 k * gcd < cans, k * gcd > cans + line - cost
            LL high = cans - line;
            LL low = cans - cost;
            LL k = high % gcd == 0 ? high / gcd - 1 : high / gcd;
            if (k * gcd > low) {
                printf("No\n");
            } else {
                printf("Yes\n");
