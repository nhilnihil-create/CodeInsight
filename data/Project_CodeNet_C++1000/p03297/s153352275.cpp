...
bool solve(lli a, lli b, lli c, lli d){
  ...
  if(a < b) return false;
  // 以降 a >= bを仮定して良い
  if(d < b) return false; // 補充能力の限界を超える
  // 以降 b <= dを仮定して良い
  /*************************************/
  // c本数以下 => d本追加
  // " 実質的に "q = d % b本 追加とみなせる (特に, b　> cならば)
  const lli q = d % b;
  if(c >= b) return true; // 毎日補充されるとき
  // a , d >= b and b > cのとき
  // Q, どんな条件の時になくなるの？？
  //    --> c < f(x) < bとなる時が発生するとき !!!
  // then, find max a % b + kq mod b ~ (a + kq) (mod b)
  //  a + kq = mb + u, kq - mb = u - a; => gcd(q, b)を動く!!
  // 求) maximazing u - a = k * GCD => u = a + k * GCD
  a %= b;
