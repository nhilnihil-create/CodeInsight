...
void dfs(ll prev, ll now, ll pc){
  map<ll,ll> mp = graph[now];
  ll c = 0;
  for(auto itr = mp.begin(); itr != mp.end(); itr++){
    ll next = itr->first; ll nc = itr->second;
    if(next == prev){ continue; }
    while(c == pc){ c++; }
    graph[now][next] = c; graph[next][now] = c; 
    dfs(now,next,c);
    //cout << now << next << c << endl;
    c++;
  }
}
...
int main()
{
  ...
  FOR(i,N-1){ 
    cin >> a[i] >> b[i]; 
    a[i] --; b[i] --;
    graph[a[i]].emplace(b[i],-1);
    graph[b[i]].emplace(a[i],-1);
  }
  
  //main
  ll K = 0LL; FOR(from,N){ K = max(K,(ll)(graph[from].size())); }
  dfs(-1,0,-1);
