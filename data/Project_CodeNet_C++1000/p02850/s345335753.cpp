...
void Main()
{
	...
	vec<int> colorRoad(n - 1, 0);

	// pair< いるノード , 来た道 >
	queue<pint> q;
	q.push(pair(road[0].first , 0));
	q.push(pair(road[0].second, 0));
	colorRoad[0] = 1;
	while (!q.empty())
	{
		int now = q.front().first;
		int fromRoad = q.front().second;
		q.pop();

		int color = 1;

		for (int toRoad : g[now])
		{
			if (color == colorRoad[fromRoad])
			{
				color++;
			}

			if (colorRoad[toRoad] == 0)
			{
				colorRoad[toRoad] = color;

				int to = now == road[toRoad].first ? road[toRoad].second : road[toRoad].first;

				q.push(pair(to, toRoad));
	...
	cout << *max_element(all(colorRoad)) << '\n';
	rep(i, n - 1)
	{
		cout << colorRoad[i] << '\n';
	}
