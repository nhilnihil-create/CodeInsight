...
int cnt[114514];
int ans[114514];
pair<pair<pair<int,int>,int>,int> pp[114514];
pair<pair<int,int>,int> ppp[214514];
int cc[114514];
signed main(){
	ic(n)
	Graph G(n);
	...
	dfs(G,root,-1,0);
	rep(0,i,n-1){
		pp[i].f.f.f=min(depth[a[i]],depth[b[i]]);
		pp[i].f.f.s=a[i];
		pp[i].f.s=b[i];
		pp[i].s=i;
		ppp[i].f.f=a[i];
	...
	rep(k,i,n-1){
		...
		int to=0,ris=0;
		int tou=0,rist=0;
		if(depth[pp[i].f.f.s]<depth[pp[i].f.s]){
			to=pp[i].f.f.s;
			ris=oya[to];
		}
		else{
		...
		while(r-l>=1){
			...
			if(ppp[m].f.f==rist){
				...
				else if(ppp[m].f.s<tou)l=m+1;
				else r=m;
			}
			else if(ppp[m].f.f<rist)l=m+1;
			else r=m;
		}
		ans[pp[i].s]=(ans[place]+cc[to])%k+1;
		cc[to]++;
	}
	c(k)
	rep(0,i,n-1)c(ans[i])
