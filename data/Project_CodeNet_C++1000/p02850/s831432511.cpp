...
queue<ll> q;
//p:parent d:depth
vll used(100002),p(100002),d(100002);
vector<vll>adj(100002);
...
void bfs(ll s)
{
    ...
    while (!q.empty()) 
    {
        ...
        for (ll u : adj[v]) 
        {
            if (!used[u]) 
            {
                used[u] = 1;q.push(u);p[u] = v;
                col++;
                if(edge[mp(p[v],v)]==col) col=edge[mp(p[v],v)]+1;
                edge[mp(v,u)]=col;edge[mp(u,v)]=col;
                mx=max(mx,col);
            }
        }
    }
...
int main()
{
    ...
    For(i,1,T)
    {
        ...
        bfs(1);
        cout<<mx<<endl;
        For(j,1,n-1) cout<<edge[mp(x[j],y[j])]<<endl;
    }
}
