...
void solve() {
	...
	q.push({0,-1});
	while(!q.empty()) {
		int p=q.front().fi, c=q.front().se;
		q.pop();
		int nowc=1;
		for(int i=0; i<pnt[p].deg; i++) {
			int egnum=pnt[p].ne[i];
			if(egclr[egnum]!=-1) continue;
			if(nowc==c) nowc++;
			egclr[egnum]=nowc;
			q.push({pnt[p].np[i],nowc});
			chmax(mx,nowc);
			nowc++;
		}
	}
	...
	cout<<mx<<endl;
	for(int i=0; i<N-1; i++) cout<<egclr[i]<<endl;
}
