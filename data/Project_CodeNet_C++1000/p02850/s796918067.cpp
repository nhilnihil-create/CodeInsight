...
struct Edge {
  int to, numb;
  Edge(int to, int numb){
    this->to = to, this->numb = numb;
  }
};
...
int main() {
  ...
  REP(i,N) chmax(cnt,SZ(tree.at(i)));
  ...
  for(q.push(0);!q.empty();q.pop()){
    ...
    int pcolor;
    int t = 1;
    REP(i,SZ(tree.at(nowp))){
      Edge nxt = tree.at(nowp).at(i);
      if(color.at(nxt.numb)>0){
        pcolor = color.at(nxt.numb);
        break;
      }
    }
    REP(i,SZ(tree.at(nowp))){
