...
vi v(siz);
ll t[siz][siz];
ll solve(ll l,ll r,ll n,int mov)
{
    if(l>r)
        return 0;
    if(t[l][r]!=-1)
            return t[l][r];
    if(mov)
    return t[l][r]=(max(v[l]+solve(l+1,r,n,mov^1),v[r]+solve(l,r-1,n,mov^1)));
    else
    return t[l][r]=(min((-1*v[l])+solve(l+1,r,n,mov^1),(-1*v[r])+solve(l,r-1,n,mov^1)));
