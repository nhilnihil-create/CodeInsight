...
ll rec(ll l=0,ll r=n){
    ...
    ll oe=n-(r-l);
    ll& ret=dp[l][r];
    if(seen[l][r]) return ret;
    seen[l][r]=1;
    if(oe%2==0) ret=max(a[l]+rec(l+1,r),a[r-1]+rec(l,r-1));
    else ret=min(-a[l]+rec(l+1,r),-a[r-1]+rec(l,r-1));
    return ret;
}
...
int main(){
    ...
    a.resize(n);
    rep(i,n) cin>>a[i];
    dp.resize(n+1,V<ll>(n+1,0));
    seen.resize(n+1,V<bool>(n+1,0));
    ll tmp=rec(0,n);
    cout<<dp[0][n]<<endl;
}
