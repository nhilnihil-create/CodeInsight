...
ll getdp( int x, int y, int st ) {
    if ( x > y ) return 0;
    ll &ret = dp[x][y][st];
    if ( vis[x][y][st] ) return ret;

    vis[x][y][st] = true;
    int nst = st^1;
    if ( st ) ret = min(getdp(x+1, y, nst) - a[x], getdp(x, y-1, nst) - a[y]);
    else ret = max(getdp(x+1, y, nst) + a[x], getdp(x, y-1, nst) + a[y]);
    return ret;
}

int main () {
    scan1(n);
    for ( int i = 1; i <= n; i++ ) scanl1(a[i]);
    printl1(getdp(1, n, 0));
}