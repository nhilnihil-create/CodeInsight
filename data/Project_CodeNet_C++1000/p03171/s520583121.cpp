...
const int N = 3001;
ll dp[2][N+1][N+1];     // dp[player][left][right]
ll ar[N+1], n;
...
ll calc(int left, int right, bool player, ll score1, ll score2)
{
    ...
    if(dp[player][left][right] != -1)   // Already computed
        return dp[player][left][right];
    
    if(player%2 == 0)   // Player 1
        return dp[player][left][right] = max(calc(left+1, right, player^1, score1+ar[left], score2) + ar[left],
                                        calc(left, right-1, player^1, score1+ar[right], score2) + ar[right]);
    
    // Player 2
    return dp[player][left][right] = min(calc(left+1, right, player^1, score1, score2+ar[left]) - ar[left],
                                        calc(left, right-1, player^1, score1, score2+ar[right]) - ar[right]);
}
