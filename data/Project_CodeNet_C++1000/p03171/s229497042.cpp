...
*/
ll dp[3005][3005][2];
ll solve(vector<ll>& a,ll l,ll r,ll turn){
	if(l>r){
		re 0;
	}
	if(dp[l][r][turn]!=-1){
	...
	if(turn==1){
		re dp[l][r][turn]=max(a[l]+solve(a,l+1,r,0),a[r]+solve(a,l,r-1,0)); 
	}
	else{
		re dp[l][r][turn]=min(solve(a,l+1,r,1),solve(a,l,r-1,1));
	}
}
void solve(){
	bhar(dp,-1);
