...
int main(){
    ...
    ll summy[N];
    summy[0] = a[0];
    for(int i = 1; i < N; i++) summy[i] = summy[i-1] + a[i];
    int prev = 0;
    for(int i = 0; i < N; i++) {
        if(summy[i]<summy[N-1]/2){
            prev = i;
        }
    }
    int after = prev+1;
    if(N==2){
        ...
        // cout<<summy[prev]<<endl;
        cout<<min(abs(summy[after]-(summy[N-1]-summy[after])),abs(summy[prev]-(summy[N-1]-summy[prev])))<<endl;
    }
    return 0;
}
