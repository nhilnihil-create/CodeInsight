...
int main() {
    ...
    //BCをDに置換してstackで扱いやすくする BCは崩れないので可能
    for(int i = 0;i<s.size()-1;i++){
        if(s.at(i) == 'B' && s.at(i+1) == 'C'){
            S.push_back('D');
            i++;
            if(i == s.size()-2) S.push_back(s.at(i+1));
        }
        else{
            S.push_back(s.at(i));
        }
    }
    long long renzokuA = 0;
    //stackするかと思ったけどswapが定数時間で神すぎる件について
    //と思ったけどよく考えたらA,Dをswapさせたら自動的にAが前にくる
    //なのでB,Cが登場するまでのAの個数を考えてれば、勝手に繋がってくれる
    for(int i = 0;i<S.size()-1;i++){
        if(S.at(i) == 'A') renzokuA++;
        else if(S.at(i) != 'D') renzokuA = 0;
        if(S.at(i+1) == 'D'){
            ans += renzokuA;
        }
    }
