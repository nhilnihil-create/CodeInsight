...
int main(void) {
    ...
    REP(i,N) cin >> a[i];
    VI a_s(a);
    sort(a_s.begin(), a_s.end(), greater<LL>() );
    LL ng = (-1); //「index = 0」が条件を満たすこともあるので、初期値は -1
    LL ok = a.size() - 1; // 「index = a.size()-1」が条件を満たさないこともあるので、初期値は a.size()
    LL board = ((N+1)*(N)/2);
    ...
    while (abs(ok - ng) > 1) {
        ...
        REP(i,N) {
            ...
            a2[i+1] += a2[i];
            minv = min(a2[i+1],minv);
        }
        REP(i,N+1) a2[i] -= minv;
        SegTree<RSQ> sg(N+1,0);
        REP(i,N+1) {
            inv_num += (i - sg.find(0,a2[i]+1));
            sg.update(a2[i],1);
        }
        if (board >= inv_num*2) ok = mididx;
        else ng = mididx;
    }
    cout << a_s[ok] << endl;
}
