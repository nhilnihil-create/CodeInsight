...
int solve(vel &a, int x) {
	...
	int now_sum = 0;
	for (int i = n+1; i <= 2 * n+2; i++) { now_sum += count[i]; }
	int ans = 0;
	for (int i = 0; i < n; i++) {
		ans += now_sum;
		if (sum[i + 1] == sum[i] + 1) { now_sum -= count[sum[i]]; }
		else { now_sum += count[sum[i] - 1]; }
		now_sum--;
		count[sum[i + 1]]--;
	}
	return ans;
...
signed main() {
	...
	while (upp - low > 1) {
		int mid = (upp + low) / 2;
		int count = solve(a, b[mid]);
		if (count >= (m + 1) / 2) { low = mid; }
		else { upp = mid; }
	}
	cout << b[low] << endl;
	return 0;
