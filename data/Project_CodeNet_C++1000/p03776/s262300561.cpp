...
int main()
{
        ...
        vll comp(N);
        rep(i, 0, N) comp[i] = lower_bound(all(tmp), v[i]) - tmp.begin();
        // print1d(comp);

        ll mx = accumulate(v.end()-A, v.end(), 0LL);

        ld ans1 = accumulate(v.end()-A, v.end(), 0LL) / (ld)A;

        ll kind = len(tmp);

        vll allcnt(kind);
        rep(i, 0, N) allcnt[comp[i]]++;

        ll ans2 = 0;
        rep(i, A, B+1) {
                ll sm = accumulate(v.end()-i, v.end(), 0LL);
                if (sm*A != mx*i) continue;

                vll cnt(kind);
                repreverse(j, N-1, N-i) {
                        cnt[comp[j]]++;
                }
                


                //rep(j, 0, kind) if (cnt[j] > allcnt[j]) continue;
                ll pr = 1;
                
                rep(j, 0, kind) {
                        
                        ll x = comb(allcnt[j], cnt[j]);
                        pr *= x;
                        // cout << allcnt[j] << " C " << cnt[j] << endl;
                }

                ans2 += pr;
        }

        cout << setprecision(21) << ans1 << endl;
        cout << ans2 << endl;
}
