...
int main() {
  ...
  vector<int> XS(N);
  REP(i,N) cin>>XS[i];
  // 原則左から順番に移動していくでも問題はない
  // どことどこの間を歩くかして、それ以外は全てテレポートなのだから問題はないから
  // o...o..o.oo...o..oo...
  // 歩く距離はA*x<Bとなるところ
  vector<ll> dists(N-1);
  REP(i,N-1) dists[i] = (XS[i+1]-XS[i])*A;
  sort(ALL(dists));

  int idx = lower_bound(ALL(dists),B)-dists.begin();
  ll ans = 0;
  REP(i,idx) ans += dists[i];
  ans += B*(N-idx-1);
  OUT(ans);
