#include <iostream>
#include <vector>
#include <algorithm>
#include <queue>
#include <cmath>
#include <functional>
#include <set>
#define DB cerr<<"D"<<endl
#define pb push_back
using namespace std; using ll=long long; using ld=long double; const int INF=1e9; const ll LINF=1e18; const double dINF = 1e18; const ld ldINF = 1e18; const double EPS = 1e-6;
template<typename T, typename U, typename O> void caut(T a, U b, O c){cout<<"("<<a<<","<<b<<","<<c<<") ";}
template<typename T, typename U> void caut(T a, U b){cout<<"("<<a<<","<<b<<") ";}
template<typename T> void caut(T a){cout<<"("<<a<<") ";}
using P=pair<double,int>;
const ll M = 1e9+7;

void add(ll& x, ll y) {x+=y; x%=M;}

ll mod_pow(ll x, ll a) { ll an = 1; while(a > 0) { if (a&1) an = an * x % M; x = x * x % M; a >>= 1;} return an;}

int main() {
  int n; cin>>n;
  string _; cin>>_; int m = _.size();

  ll dp[2][n+1]; fill(dp[0],dp[2],0);
  dp[0][0]=1;
  for(int i=0;i<n;i++) {
    int nw = i%2, nx = (i+1)%2;
    fill(dp[nx],dp[nx]+n+1,0);
    for(int j=0;j<n;j++) {
      add(dp[nx][max(0,j-1)],dp[nw][j]);
      add(dp[nx][j+1],dp[nw][j]*2);
    }
  }
  ll an = mod_pow(mod_pow(2, m), M-2);
  an = an * dp[n%2][m] % M;
  cout<<an<<endl;
}