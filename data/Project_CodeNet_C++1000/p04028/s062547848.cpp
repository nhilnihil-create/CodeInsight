...
int main() {
  ...
  rep(i,n){
    rep(f,n+1){
        dp[i+1][f+1]=dp[i+1][f+1]+dp[i][f]%mod+dp[i][f]%mod;
        if(f!=0){
          dp[i+1][f-1]=dp[i+1][f-1]+dp[i][f]%mod;
        }else{
          dp[i+1][0]=dp[i+1][0]+dp[i][f]%mod;
        }
    }
  }
  
  rep(i,s.length())
  dp[n][s.length()]=dp[n][s.length()]*500000004ll%mod;

  pri(dp[n][s.length()]%mod);
  return 0;
}
