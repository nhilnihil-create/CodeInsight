...
int main(){
 ...
 cin>>n;
 ll c[11][11];
 rep(i,0,10)rep(j,0,10)c[i][j]=0;
  
 rep(j,1,n+1){
  ll a=j;
  if(dig(j)>1)a=j/llpow(10,dig(j)-1);
  ll b=j%10;
  c[a][b]++;
  //先頭、末尾の順
 // cout<<a<<" "<<b<<endl;
 ...
 rep(i,0,10){
  rep(j,0,10){
   ans+=c[i][j]*c[j][i];
   //cout<<i<<" "<<j<<" "<<c[i][j]<<" "<<c[j][i]<<endl;
  }
 }
 pr(ans);
return 0;}
/*
x=dig(a),y=dig(b)とすると
a%10=b/(y-1)
b%10=a/(x-1)
a:p???q
b:q????p
...
i)a<10のとき
 ...
 r<pのときは10^(z-3)*4通り
 r=pのときは
  n%10<=pのとき10^(z-3)*4+(n%(z-1))/10通り
  n%10>pのとき10^(z-3)*4+(n%(z-1))/10-1通り
