...
int main(){
    ll n,k;cin >> n >>k;
    vl v(n);rep(i,n)cin >> v[i],v[i]-=k;
    ll ans=0;
    ll now=0;
    vl cum(n+1);cum[0]=0;
    rep(i,n)cum[i+1]=cum[i]+v[i];
    sort(all(cum));cum.erase(unique(all(cum)),cum.end());
    SegTree<ll> st(n+1,0,[](ll a,ll b){return a+b;},[](ll a,ll b){return a+b;});
    ll z=lower_bound(all(cum),0)-cum.begin();
    st.change(z,1);
    rep(i,n){
        now+=v[i];
        ll p=lower_bound(all(cum),now)-cum.begin();
        ans+=st.query(0,p+1);
        //cout << ans <<endl;
        st.change(p,1);
    }
