...
1-indexed
verified by
http://judge.u-aizu.ac.jp/onlinejudge/review.jsp?rid=4490492#1
https://atcoder.jp/contests/arc033/submissions/11204957
*/
template<typename T>
struct BIT{
    vector<T> node;
...
int main(){
    ...
    Compress<ll> comp(sum);
    rep(i,n+1){
        sum[i]=comp.id(sum[i])+1;
    }
    BIT<ll> seg(n+1);
    ll ans=0;
    seg.add(sum[0],1);
