...
// 各項の順序を保ったまま最小値0,最大値(要素種)-1以下にする
template <typename T>
vector<int> simplify(vector<T>& a) {
    int n = a.size();
    map<T, int> dic;
    for (int i = 0; i < n; i++) dic[a[i]] = 0;
    int k = 0;
    for (auto itr = dic.begin(); itr != dic.end(); itr++) (*itr).second = k++;
    vector<int> res(n);
    for (int i = 0; i < n; i++) res[i] = dic[a[i]];
    return res;
}
// 転倒数を計算
// depend:segment_tree
