...
int main() {
  ...
  io.read(a);
  vector<ll> acc(N + 1);
  REP(i, N) acc[i + 1] = acc[i] + a[i] - K;
  vector<ll> comp(acc);
  sort(ALL(comp));
  comp.erase(unique(ALL(comp)), comp.end());
  SegmentTree<ll, ll> seg(comp.size(), plus<ll>(), plus<ll>(), plus<ll>(), 0, 0,
                          vector<ll>(comp.size()));
  ll ans = 0;
  for (int i = N; i > 0; --i) {
    int pos = lower_bound(ALL(comp), acc[i]) - comp.begin();
    seg.update(pos, pos + 1, 1);
    int pos2 = lower_bound(ALL(comp), acc[i - 1]) - comp.begin();
    ans += seg.query(pos2, comp.size());
  }
  io.writeln(ans);
  return 0;
}