...
public:
	void solve(std::istream& in, std::ostream& out) {
        ...
        mp['L'] = mp['R'] = sc;
        for(int i=0; i<n; i++){
            if(s[i] == 'R' || s[i] == 'D') mp[s[i]]++;
            else mp[s[i]]--;

            if(mp['R'] >= w || mp['L'] < 0 || mp['U'] < 0 || mp['D'] >= h){
                flg = false;
                break;
            }

            if(t[i] == 'L') mp['R'] = max(mp['R']-1, 0);
            if(t[i] == 'R') mp['L'] = min(mp['L']+1, w-1);
            if(t[i] == 'U') mp['D'] = max(mp['D']-1, 0);;
            if(t[i] == 'D') mp['U'] = min(mp['U']+1, h-1);;
        }
