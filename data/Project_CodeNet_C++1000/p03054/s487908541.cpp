...
int main(){
  ...
  if(s[n-1] == 'R') right--;
  rrep(i,n-1) {
    if(t[i] == 'L') right = min(W, right+1);
    if(t[i] == 'R') left = max(0, left-1);
    if(s[i] == 'L') left++;
    if(s[i] == 'R') right--;
    if(right <= left) ok = false;
  }

  if(sc < left || sc >= right) ok = false;
  ...
  if(s[n-1] == 'D') right--;
  rrep(i,n-1) {
    if(t[i] == 'U') right = min(H, right+1);
    if(t[i] == 'D') left = max(0, left-1);
    if(s[i] == 'U') left++;
    if(s[i] == 'D') right--;
    if(right <= left) ok = false;
  }

  if(sr < left || sr >= right) ok = false;
