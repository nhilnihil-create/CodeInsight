...
int main(void){
	...
	int h=readi(),w=readi(),n=readi(),sy=readi(),sx=readi();
	int xmi=1,xma=w,ymi=1,yma=h;
	string s=readstr(),t=readstr();
	for(int i=n-1;i>=0;i--){
		if(t[i]=='R')xmi=max(1,xmi-1);
		if(t[i]=='L')xma=min(w,xma+1);
		if(t[i]=='D')ymi=max(1,ymi-1);
		if(t[i]=='U')yma=min(h,yma+1);

		if(s[i]=='R')xma--;
		if(s[i]=='L')xmi++;
		if(s[i]=='D')yma--;
		if(s[i]=='U')ymi++;
		if(xmi>xma||ymi>yma)NO();
	}
	orYES(xmi<=sx&&sx<=xma&&ymi<=sy&&sy<=yma);
}