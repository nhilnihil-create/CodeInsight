...
signed main() {
	...
	vector<char> vt({'L','R','U','D'}),va({'R','L','D','U'});
	vector<ll> startd({Y-1,W-Y,X-1,H-X});
	vector<ll> direction({-1,1,-1,1});
	rep(d,4){
		debug(d);
		ll t=d<2?Y:X;
		rep(i,N)rep(j,2){
			...
			if(j==0){
				if(St[i]==vt[d])t+=direction[d];
				if(t==-1||(d<2&&t==W)||(d>=2&&t==H)){
					p("NO");return 0;
				}
				debug(St[i],j,t);
			}else{
				if(Sa[i]==va[d])t-=direction[d];
				if(t<0)t=0;
				if(d<2&&t>=W)t=W-1;
				if(d>=2&&t>=H)t=H-1;
				debug(Sa[i],j,t);
