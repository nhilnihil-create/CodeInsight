...
//逆から見る。横縦を独立に考える。
int lowh = H, upph = 1; //最終回で青木が勝ちの範囲
int loww = 1, uppw = W; 
Rrep(i, N-1, 0){
    if (T[i] == 'U') lowh = min(H, lowh+1);
    else if (T[i] == 'D') upph = max(1, upph-1);
    else if (T[i] == 'R') loww = max(1, loww-1);
    else if (T[i] == 'L') uppw = min(W, uppw+1);
    if (S[i] == 'U') ++upph;
    else if (S[i] == 'D') --lowh;
    else if (S[i] == 'R') --uppw;
    else if (S[i] == 'L') ++loww;
    if (loww > uppw || lowh < upph){
        cout << "NO\n";
        return 0;
    }
}

if (upph <= sr && sr <= lowh && loww <= sc && sc <= uppw) cout << "YES\n";
else cout << "NO\n";
