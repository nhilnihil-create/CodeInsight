...
int main(void)
{
  ...
  rep(i, n) cin >> A[i];
  using pa = pair<ll, ll>;
  pa AMax = make_pair(A[0], 0LL);
  pa AMin = make_pair(A[0], 0LL);
  ...
  for (ll i = 1; i < n; i++)
  {
    ...
    if (x > AMax.first)
    {
      AMax = make_pair(A[i], i);
    }
    if (x < AMin.first)
    {
      AMin = make_pair(A[i], i);
    }
  }

  queue<pa> que;

  if (abs(AMax.first >= abs(AMin.first)))
  {
    for (ll i = 0; i < n; i++)
    {
      if (i != AMax.second)
      {
        que.push(make_pair(AMax.second + 1, i + 1));
      }
    }
    for (ll i = 1; i < n; i++)
    {
  ...
  else
  {
    for (ll i = 0; i < n; i++)
    {
      if (i != AMin.second)
      {
        que.push(make_pair(AMin.second + 1, i + 1));
      }
    }
    for (ll i = n; i > 1; i--)
    {
