...
signed main() {
  ...
  rep(i,0,n){
    minn = min(minn,a[i]);
    if(minn == a[i]) minmemo = i;
    maxx = max(maxx,a[i]);
    if(maxx == a[i]) maxmemo = i;
  }
  cerr<< minn _ maxx<<endl;
  
  if(minn>0 && maxx>0){
    cout<<n<<endl;
    cout<<maxmemo+1 _ 1 << endl;
    rep(i,0,n-1){
      cout << i+1 _ i+2 << endl;
    }
  }
  else if(minn<0 && maxx<0){
    cout<<n<<endl;
    cout<<minmemo+1 _ n<<endl;
    repreq(i,n,2){
      cout<< i _ i-1 << endl;
    }
  }
  else if(minn == maxx){cout<<0<<endl;}
  else if(abs(minn)>=abs(maxx)){
    cout << 2*n << endl;
    cout << minmemo+1 _ n << endl;
    cout << minmemo+1 _ n << endl;
