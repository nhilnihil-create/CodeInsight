...
int main()
{
	...
	auto begin_iter = as.begin();
	auto end_iter = as.end();

	auto arg_max_iter = max_element(begin_iter, end_iter);
	auto arg_min_iter = min_element(begin_iter, end_iter);
	if (*arg_min_iter < 0)
	{
		if (*arg_max_iter >= -(*arg_min_iter))
		{
			for (ll i = 1; i <= N; ++i)
			{
				if (i != arg_max_iter - as.begin() + 1)
				{
					res.emplace_back(arg_max_iter - as.begin() + 1, i);
				}
			}
		}
		else
		{
			for (ll i = 1; i <= N; ++i)
			{
				if (i != arg_min_iter - as.begin() + 1)
				{
					res.emplace_back(arg_min_iter - as.begin() + 1, i);
				}
			}
		}
	}
