...
int main() {
    int N;cin >> N;
    vector<int> retu(N);
    vector<vector<long long>> nowsum(N+1,vector<long long>(3));
    vector<long long> times(N,0);
    for(int i = 0;i<N;i++) cin >> retu.at(i);

    bool ok = true;
    for(int i = 0;i<N;i++){
        long long hat = retu.at(i);
        for(int j = 0;j<3;j++){
            if(nowsum.at(i).at(j) == hat){
                if(ok){
                    nowsum.at(i+1).at(j) += nowsum.at(i).at(j) + 1;
                    ok = false;
                }
                else{
                    nowsum.at(i+1).at(j) = nowsum.at(i).at(j);
                }
                times.at(i)++;
            }
            else{
