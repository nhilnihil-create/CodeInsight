...
int main(){
  ...
  // 最初の人は必ず入力が0であり、RGBどれもありえる
  ll ans = 3;
  // 必ずしも {R, G, B} に対応するとは限らない
  // 最初の人をとりあえず[0]に入れる
  vector<ll> RGBs = vector<ll>{1, 0, 0};
  ...
  REP(i, 1, N){
    ll a = as[i];
    // 問題文から、恐らく矛盾する（答えが0になる）発言はない
    // すると、RGBs[0~2]の間に必ずaがあるはず
    ll acnt = 0;
    rep(j, 3) if (RGBs[j]==a) acnt++;
    if (acnt==0){
      cout << 0 << endl;
      return 0;
    }
