...
{  
  ...
  int ANS = INF;
  string str_zero(str.size(),'0');
  string str_one(str.size(),'1');

  if(str == str_zero) ANS = (int)str.size();
  if(str == str_one ) ANS = (int)str.size();

  for (int i = 0; i < str.size()-1; i++)
  {
    if(str[i] != str[i+1]) chmin(ANS,max(i+1,(int)str.size()-i-1));
  }
  cout<<ANS<<endl;
