...
int main() {
	...
	if (cont.size() == 1) {
		...
		//result = max(result, cont[0]);
		//result = max(result, cont[cont.size() - 1]);
		//reps(i, 1, cont.size() - 2) {
		//	ll temp = min(cont[i - 1] + cont[i], cont[i] + cont[i + 1]);
		//	result = max(result, temp);
		//}

		vll sumLeft(cont.size(), 0);
		sumLeft[0] = cont[0];
		reps(i, 1, cont.size() - 1) {
			sumLeft[i] = sumLeft[i - 1] + cont[i];
		}
		vll sumRight(cont.size(), 0);
		sumRight[cont.size() - 1] = cont[cont.size() - 1];
		deps(i, cont.size() - 2, 0) {
			sumRight[i] = sumRight[i + 1] + cont[i];
		}
		rep(i, cont.size()) {
			result = max(result, min(sumLeft[i], sumRight[i]));
		}
	}
...
//AtCoder Beginner Contest 038 - D
/*
struct LLPair {
	ll w = 0;
	ll h = 0;
...
int main() {
	...
	std::sort(P.begin(), P.end(), [](const LLPair &a, const LLPair &b) {
		if (a.w == b.w) {
			return a.h > b.h; //�~���ɂ���ɂ� > �Ƃ���
		}
		else {
			return a.w < b.w; //�~���ɂ���ɂ� > �Ƃ���
		}
	});
	...
	vll dp(N, INF); //dp[h / w][�Œ���]
	int iP = 0;

	rep(i, numOfWidth) {
		ll loopCount = sameWidthCount[P[iP].w];
		vll writeIndex;
		vll writeValue;

		rep(j, loopCount) {
			ll index = lower_bound(dp.begin(), dp.end(), P[iP].h) - dp.begin();
			writeIndex.push_back(index);
			writeValue.push_back(P[iP].h);
			iP++;
		}
		rep(j, loopCount) {
			dp[writeIndex[j]] = writeValue[j];
		}

		rep(j, N) {
			dp[j] == INF ? eprintf("- ") : eprintf("%lld ", dp[j]);
		}
		eprintf("\n");
	}

	ll result = lower_bound(dp.begin(), dp.end(), INF) - dp.begin();
	printf("%lld\n", result);
}
*/
