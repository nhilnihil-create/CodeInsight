...
int main()
{
    ...
    while(t--)
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            cin>>a>>b>>c;
            S[i]={b,{a,c}};
            //weights[a]=max(weights[a],c);
        }
        sort(S.begin(),S.end(),compar);
        for(i=0;i<n;i++)
        {
            for(j=S[i].first;j>=0;j--)
            {
             weights[j+S[i].second.first]=max(weights[j+S[i].second.first],S[i].second.second+weights[j]);
            }
            // for(j=0;j<=20;j++)
            // cout<<weights[j]<<" ";
            // cout<<"\n";
