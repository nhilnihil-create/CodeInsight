...
int rec(int ind,int weight)
{
    ...
    int& res=dp[ind][weight];
    if(res!=-1)
        return res;

    res=rec(ind+1,weight);
    if(weight<=arr[ind].s)
        remax(res, arr[ind].v + rec(ind+1,weight+arr[ind].w));
