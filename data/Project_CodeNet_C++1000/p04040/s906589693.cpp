...
ll factorial[2*MAX_H];//階乗
ll inverse[2*MAX_H];//逆元
...
ll calc_nCr(ll n,ll r){
    return ((((factorial[n]%MOD) * (inverse[r]%MOD))%MOD) * (inverse[n-r]%MOD))%MOD;
}
...
int main(){
    ...
    for(ll i = b;i < w;i++){
        ...
        ll down = a - 1;
        ll right = w - i - 1;
        ans = (ans + ((calc_nCr(n+i,i)%MOD) * (calc_nCr(down+right,right)%MOD))%MOD)%MOD;
    }
    cout << ans << endl;
