...
ll modcomb(ll n, ll r, vll &fac, vll &inv) {
...
void solve() {
    ll H, W, A, B;
    cin >> H >> W >> A >> B;
    vll fac = modfactorial(200010);
    vll inv = invmodfactorial(200010);
    ll ans = 0;
    debug(modcomb(1000, 1, fac, inv));
    rep(b, B, W-1) {
        ans += modcomb(W-b-1+A-1, A-1, fac, inv) * modcomb(b+H-A-1, b, fac, inv);
        ans = ans % MOD;
    }
    out(ans %MOD);
}
