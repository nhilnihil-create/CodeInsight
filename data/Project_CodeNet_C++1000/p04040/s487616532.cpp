...
lint fa[200020],rfa[200020];//kaijou,gyakugen n==10^7kurai made
void setfa(lint n){
	fa[0]=1;
	rep(i,1,n)fa[i]=(fa[i-1]*i)%md;
	rfa[n-1]=powmod(fa[n-1],md-2);
	rrep(i,n-1)rfa[i]=(rfa[i+1]*(i+1))%md;
}
...
int main(){
	...
	setfa(200010);
	lint ans=0;
	lint u=h-a-1,d=a-1,n=w-b;
	rep(i,b,w){
		ans+=ncr(u+i,u)*ncr(w-i+d-1,d)%md;ans%=md;
	}cout<<ans<<endl;
}
