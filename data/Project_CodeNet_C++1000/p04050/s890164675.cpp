...
int main()
{
    read(n);read(m);for(int i=1;i<=m;i++) read(a[i]),ji+=a[i]&1;
    if(ji>2) return puts("Impossible"),0;
    for(int i=2;i<m;i++) if(a[i]&1) if(a[1]&1) swap(a[i],a[m]);else swap(a[i],a[1]);
    if(m==1){print(a[1]);el();print(n==1?1:2);el();prints("1 ",0);if(n>1) print(n-1);flush();return 0;}
    for(int i=1;i<=m;i++) print(a[i]),sp();el();a[1]++;a[m]--;if(!a[m]) m--;
    print(m);el();for(int i=1;i<=m;i++) print(a[i]),sp();flush();
}