...
signed main() {
	...
	rep(i, H) {
		rep(j, W) {
			if (G.at(i).at(j) == '#') dist.at(i).at(j) = INF;
		}
	}

	BFS_labyrinth(que, dist);
	
	if (dist.at(g.first).at(g.second) == -1) cout << "-1" << endl;
	else {
		int white = 0;
		rep(i, H) {
			rep(j, W) {
				if (G.at(i).at(j) == '.') white++;
			}
		}

		cout << white - dist.at(g.first).at(g.second) << endl;
	}
}