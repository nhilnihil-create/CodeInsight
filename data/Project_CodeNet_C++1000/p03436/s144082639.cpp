...
int main()
{
    ...
    int floor_num = 0;
    std::vector<std::vector<int>> walk_num(h, std::vector<int>(w, 0));
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            if (s[i][j] == '#') {
                walk_num[i][j] = -1;
    ...
    while (q.size()) {
        ...
        auto check = [=](int y, int x) {
            return y >= 0 && y < h && x >= 0 && x < w && !walk_num[y][x];
        };

        auto push = [&](int y_, int x_) {
            if (check(y_, x_)) {
                walk_num[y_][x_] = walk_num[y][x] + 1;
                q.push(std::make_pair(y_, x_));
            }
        };
    ...
    std::cout << (walk_num[h - 1][w - 1] == 0 ? -1 : floor_num - walk_num[h - 1][w - 1] - 1) << std::endl;
}
