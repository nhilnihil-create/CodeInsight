...
void solve(){
    ...
    q.push({1, {0, 0}});
    while(!q.empty()){
        ll d = q.front().ft, x = q.front().sd.ft, y = q.front().sd.sd; q.pop();
        if(vis[x][y]){
        ...
        rep(j, 4){
            ll nx = x + lx[j], ny = y + ly[j];
            if(nx >= 0 && ny >= 0 && nx < n && ny < m && a[nx][ny] == '.') q.push({d+1, {nx, ny}});
        }
    }

    if(vis[n-1][m-1] == 0) ANS(-1);
    ll ans = n*m - vis[n-1][m-1];
