...
int main()
{
    ...
    while(!que.empty()){
        ...
        que.pop();
        for(int i=0; i<4; i++){
            if(v.F+dx[i]<0 || v.F+dx[i]<0){
                continue;
            }
            if(v.F+dx[i]>=H || v.S+dy[i]>=W) continue;
            if(dist[v.F+dx[i]][v.S+dy[i]]!=-1){
                continue;
            }
            if(s[v.F+dx[i]][v.S+dy[i]]=='#') continue;
            dist[v.F+dx[i]][v.S+dy[i]]=dist[v.F][v.S]+1;
            que.push(make_pair(v.F+dx[i],v.S+dy[i]));
        }
    }
    if(dist[H-1][W-1]==-1){
        cout << -1 << endl;
