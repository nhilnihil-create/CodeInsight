...
void dfs(P s){
  //coutn(s);
  if(s.fs+1<h && g[s.fs+1][s.sd]=='.' && dist[s.fs+1][s.sd]>dist[s.fs][s.sd]+1){
    dist[s.fs+1][s.sd]= dist[s.fs][s.sd]+1;
    dfs(P(s.fs+1, s.sd));
  }
  if(s.fs-1>=0 && g[s.fs-1][s.sd]=='.' && dist[s.fs-1][s.sd]>dist[s.fs][s.sd]+1){
    dist[s.fs-1][s.sd]= dist[s.fs][s.sd]+1;
    dfs(P(s.fs-1, s.sd));
  }
  if(s.sd+1<w && g[s.fs][s.sd+1]=='.' && dist[s.fs][s.sd+1]>dist[s.fs][s.sd]+1){
    dist[s.fs][s.sd+1]= dist[s.fs][s.sd]+1;
    dfs(P(s.fs, s.sd+1));
  }
  if(s.sd-1>=0 && g[s.fs][s.sd-1]=='.' && dist[s.fs][s.sd-1]>dist[s.fs][s.sd]+1){
    dist[s.fs][s.sd-1]= dist[s.fs][s.sd]+1;
    dfs(P(s.fs, s.sd-1));
  }
}
...
void solve(){
  cin >> h >> w;
  g = V<string>(h); cin >> g;
  dist = V2<int>(h,V<int>(w,IINF));
  dist[0][0] = 0;
  dfs(P(0,0));
  int ans = -dist[h-1][w-1]-1;
  FOR(i,0,h) FOR(j,0,w) ans += (g[i][j]=='.');
  //coutn(dist[h-1][w-1]);
  //FOR(i,0,h){FOR(j,0,w)couts(dist[i][j]); coutn("");}
  //coutn(ans);
  if(dist[h-1][w-1]==IINF || ans<0) cout << -1;
  else cout << ans;
}
