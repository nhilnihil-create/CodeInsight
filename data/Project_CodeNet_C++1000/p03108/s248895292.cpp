...
int main() {
  ...
  for (int i = M - 1; i >= 0; i--) {
    K.at(i) = K.at(i + 1);
    if (T.same(B.at(i).at(1) - 1, B.at(i).at(0) - 1) == false) {
      K.at(i) += H.at(T.root(B.at(i).at(0) - 1)) * H.at(T.root(B.at(i).at(1) - 1));
      H.at(T.root(B.at(i).at(1) - 1)) += H.at(T.root(B.at(i).at(0) - 1));
      H.at(T.root(B.at(i).at(0) - 1)) = 0;
    }
    T.unite(B.at(i).at(0) - 1, B.at(i).at(1) - 1);
  }
  for (int i = 1; i <= M; i++) {
    cout << K.at(0) - K.at(i) << endl;
  }
}