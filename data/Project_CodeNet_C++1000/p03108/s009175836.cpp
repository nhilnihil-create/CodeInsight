...
int main() {
    ...
    ans[m] = n*(n-1)/2;
    for(ll i = m-1; i > 0; i--) {
        // 見る橋はbr[i], ans[i]に答え代入
        if(tree.issame(br[i].first, br[i].second)) ans[i] = ans[i+1];
        else {
            ans[i] = ans[i+1]-tree.size(br[i].first)*tree.size(br[i].second);
            tree.merge(br[i].first, br[i].second);
        }
    }
