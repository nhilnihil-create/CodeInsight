...
signed main() {
	...
	int count_mid = 0;
	rep(i, n - 1) {
		ans[0] += min(per(a[i + 1] - a[i], m), a[i + 1]+1);
		if (a[i] > a[i + 1]) { count_mid++; }
	}
	vvel lis(m);
	rep(i, n) { lis[a[i]].push_back(i); }
	for (int i = 0; i < m-1; i++) {
		ans[i+1] = ans[i];
		ans[i+1] -= count_mid;
		for (auto x : lis[i]) {
			if (x != 0) { ans[i + 1] += per(a[x] - a[x - 1],m); count_mid--; }
			if (x != n - 1) { count_mid++; }
		}
	}
	int ret = ans[0];
	rep(i, m) { mmin(ret, ans[i]); }
	cout << ret << endl;
	return 0;
}
/*18:07:40*/