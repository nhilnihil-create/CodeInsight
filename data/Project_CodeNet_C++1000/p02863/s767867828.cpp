...
int main(void)
{
	ll N, T; cin >> N >> T;
	vt<pll> nyaa(N);
	each(i, e, nyaa) cin >> e.first >> e.second;
	nyaa.insert(nyaa.begin(), { 0,0 });
	nyaa.insert(nyaa.end(), { 0,0 });

	vvt<ll> dp1; MakeVVT(N+2, T, dp1);
	rep(i, 1, Size(nyaa)) rep(t, 0, T)
	{
		dp1[i][t] = dp1[i - 1][t];
		if (t - nyaa[i].first < 0) continue;
		dp1[i][t] = max({ dp1[i][t], dp1[i - 1][t - nyaa[i].first] + nyaa[i].second });
	}

	vvt<ll> dp2; MakeVVT(N+2, T, dp2);
	repr(i, Size(nyaa)-2, -1) rep(t, 0, T)
	{
		dp2[i][t] = dp2[i + 1][t];
		if (t - nyaa[i].first < 0) continue;
		dp2[i][t] = max({ dp2[i][t], dp2[i + 1][t - nyaa[i].first] + nyaa[i].second });
	}

	ll ans = 0;
	rep(i, 1, Size(nyaa)-1) rep(t, 0, T)
	{
		ans = max(ans, dp1[i - 1][t] + dp2[i + 1][T - 1 - t] + nyaa[i].second);
	}
	cout << ans;
	return 0;
}
