...
void solve()
{
  ...
  forf(i,1,n+1) cin >> a[i].fi >> a[i].se;
  ...
  for(int i=1;i<=n;i++)
  {
     for(int j=0;j<=6000;j++) dp[i][j] =dp[i-1][j];

     for(int j=0;j<t;j++)  dp[i][j+a[i].fi] = max(dp[i][j+a[i].fi] , dp[i-1][j]+ a[i].se); 
  }
  
  for(int i=1;i<=6000;i++) an = max(an , dp[n][i]);
  cout<<an;
