...
vvi knapsack01_W(int N, int W, vi v, vi w) {
  vvi wv(N + 1, vi(W + 1));
  rep(i, N) {
    rep(j, W + 1)wv[i + 1][j] = wv[i][j];
    rep(j, W - w[i] + 1)amax(wv[i + 1][j + w[i]], wv[i][j] + v[i]);
  }
  return wv;
}

void solve(int n, int t, vi a, vi b) {
  vvi rk = knapsack01_W(n, t - 1, b, a);
  rev(a);rev(b);
  vvi lk = knapsack01_W(n, t - 1, b, a);
  int ans = 0;
  rep(i, n)rep(j, t)amax(ans, lk[i][j] + b[i] + rk[n - i - 1][t - j - 1]);
  COUT(ans);
}
