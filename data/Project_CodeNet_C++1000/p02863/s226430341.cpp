...
void solve() {
    ll N, T; cin >> N >> T;
    vector<LP> dish; // <time,tasty>
    rep(i,N){
        ll a,b;cin >> a >> b;
        dish.push_back(LP(a,b));
    }
    sort(all(dish));
    ...
    rep1(i,N){
        rep(j,T){
            dp[i][j] = dp[i-1][j];
            ans = max(ans,dp[i][j] + dish[i-1].second);

            if(j-dish[i-1].first >= 0){
                dp[i][j] = max(dp[i][j], dp[i-1][j-dish[i-1].first] + dish[i-1].second);
            }
        }
    }
