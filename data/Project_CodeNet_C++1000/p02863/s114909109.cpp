...
signed main(){
	...
	rep(0,i,n)rep(0,j,k)dp[i][j]=0;
	rep(0,i,n){
		rep(0,j,k){
			if(j>=a[i]){
				if(dp[i][j-a[i]]+b[i]>dp[i][j]){
	...
	rep(0,i,n)rep(0,j,k+1)dp2[i][j]=0;
	for(int i=n-1;i>=1;i-=1){
		rep(0,j,k){
			if(j>=a[i]){
				if(dp2[i][j-a[i]]+b[i]>dp2[i][j]){
					dp2[i-1][j]=dp2[i][j-a[i]]+b[i];
				}
				else{
					dp2[i-1][j]=dp2[i][j];
				}		
	...
	rep(0,i,n){
		rep(0,j,k){
			ans=max(ans,dp[i][j]+dp2[i][k-1-j]+b[i]);
		}	
	}	
	c(ans)
}