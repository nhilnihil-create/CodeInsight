...
signed main() {
	int n, t; cin >> n >> t;
	vel a(n), b(n); vvel qa(n);
	rep(i, n) { cin >> a[i] >> b[i]; qa[i] = { a[i],b[i]}; }
	sor(qa);
	rep(i, n) { a[i] = qa[i][0]; b[i] = qa[i][1]; }
	vvel dp(n + 1, vel(t,0));
	rep(i, n) {
		rep(j, t - a[i]) {
			mmax(dp[i + 1][j + a[i]], dp[i][j] + b[i]);
		}
		rep(j, t) { mmax(dp[i + 1][j], dp[i][j]); }
	}
	int ans = 0;
	rep(i, n+1) {
		rep(j, t) {
			mmax(ans, dp[i][j]);
			if (i != n) { mmax(ans, dp[i][j] + b[i]); }
		}
	}
	cout << ans << endl;
	return 0;
