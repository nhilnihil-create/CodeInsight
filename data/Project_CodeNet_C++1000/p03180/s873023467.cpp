...
void solve(){
	int n; cin>>n;
	vvl v(n,vl(n)); fore(i,0,n)fore(j,0,n)cin>>v[i][j];
	vl mask(1<<n,0);
	fore(i,0,1<<n)fore(j,0,n)if(i & (1<<j))fore(k,j+1,n)if(i & (1<<k))mask[i]+=v[j][k];
	vl dp(1<<n,-INF);
	dp[0]=0;
	fore(i,1,1<<n)for(int j=i;j; j=((j-1)&i))dp[i]=max(dp[i],dp[i^j]+mask[j]);
	cout<<dp[(1<<n)-1]<<ENDL;
