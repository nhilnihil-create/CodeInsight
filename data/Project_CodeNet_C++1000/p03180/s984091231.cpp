...
ll a[17][17],n,pre[(1<<16)],dp[(1<<16)];

void sol(ll idx,vll &nt,ll mask,ll sc,ll group){
    if(idx==(ll)nt.size()){
        dp[mask] = max(dp[mask],sc + pre[group]);
        return ;
    }
    sol(idx+1,nt,mask,sc,group);
    sol(idx+1,nt,mask|(1<<nt[idx]),sc,
        group|(1<<nt[idx]));
}
...
int32_t main(){
    ...
    //dp[mask] - best score if bits in mask are 
    //already chosen
    fori(1<<16) dp[i] = -1e18-5;
    dp[0] = 0;
    for(mask = 0 ;  mask < (1<<n) ; mask++){
        vll nt;
        fori(n){
