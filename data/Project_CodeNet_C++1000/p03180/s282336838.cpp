...
void solve() {
  int N;
  cin >> N;
  vector<vi32> a(N, vi32(N));
  cin >> a;
  vi64 dp(bit32(N), -1e18);
  int c = 0;
  function<i64(int S)> rec = [&](int S) {
    if (dp[S] != -1e18) return dp[S];
    i64 ret = 0;
    rep(i, N) rep(j, i) {
      if ((S & bit32(i)) && (S & bit32(j))) ret += a[i][j];
    }
    for (int T = 1; T < S; T++) {
      if ((~S) & T) continue;
      amax(ret, rec(T) + rec(S ^ T));
    }
