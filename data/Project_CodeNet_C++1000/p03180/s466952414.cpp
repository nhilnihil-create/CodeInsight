...
ll search(ll state){
  if(dp[state]<INFLL) return dp[state];
  ll ans = 0;
  for(ll i=0; (1<<i)<(1<<N); ++i) for(ll j=0; (1<<j)<(1<<N); ++j){
    if(state&(1<<i) && state&(1<<j)) ans += A[i][j];
  }
  dp[state] = ans;
  for(ll tmp = (state-1)&state; tmp>0; tmp=(tmp-1)&state){
    chmax(dp[state], search(tmp)+search(state-tmp));
  }
  return dp[state];
}
