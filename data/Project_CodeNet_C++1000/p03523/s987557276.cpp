...
int main() {
    ...
    for(int i = 0;i <= (1<<4);i++){
        ...
        if(B.test(1)){
            if(T.at(3) == 'H'){
                string R = T.substr(0,4);
                string U = T.substr(4,2);
                T = R + 'A' + U;
               
            }
            if(T.at(2) == 'H'){
                string R = T.substr(0,3);
                string U = T.substr(3,3);
                T = R + 'A' + U;
            }
        }
        if(B.test(2)){
            if(T.at(3) == 'B'){
                string R = T.substr(0,4);
                string U = T.substr(4,1);
                T = R + 'A' + U;
            }
            if(T.at(4) == 'B'){
                string R = T.substr(0,5);
                string U = T.substr(5,1);
                T = R + 'A' + U;
            }
            if(T.at(5) == 'B'){
                string R = T.substr(0,6);
                string U = T.substr(6,1);
                T = R + 'A' + U;
            }
        }
        if(B.test(3)){
