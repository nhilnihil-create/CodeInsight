/*!enjoy karo yaar!*/

//CF,CC,AtC,SPOJ: hp1999
//HE: hemant269
//HR: hemant2132

#include<bits/stdc++.h>
using namespace std;

#define     int             long long int
#define     fast()          ios::sync_with_stdio(0);cin.tie(0);cout.tie(0);
#define     all(x)          begin(x),end(x)
#define     rz(x)           resize(x)
#define     asn(x,y)        assign(x,y)
#define     mem(a,b)        memset(a,b,sizeof(a))
#define     sz(x)           ((int)(x.size()))
#define     eb              emplace_back
#define     pb              push_back
#define     pf              push_front
#define     pob             pop_back
#define     pof             pop_front
#define     ins             insert
#define     vi              vector<int>
#define     pii             pair<int,int>
#define     mii             map<int,int>
#define     F               first
#define     S               second
#define     makep           make_pair
#define     maket           make_tuple
#define     remax(a,b)      a=max(a,b)
#define     remin(a,b)      a=min(a,b)
#define     bitcount(x)     __builtin_popcountll(x)
#define     iceil(n,x)      (((n)+(x)-1)/(x))
#define     gcd(a,b)        (__gcd((a),(b)))
#define     lcm(a,b)        ((a)*((b)/gcd((a),(b))))
#define     dbug(x)         cout<<#x<<": "<<(x)<<"\n"
#define     flush           fflush(stdout)
#define     show(x)         for(auto zz:x)cout<<zz<<" ";cout<<"\n";
#define     show2(x)        for(auto zz:x)cout<<zz.F<<" "<<zz.S<<"\n";

typedef unsigned long long ull;
typedef long long ll;
typedef long double ld;

const ld pi=acos(-1);
const ll inf=1e18,M=1e9+7;
const int N=1e5+5;

vi v[N],have(N);

int n,k;
int ans=1;

void upd(int x)
{
    //cout<<x<<"...\n";

    if(x<=0)
    {
        cout<<0;
        exit(0);
    }

    ans=(ans*x)%M;
}

void dfs(int x,int p,int d)
{
    //cout<<x<<" "<<p<<" "<<d<<"\n";

    have[x]=1;

    if(p && sz(v[x])==1)
    {
        upd(k-d);
        return;
    }

    if(d)           // par assigned, can determine for x
        upd(k-d);

    int cur=0;      // for the children of x
    if(d) cur+=2;           // as both par and x have been assigned
    for(auto c:v[x])
    {
        if(c==p)
            continue;

        dfs(c,x,cur);

        if(have[x]==1 && !d)        // first child helps in determining x if not done so yet
        {
            upd(k-have[c]);
            ++cur;
        }

        ++cur;
        ++have[x];
    }
}

void solve()
{
    cin>>n>>k;

    if(n==1)
    {
        cout<<k;
        return;
    }

    int a,b;
    for(int i=1;i<n;++i)
    {
        cin>>a>>b;
        v[a].pb(b);
        v[b].pb(a);
    }

    dfs(1,0,0);
    cout<<ans;
}

int32_t main()
{
    fast();

    int t=1;
    //cin>>t;
    for(int z=1;z<=t;++z)
    {
        solve();
        //cout<<"\n";
    }

    return 0;
}

