...
void dfs(ll now, ll parent=-1){
  ...
  if(parent == -1 && K-1 >= c){
    t = FacMod[K-1]*FacInvMod[K-1-c]%MOD;
  }else if(parent != -1 && K-2 >= c){
    t = FacMod[K-2]*FacInvMod[K-2-c]%MOD;
  }else{
    t = 0;
  }
  //cout << now << " " << parent << " " << t << endl;
  
  ans = (ans*t)%MOD;
  
  for(auto child : graph[now]){
    if(child != parent && !searched[child]){
      dfs(child,now);
    }
  }
