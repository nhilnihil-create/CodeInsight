...
struct Fiduccia{
    ...
    */
    ...
    void consist(char S,int a,int b){
        /*
        ax^bの計算
        0乗も忘れずに
        */
    ...
    void calculate_init(long long N){
        /*
        n乗の係数を求める。
        */
        while(N>0){
            /*
            P(x)とQ(-x)の積をまず計算(R(x))で表す
            偶数乗項の係数のみを順番に集めていく
            */
            for(int i=0;i<PP;i++){
                for(int j=0;j<QQ;j++){
            ...
            if(N%2==0){
                ...
                for(int i=0;i<QQ;i++){
                    for(int j=0;j<QQ;j++){
                        if((i+j)%2==0 && j%2==0){
                            //cout<<i<<" "<<j<<endl;
                            Q.at((i+j)/2)+=Qcopy.at(i)*Qcopy.at(j);
                        }
                        if((i+j)%2==0 && j%2==1){
                            //cout<<i<<" "<<j<<endl;
                            Q.at((i+j)/2)+=-1*Qcopy.at(i)*Qcopy.at(j);
                        }
                    }
                }
            }
    ...
    T calculate(){//N=0の時を考える//Qの定数項の方が大きい場合はできないので注意
        int a=P.at(0);
        int b=Q.at(0);
        return a/b;
    }
...
int main(){
    ...
    while(!que.empty()){
        ...
        que.pop();
        int count=0;
        for(int i=0;i<(int)vec.at(s).size();i++){
            int g=vec.at(s).at(i);
            if(dist[g]!=-1){
                continue;
            }
