...
void dfs(vector<node> &nodes, const VVI &connect, int s, int n, int k, ll &ans)
{
    ans *= (k - min(nodes[s].depth, 2) - nodes[s].elder);
    ans %= mod;
    ...
    for (int t : connect[s])
    {
        ...
        {
            nodes[t].parent = s;
            nodes[s].children.emplace_back(s);
            nodes[t].depth = nodes[s].depth + 1;
            nodes[t].elder = elder;
            elder++;
            dfs(nodes, connect, t, n, k, ans);
        }
    }
}
...
int main()
{
    ...
    cin >> n >> k;
    vector<node> nodes(n);
    VL frac(intmax), fracinv(intmax);
    make_frac_tables(frac, fracinv);
