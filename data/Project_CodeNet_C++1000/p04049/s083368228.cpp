...
bool solve() {
	...
	REP (i,n) {
		...
		REP (j,sub.size()) {
			dep[edge[i][j]] = 1;
			queue<int> nxt; nxt.push(edge[i][j]);
			while (!nxt.empty()) {
				int p = nxt.front(); nxt.pop();
				sub[j].pb(dep[p]);
				for (auto to : edge[p]) {
					if (dep[to] == -1) {
						dep[to] = dep[p] + 1;
			...
			if (k % 2) {
				//奇数
				int rem = upper_bound(all(sub[j]),k/2) - sub[j].begin();
				tmp += sub[j].size() - rem;
				int can = upper_bound(all(sub[j]),k/2+1) - sub[j].begin();
				chmin(odd,rem-can);
			} else {
				int rem = upper_bound(all(sub[j]),k/2) - sub[j].begin();
				tmp += sub[j].size() - rem;
