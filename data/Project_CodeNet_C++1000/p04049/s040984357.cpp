...
int n, k;
vector<int> g[N];
int dp[N], h[N], tin[N], tout[N], timer = 0;;
...
signed main() {
	...
	for (int c = 1; c <= n; ++c) {
		...
		if (k % 2 == 0) {
			...
			int sz = (int)g[c].size();
			vector<int> val1(sz), val2(sz);
			int sum = 0;
			for (int i = 0; i < g[c].size(); ++i) {
				int to = g[c][i];
				for (int v = 1; v <= n; ++v) {
					if (anc(to, v)) {
						if (h[v] == k / 2) val1[i] += dp[v] - 1;
						else if (h[v] == k / 2 + 1) val2[i] += dp[v] - 1;
					}
				}
				sum += val1[i];
			}
