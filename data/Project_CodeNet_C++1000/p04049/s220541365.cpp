...
void usingDFS()
{
	...
	if(diameter%2==0)
	{
		...
		cout<<showMinValue(counters)<<endl;
	}
	else
	{
...
int showMinValue(vi initialVector)
{
	int minimum=initialVector[0];
	for(int i=1; i<initialVector.size(); ++i)
	{
		if(minimum>initialVector[i])
		{
			minimum=initialVector[i];
		}
	}
	return minimum;
}
