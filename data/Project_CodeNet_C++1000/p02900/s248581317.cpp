...
namespace NyaGadget
{
	...
	@note
	nを素因数分解する。
	res[i]がnだったとき、素因数分解して得られた積のなかにn個の素数iが含まれていることを表す。
	例えば n=12 のとき res[2]=2、res[3]=1 となるので 12=2×2×3 と分かる。
	**/
	void PrimeFactorization(long long n, std::map<long long, long long>& res)
	{
...
int main(void)
{
	...
	map<ll, ll> res;
	PrimeFactorization(gcd(a, b), res);
