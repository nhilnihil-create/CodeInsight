...
// - library ------------- //
  ...
  vector<bool> Eratosthenes2(ll n){
    vector<ll> primes;
    vector<bool> isPrime(n + 1, false);
    ll p = 0; // n 以下の素数の数
    REP(i, 2, n+1) isPrime[i] = true;
  ...
  void recursive_comb(int *idxs, int s, int rest, function<void(int *)> f) {
  if (rest == 0) {
    f(idxs);
  } else {
    if (s < 0) return;
    recursive_comb(idxs, s - 1, rest, f);
    idxs[rest - 1] = s;
    recursive_comb(idxs, s - 1, rest - 1, f);
  }
}
...
void foreach_comb(int n, int k, function<void(int *)> f) {
  int idxs[k];
  recursive_comb(idxs, n - 1, k, f);
}

vector<ll> primes;
vector<bool> isPrime;
vector<ll> prime_55ko;
...
int main(){
  ...
  isPrime = Eratosthenes2(10000);
  rep(i, 55){
    prime_55ko.p_b(primes[i]);
  }
  ...
  // //下の関数内はmain()の外だから、変数のスコープに注意
  // foreach_comb(55, 5, [](int *idxs) { //0～4までの数字から3つ取る組合せ
  //   //組合せの数だけループし、ループごとの選ばれた要素番号がidxs[i]に入る
  ...
  //   four = primes[idxs[3]];
  //   five = primes[idxs[4]];
  //   if (limit<=100000 && !isPrime[one+two+three+four+five]){
  //     cout << "OK : " << one+two+three+four+five << " : " << one << " " << two << " " << three << " "  << four << " " << five << endl;
  //     limit++;
  //   }
  // });
