...
namespace NyaGadget
{	// 素数テーブルライブラリ
	void PrimeTable(ll n, vector<ll>& res)
	{
...
int main(void)
{
	ll N; cin >> N;
	vt<ll> res;
	PrimeTable(Pow10(5), res);
	vt<ll> resmod(Size(res));
	each(i, e, res) resmod[i] = e % 5;

	vt<ll> ans;
	each(i, e, resmod) if (e == 1) ans.push_back(res[i]);
