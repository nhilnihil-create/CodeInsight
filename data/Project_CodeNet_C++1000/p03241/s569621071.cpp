...
// divisor(n): nの約数を全て列挙したvector<ll>を返す。
// 返す値には1も含む。値はソートされていない。
// O(√N)
vector<ll> divisor(ll n){
  vector<ll> res;
  for (ll i = 1; i * i <= n; i++){
    // 1から√N までを全て見ればいい
    // √Nより大きい約数もあるが、それは √Nより大きい約数 × √Nより小さい約数という組み合わせでNを実現するものであるはずなので、N / i を約数として入れることでちゃんとvectorに入る
    if(n % i == 0){
      // 単純に、割って0なら約数
      res.push_back(i);
      if(i * i < n){
        // 上記の通り N / i を約数として入れる
        // ただし、i = √N の場合は、N / i も√Nで被ってしまうので入れない
        res.push_back(n / i);
      }
    }
  }
