...
pair<int,int>a[250005];int cnt,cur,n,b[250005];stack<pair<int,int> >c;
void GG(){prints("No\n",0);flush();exit(0);}
int main()
{
	read(n);for(int i=1;i<=n;i++) read(a[i].first),a[i].second=i;sort(a+1,a+n+1);
	for(int i=1;i<=n;i++)
	{
		cnt=cur=1;b[a[i].first]=a[i].second;
		for(;cur<a[i].first&&cnt<a[i].second;cur++) if(!b[cur]) b[cur]=a[i].second,cnt++;
		if(cnt<a[i].second) GG();
		for(int j=1;j<=n-a[i].second;j++) c.push(make_pair(a[i].first,a[i].second));
	}
	for(int i=n*n;i;i--) if(!b[i])
	{
		if(c.top().first>=i) GG();
		b[i]=c.top().second;c.pop();
	}
	prints("Yes\n",0);for(int i=1;i<=n*n;i++) print(b[i]),sp();flush();
}