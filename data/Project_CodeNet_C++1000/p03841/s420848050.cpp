...
ll N;
vector<ll> x(600);
vector<ll> ans(250010, -1);
vector<ll> jun;
int main(){
  ll N;
  cin >> N;
  for(ll i = 1; i <= N; i++) cin >> x[i];
  for(ll i = 1; i <= N; i++) ans[x[i]] = i;
  for(ll i = 1; i <= N*N; i++){
    if(ans[i] != -1) jun.pb(ans[i]);
  }
  vector<ll> pre, sef;
  for(ll i = 1; i <= N*N; i++){
    if(ans[i] != -1) rep(j, ans[i]-1) pre.pb(ans[i]);
  }
  ...
  for(ll i = 1; i <= N; i++){
    ll cnt1 = i-1;
    ll cnt2 = N-i;
    for(ll j = 1; j < x[i]; j++)if(ans[j] == i) cnt1--;
    if(ans[x[i]] != i) l = 0;
    for(ll j = x[i]+1; j <= N*N; j++)if(ans[j] == i) cnt2--;
    if(cnt1 != 0 || cnt2 != 0) l = 0;
  }
  if(l == 1){
    cout << "Yes" << endl;
    for(ll i = 1; i <= N*N; i++){
