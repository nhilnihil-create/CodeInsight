...
int dp[10001][10][100][2][2]; // digitnum, digitval , modval , sameask , greatrthanzero
   ...
   // deb(i,j,k,l,m);
    ...
    if(l)
    {
        for(int digit=0;digit<10;digit++)
        {
            ...
            { 
            if(digit > 0)
            ans  = (ans +  sol(i+1,digit,(k+digit)%n,0,1))%mod;
            else
            ans = (ans + sol(i+1,digit,(k+digit)%n,0,m))%mod;
            }
            else if(now == digit)
            {
                if(now == 0)
                ans = (ans + sol(i+1,digit,(k+digit)%n,1,m))%mod;
                else
                ans  = (ans + sol(i+1,digit,(k+digit)%n,1,1))%mod;
            }
