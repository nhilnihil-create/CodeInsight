...
ll dp[100005][105][2];
ll solve(string & k , ll idx , ll sum , ll condn)
{
  // cout<<idx<<" "<<sum<<" "<<condn<<endl;
  if(idx==k.length() ) return (sum == 0) ;
  if(dp[idx][sum][condn] !=-1) return dp[idx][sum][condn];
  ll ans =0;
  if(condn)
  {
    ll val = k[idx]-'0';
    f(i,0,val) ans =(ans+solve(k,idx+1,(sum+i)%d ,0))%M;
    ans += solve(k,idx+1,(sum+val)%d , 1);
    ans %= M;
  }
  else
  {
    f(i,0,10) ans = (ans+solve(k,idx+1,(sum+i)%d , 0))%M;
  }
  return dp[idx][sum][condn] = ans;
