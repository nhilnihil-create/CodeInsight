    ...
    while (tests--)
    {
        ...
        rf(i, n - 1, 0)
        {
            if (i > 0)
            {
                f(j, 0, 10)
                {
                    f(k, 0, m) cadd(dp[i][(k + j) % m], dp[i + 1][k]);
                }
            }
            else
            {
        ...
        /* f(i,0,1+n){
            f(j,0,m)cout<<dp[i][j]<<sp;cout<<nl;
        } */
        int cur = (s[0] - '0') % m;
        f(i, 1, n)
        {
            f(j, 0, s[i] - '0')
            {
                cadd(ans, dp[i + 1][(m - (cur + j) % m) % m]);
            }
            if (i == n - 1)
                cadd(ans, dp[i + 1][(m - (cur + (s[i] - '0')) % m) % m]);
            cur = (cur + (s[i] - '0')) % m;
        }
        if (n == 1)
            cadd(ans, dp[n][(m - (cur + (s[n - 1] - '0')) % m) % m]);
        csub(ans, 1);
        cout << ans;
    }
