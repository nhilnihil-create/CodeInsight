...
ll k, d, dp[10000][2][100];

//<<-----Implement Functions Here---->>//
ll solve(string &S, ll index, bool smaller, ll rem) {
	if (index == S.size()) {
		if (rem == 0) return 1;
		else return 0;
	}
	if (dp[index][smaller][rem] != -1) return dp[index][smaller][rem];
	ll limit = 9, cnt = 0;
	...
	foo(i, 0, limit) {
		...
		cnt = (cnt % mod + (solve(S, index + 1, ns, (rem  + i) % d)) % mod) % mod;
	}
