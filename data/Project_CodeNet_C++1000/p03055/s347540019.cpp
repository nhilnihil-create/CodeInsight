...
using namespace std;
vector<vector<SINT64>> dat(200005);
vector<SINT64> root(200005,0);
vector<SINT64> root2(200005,0);
int main() {
	...
	rep(i,0,N-1) {
		...
		cin >> a >> b;
		a--; b--;
		dat[a].emplace_back(b);
		dat[b].emplace_back(a);
	}
	...
	while(qq.size() != 0) {
		...
		qq.pop();
		root2[buf.F] = 1;
		rep(i,0,dat[buf.F].size()) {
			if (root2[dat[buf.F][i]] == 0) {
				qq.push(Pll(dat[buf.F][i],buf.S+1));
				ans = MAX(ans,buf.S+1);
			}
		}
	}
