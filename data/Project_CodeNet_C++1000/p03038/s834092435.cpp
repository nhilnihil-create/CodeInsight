...
void solve()
{
    ...
    itr(it, mp) q.push({it->ft, it->sd});

    while(m--){
        ll k = 0 ;
        ll b, c; cin >> b >> c;
        if(q.top().ft >= c) continue;
        while(!q.empty()){
            ll t = q.top().ft, fre = q.top().sd; k += fre; //cout << t << " " << fre << endl;
            if(t >= c) { q.push({c, k-fre}); break; }
            else if(k >= b) {
                k -= fre;
                fre -= b-k; k = b;
                q.push({c, b}); q.pop(); q.push({t, fre});
                break;
            }
            else q.pop();
        }
    ...
    ll ans  =0;
    while(!q.empty()){
        ll t = q.top().ft, fre = q.top().sd;q.pop(); ans += t*fre;
    }
    cout << ans;
}
