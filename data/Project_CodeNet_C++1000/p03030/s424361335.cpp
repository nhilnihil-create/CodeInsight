...
void Main() {
    ll n;
    cin >> n;
    vector<pair<string, ll>> shop(n);
    vll shop_id(n);
    iota(shop_id.begin(), shop_id.end(), 0);

    REP(i, n) {
      string name;
      ll point;
      cin >> name >> point;
      shop[i] = paired(name, point);
    }

    sort(shop_id.begin(), shop_id.end(), [&](ll i1, ll i2) {
      if (shop[i1].first == shop[i2].first) {
        return shop[i1].second > shop[i2].second;
      }
      return shop[i1] < shop[i2];
    });

    REP(i, n) {
      COUT(shop_id[i] + 1);
      //cout << shop[shop_id[i]].first << " " << shop[shop_id[i]].second << endl;
    }
