...
signed main() {
    ...
    vector<pair<int, int>> poss_cnt;
    poss_cnt.push_back(make_pair(1, 1));

    FOR(i, 1, n) { poss_cnt.push_back(make_pair(0, 1)); }
    REP(mi, m) {
        int x, y;
        cin >> x >> y;
        x--;
        ...
        if (poss_cnt[x].first == 0) {
            poss_cnt[x].second--;

            poss_cnt[y].second++;

        } else if (poss_cnt[x].first == 1 && poss_cnt[x].second == 1) {
            poss_cnt[x].second--;
            poss_cnt[x].first = 0;
            poss_cnt[y].first = 1;
    ...
    int ans = 0;
    REP(ni, n) { ans += poss_cnt[ni].first; }
    print(ans);
}
