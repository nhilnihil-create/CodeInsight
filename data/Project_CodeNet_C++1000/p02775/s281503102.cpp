...
int main()
{
	...
	dp[0][0] = 0;
	rep(i, 0, Size(N) - 1) rep(d, 0, 2)
	{
		rep(pay, 0, 10)
		{
			if (pay - d < CtoL(N[i]))
			{
				ll change = 10 + pay - d - CtoL(N[i]);
				dp[i + 1][1] = min(dp[i + 1][1], dp[i][d] + pay + change);
			}
			else
			{
				ll change = pay - d - CtoL(N[i]);
				dp[i + 1][0] = min(dp[i + 1][0], dp[i][d] + pay + change);
			}
		}
