...
/*
dp[i][0]:上からi桁目まで見て、ぴったり支払う時の使用紙幣の枚数の最小値
dp[i][1]:上からi桁目まで見て、i桁目は余分に払っておつりをもらう
*/
ll dp[1000100][2];
int main()
{
    ...
    loop(i, N)
    {
        ...
        //1->0
        chmin(dp[i + 1][0], dp[i][1] + (10 - now));
        //0->1
        chmin(dp[i + 1][1], dp[i][0] + now + 1);
        //1->1
        chmin(dp[i + 1][1], dp[i][1] + (9 - now));
    }
    putout(dp[N][0]);
    return 0;
}
