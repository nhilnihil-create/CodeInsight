...
int main(){
    ...
    cin >> S;
    ll s = S.size();
    vector<vector<ll>> memo(s + 1,vector<ll>(2,INF));
    memo[0][0] = 0;
    memo[0][1] = 1;
    rep(i,s){
        ll x = S[i] - '0';
        chmin(memo[i + 1][0],memo[i][0] + x);
        chmin(memo[i + 1][0],memo[i][1] + (10 - x));
        chmin(memo[i + 1][1],memo[i][0] + x + 1);
        chmin(memo[i + 1][1],memo[i][1] + (9 - x));
    }
    cout << memo[s][0] << "\n";
}