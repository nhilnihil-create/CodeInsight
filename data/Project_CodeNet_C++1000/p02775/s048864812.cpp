...
int main(int argc, const char * argv[]) {
	...
	rep(i, N.size()+1) rep(j, 2) dp[i][j] = LLINF;
	dp[0][0] = 0;
	rep(i, N.size()){
		int d = N[i] - '0';
		rep(j, 2){
			rep(k, 2){
				...
				if(j == 0 && k == 0){
					chmin(dp[i+1][k], dp[i][j] + d);
				}
				else if(j == 0 && k == 1){
					chmin(dp[i+1][k], dp[i][j] + d+1);
				}
				else if(j == 1 && k == 0){
					chmin(dp[i+1][k], dp[i][j] + (10 - d));
				}
				else if(j == 1 && k == 1){
					chmin(dp[i+1][k], dp[i][j] + (9 - d));
				}
			}
		}
	}
