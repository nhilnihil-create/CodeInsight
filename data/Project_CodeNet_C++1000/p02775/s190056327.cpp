...
int64_t f(int64_t x) { return (x == 9 ? 1 : x + 1); }
int64_t g(int64_t x) {
  int64_t a = x + 1;
  int64_t b = 10 - x;
...
signed main(int argc, char* argv[]) {
  ...
  string N_;
  In(N_);
  int64_t p = int64_t(N_.size());
  vector<int64_t> N(p);
  rep(i, p) N[i] = N_[i] - '0';

  vector<vector<int64_t>> dp(
      p, vector<int64_t>(2, numeric_limits<int64_t>::max()));

  dp[0][0] = min(N[0], 10 - N[0] + 1);
  dp[0][1] = min(f(N[0]), 10 - f(N[0]) + 1);

  rep(i, 1, p) {
    ChMin(dp[i][0], dp[i - 1][0] + N[i]);
    ChMin(dp[i][1], dp[i - 1][0] + f(N[i]));
    ChMin(dp[i][0], dp[i - 1][1] + g(N[i]));
    ChMin(dp[i][1], dp[i - 1][1] + h(N[i]));
  }
