...
int main()
{
	...
	vvt<ll> dp; MakeVVT(Size(N), 2, dp, Pow10(8));
	dp[0][0] = 0;
	rep(i, 0, Size(N) - 1) rep(j, 0, 2)
	{
		rep(pay, 0, 10)
		{
			if (pay - j < CtoL(N[i]))
			{
				ll change = 10 + pay - j - CtoL(N[i]);
				dp[i + 1][1] = min(dp[i + 1][1], dp[i][j] + pay + change);
			}
			else
			{
				ll change = pay - j - CtoL(N[i]);
				dp[i+1][0] = min(dp[i+1][0], dp[i][j] + pay + change);
			}
		}

	}

	cout << min(dp.back()[0], dp.back()[1] + 1);
	return 0;
}
