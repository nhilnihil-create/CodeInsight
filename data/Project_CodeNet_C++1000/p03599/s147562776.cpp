...
int main(int argc, char* argv[]){
	...
	cin >> A >> B >> C >> D >> E >> F;
	double maxconc = (double)(E) / (double)(100 + E);
	...
	for(water = 100; water <= F; water += 100){
		int possibleW = possibleWater(water);
		if(possibleW != 2) continue;
		...
		for(v = 0; (D * v) <= F - water; v++){
			for(u = 0; (C * u) <= F - water - (D * v); u++){
				...
				double tmp = ((double) sugar) / ((double) (water + sugar));
...
#ifdef DEBUG1
	printf("conc=%.6lf\n", (double)(sugar1) / (double)(sugar1 + water1));
#endif
