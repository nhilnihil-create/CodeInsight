...
signed main() {
    ...
    // 100Ax + 100By + Cz + Dw <= F, Cz+Dw <= E*(Ax+By)
    int x, y, z, w;
    double dens;
    x = 0;
    while (100*a*x <= f) {
        y = 0;
        while (100*a*x + 100*b*y <= f) {
            sugar_lim = min(e*(a*x+b*y), f-100*a*x-100*b*y);
            z = 0;
            while (c*z <= sugar_lim) {
                w = 0;
                while (c*z + d*w <= sugar_lim) {
                    dens = (c*z+d*w)*1.0/(100*a*x+100*b*y+c*z+d*w);
                    if (dens > mx_dens) {
                        mx_dens = dens;
                        best_vol = 100*a*x+100*b*y+c*z+d*w;
                        best_sugar = c*z+d*w;
                    }
                    w++;
                }
