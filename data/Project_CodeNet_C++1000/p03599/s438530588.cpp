...
int main(){
  ...
  ld max_sugarratio = ld(-1);
  ll sugarwater_when_max = -1;
  ll sugar_when_max = -1;
  // A, B, "D" の投入量を全探索 Cは最大3000近くいくのでやめておく
  // こぼれないように入れていく
  rep(a, F/(A*100) + 1){
    rep(b, (F-a*A*100) / (B*100) + 1){
      if (a == 0 && b == 0) continue; // 水が入っていない
      rep(d, (F - a*A*100 - b*B*100) / D + 1){
        // コーナーケース処理
        if ((a*A+b*B) * E < (d * D) ) continue; // 溶け切らない

        // c(Cを入れる回数)を、ビーカーからこぼれず、かつ溶け切るように求める
        ll c = min( (F - a*A*100 - b*B*100 - d*D) / C , ((a*A+b*B)*E - d*D) / C );

        ld sugarratio = ld((c*C+d*D)*100) / ld((a*A+b*B)*100 + c*C + d*D);
        if (max_sugarratio < sugarratio ){
          max_sugarratio = sugarratio;
          sugarwater_when_max = (a*A+b*B)*100 + c*C + d*D;
          sugar_when_max = c*C+d*D;
        }
      }
    }
  }

  cout << sugarwater_when_max << " " << sugar_when_max << endl;
