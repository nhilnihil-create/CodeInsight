...
int32_t main()
{
	...
	ld ans = 0.000000000000000;
	//int p1, p2;
	//ld ans = 0;
	int p1 = 0, p2 = 0;
	rep(i, 0, 3000)// amount of sugar =c*i
	{
		int flag = 0;
		rep(j, 0, 3000) // amount of sugar =d*j
		{
			int ts = c * i + d * j;
			if (ts == 880)
			{
			...
			rep(k, 0, 100)
			{
				rep(l, 0, 100)
				{
					if (100 * k * a + 100 * l * b > max_usable)
						break;
					int water = 100 * k * a + 100 * l * b;
					if (water >= minw && (ts + water) > 0)
					{
						ld var = (100.000000000000 * ts) / (ts + water);
						if (var >= ans)
						{
							ans = var;
							p1 = water + ts;
