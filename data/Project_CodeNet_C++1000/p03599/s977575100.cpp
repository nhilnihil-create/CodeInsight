...
int main()
{
  ...
  set<ll> st;
  deque<ll> dq;
  dq.emplace_back(100*A);
  dq.emplace_back(100*B);
  ll maxw = F;
  while(!dq.empty()){
    ll tmp = dq.front(); dq.pop_front();
    if(tmp <= maxw && st.find(tmp) == st.end()){
      st.insert(tmp);
      dq.emplace_back(tmp+100*A);
      dq.emplace_back(tmp+100*B);
    }
  }
  //砂糖のありうる値
  set<ll> st2; st2.insert(0);
  deque<ll> dq2;
  dq2.emplace_back(C);
  dq2.emplace_back(D);
  ll maxs = F*E/(100+E);
  while(!dq2.empty()){
    ll tmp = dq2.front(); dq2.pop_front();
    if(tmp <= maxs && st2.find(tmp) == st2.end()){
      st2.insert(tmp);
      dq2.emplace_back(tmp+C);
      dq2.emplace_back(tmp+D);
    }
  }
  
  //main
  double ans = 0; ll anss=0, anst=0;
  for(ll s : st){
    for(ll t: st2){
      if(t <= s*E/100 && s+t <= F){
        double tmp = (double)t/((double)s+(double)t);
        if(tmp > ans){
          ans = tmp;
          anss = s; anst = t;
        }
  ...
  // output
  if(anss == 0 && anst == 0) anss = 100*A;
  ll ansst = anss+anst;
  cout << ansst << " " << anst << endl;
