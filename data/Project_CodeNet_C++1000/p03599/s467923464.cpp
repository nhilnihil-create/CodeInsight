...
int main()
{
    ...
    loop(o, 30)
    {
        loop(p, 30)
        {
            loop(q, 101)
            {
                loop(r, 101)
                {
                    long double water = 100.0 * a * o + 100.0 * b * p;
                    long double sugar = c * q + d * r;
                    if (water + sugar > f)
                        continue;
                    if ((water / 100) * e < sugar)
                        continue;
                    long double now = sugar / (water + sugar);
                    if (now > level)
                    {
                        level = now;
                        ans1 = sugar + water;
    ...
    cout << (ll)(ans1 + 0.01) << " " << (ll)(ans2 + 0.01) << endl;
    return 0;
}
