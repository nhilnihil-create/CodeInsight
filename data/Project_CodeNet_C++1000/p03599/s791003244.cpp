...
signed main()
{
    ...
    for(int i=0;i<=3000;i+=a*100){
        for(int j=0;j<=3000;j+=b*100){
            for(int k=0;k<=1000;k+=c){
                for(int l=0;l<=1000;l+=d){
                    if(i==0&&j==0&&k==0&&l==0) continue;
                    double water=i+j;
                    double suger=k+l;
                    if(suger*100>water*e) continue;
                    if(water+suger>f) continue;
                    double memo=100*suger/(suger+water);
                    if(chmax(noudo,memo)){
                        answ=water+suger;
                        anss=suger;
                    }
