...
/*
    ...
    Title : DP_G_Modified.cpp
    Date : 27/08/2020   >> 00:54:32
*/
...
int32_t dfs(int v, vc(vl)& g, vc(int)& dp) {
    if (dp[v] != -1)
        return dp[v];

    int mx = 0;
    trv(i, g[v]) 
        mx = max(mx, 1 + dfs(i, g, dp));
    dp[v] = mx;
    return dp[v];        
}

int32_t longestPath(vc(vl)& g) {
    int n = sz(g), mx = -INF(int);
    vc(int) dp(n, -1); 
    repa(i, n) 
        dfs(i, g, dp);
