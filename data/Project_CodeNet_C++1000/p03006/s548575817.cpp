...
rep(i,N){
  rep(j,N){
    if(i==j)continue;
    count[make_pair(table[i].first-table[j].first,table[i].second-table[j].second)]++;
...
rep(i,N){
  rep(j,N){
    if(i==j)continue;
    max_=max(max_,count[make_pair(table[i].first-table[j].first,table[i].second-table[j].second)]);
