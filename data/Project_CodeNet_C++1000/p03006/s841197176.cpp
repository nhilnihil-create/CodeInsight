...
int check(vector<pair<ll,ll>> &xy, set<pair<ll,ll>> balls, ll p, ll q){
  ll a = xy[0].first;
  ll b = xy[0].second;
  balls.erase(xy[0]);
  int res = 1;
  while (balls.size() > 0){
    if (balls.count({a+p, b+q})){
      balls.erase({a+p, b+q});
      a += p; b += q;
    }else{
      pair<ll,ll> tmp = *balls.begin();
      balls.erase(tmp);
      tie(a, b) = tmp;
      res++;
    }
...
int main() {
  ...
  sort(all(xy));
  set<pair<ll,ll>> balls;
  rep(i, N) balls.insert(xy[i]);
  int res = N;
  for (int i=0; i<N; i++){
    for (int j=i+1; j<N; j++){
      ll p = xy[j].first - xy[i].first;
      ll q = xy[j].second - xy[i].second;
      chmin(res, check(xy, balls, p, q));
    }
  }
  cout <<res <<endl;
}
