...
#define HelloWorld std::cin.tie(0);std::ios_base::sync_with_stdio(false)
...
signed main(){
    ...
    if(N%2==1){
        ...
        if(mp[0]==1)cnt++;
        if(cnt==mp.size()){ans=mypow(2,N/2);}
        else ans=0;
    }else{
        int cnt=0;
        RepA(mp)if(item.second==2)cnt++;
        if(cnt==mp.size())ans=mypow(2,N/2);
        else ans=0;
    }
    COUT(ans);
