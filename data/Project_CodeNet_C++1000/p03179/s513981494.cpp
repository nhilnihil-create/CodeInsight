...
int solve(int len, int st)
{
    ...
    if (st == len && u[n - len] == '<')
        return dp[len][st] = 0;
    if (u[n - len] == '>')
        return dp[len][st] = pref[len - 1][st - 1];
    else
        return dp[len][st] = suff[len - 1][st];
}
...
int32_t main()
{
    ...
    while (tests--)
    {
        ...
        {
            pref[i][0] = 0;
            f(j, 1, 1 + i) pref[i][j] = add(pref[i][j - 1], solve(i, j));
            suff[i][i] = dp[i][i];
            rf(j, i - 1, 1) suff[i][j] = add(suff[i][j + 1], dp[i][j]);
        }
        cout << suff[n][1] << nl;
