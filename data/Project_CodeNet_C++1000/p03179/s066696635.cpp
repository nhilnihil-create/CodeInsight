...
void solve(){
	int n; cin>>n;
	string s; cin>>s;
	vvl dp(n,vl(n,0)), sum(n,vl(n+1,0));
	dp[0][0]=1;
	fore(i,1,n+1)sum[0][i]=1;
	fore(i,1,n){
		if(s[i-1]=='>')fore(j,0,i+1)dp[i][j]=SUM(sum[i-1][n],-sum[i-1][j]), sum[i][j+1]=dp[i][j];
		else fore(j,0,i+1)dp[i][j]=sum[i-1][j], sum[i][j+1]=dp[i][j];
		fore(j,2,n+1)sum[i][j]=SUM(sum[i][j],sum[i][j-1]);
	}
	cout<<sum[n-1][n]<<ENDL;
}

int main(){_
	//int t; cin>>t; fore(tt,0,t)
