...
using namespace std;
int main(){
	string s;int n,i,j;cin>>n>>s;
	vi d(n+1,0),p(n+1,1);p[0]=0;for(i=1;i<n;i++){
		for(j=1;j<=i+1;j++){
			if(s[i-1]=='<') d[j]=p[j-1];
			else d[j]=(p[n]-p[j-1]+mod)%mod;
		}for(j=1;j<=n;j++) p[j]=(p[j-1]+d[j])%mod;
	}ll c=0;for(auto i:d) c=(c+i)%mod;cout<<c<<endl;
}
