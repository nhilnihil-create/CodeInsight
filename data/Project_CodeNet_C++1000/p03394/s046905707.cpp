...
int main(){
	lin(n);
	if(n==3){ cout<<"2 5 63\n"; return 0; }
	lint s=0,a[4]={2,3,4,6};
	set<lint>ans;
	rep(n){
		ans.insert(i/4*6+a[i%4]);
		s+=i/4*6+a[i%4];
	}
	if(s%6==2)ans.erase(8),ans.insert((ans.find(n/4*6+6)==ans.end()?n/4:n/4+1)*6+6);
	if(s%6==3)ans.erase(6),ans.insert((ans.find(n/4*6+3)==ans.end()?n/4:n/4+1)*6+3);
	if(s%6==5)ans.erase(8),ans.insert((ans.find(n/4*6+3)==ans.end()?n/4:n/4+1)*6+3);
	each(ans)cout<<i<<" ";cout<<endl;
}