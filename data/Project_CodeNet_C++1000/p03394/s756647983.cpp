...
void solve() {
  	scanf("%lld", &N);
	if(N==3){p("2 5 63");return;}
	if(N==4){p("2 5 20 63");return;}
	if(N==5){p("2 5 20 30 63");return;}
	vector<ll> ans(N,0);
	ll t=0;
	ll i=1;
	while(t<N){
		i++;
		if(i==0||i%6==1||i%6==5)continue;
		ans[t]=i;
		t++;
	}
	...
	ll index=-1;
	ll target=sum%6==5||sum%6==3 ? 9 : 8;
	ll mod=sum%6==5 ? 4 : 0;
	rep(i,N){
		if(target==ans[i]){index=i;break;}
	}
	while(1){
		i++;
		if(i%6==mod){
