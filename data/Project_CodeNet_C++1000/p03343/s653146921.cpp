...
int main() {
  ...
  A.push_back(0);
  int ans = INF;
  vector<int> higher_list, higher_all;
  REP(i, N) {
    int base = A[i], last = -1, cnt = 0;
    higher_list.clear();
    higher_all.clear();
    REP(j, N + 1) {
      if (A[j] < base) {
        int seg = j - last - 1;
        if (seg >= K) {
          sort(ALL(higher_list));
          REP(k, seg - K + 1) higher_all.push_back(higher_list[k]);
          cnt += seg - K + 1;
        }
        higher_list.clear();
        last = j;
      } else {
        higher_list.push_back(A[j]);
      }
    }
    // debug(i, cnt, higher_all);
    if (cnt >= Q) {
      sort(ALL(higher_all));
      ans = min(ans, higher_all[Q - 1] - base);
    }
  }
  io.writeln(ans);
  return 0;
