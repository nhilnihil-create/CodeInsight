...
signed main(){
  cin>>n>>k>>q;rep(i,n)cin>>a[i], ind[i] = P(a[i], i+1);
  int ans = INF;sort(ind,ind+n);
  rep(i,n-q+1){
    rep(i,2100)bit[i] = 0;
    int cnt = 0, pre = 0;set<int> untaken;
    rep(j,i)if(ind[j].first < ind[i].first)untaken.insert(ind[j].second);
    repd(j,i,n){
      auto x = untaken.upper_bound(ind[j].second);
      int u = (x == untaken.end() ? n : (*x)-1), v = (x == untaken.begin() ? 1 : *prev(x,1)+1);
      if(u-v+1-(sum(u)-sum(v-1)) >= k)cnt++, badd(ind[j].second, 1);
      else if(j < n-1 && ind[j].first < ind[j+1].first){
        repd(l,pre,j+1)if(sum(ind[l].second)-sum(ind[l].second-1) == 0)untaken.insert(ind[l].second);
        pre = j+1;
      }
      if(cnt == q){
        chmin(ans, ind[j].first - ind[i].first);break;
      }
    }
  }
  cout << ans << endl;
