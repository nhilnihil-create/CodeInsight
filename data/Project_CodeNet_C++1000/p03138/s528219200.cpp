...
int main(){
  ...
  rep(d, 45){
    ...
    if (d==0){
      if (K & (1LL << (44-d))){
        // Kの1桁目が1
        dp[d][0] = (N-turns[d]) * llpow(2, 44-d); // 未満でない = 1 を選んだ
        dp[d][1] = turns[d] * llpow(2, 44-d); // 未満である = 0 を選んだ
      }
      else{
        // Kの1桁目が0
        dp[d][0] = turns[d] * llpow(2, 44-d); // 未満でない=ピッタリ
        dp[d][1] = -1; // つまり変更なし
      }
    }
    else{
      // 1桁目以降～
      if (dp[d-1][1] == -1 && !(K & (1LL << (44-d)))){
        // まだ0が一度も出てきていなく、今回もKのbitが立っていない
        dp[d][0] = dp[d-1][0] + turns[d] * llpow(2, 44-d); // 未満でない=ピッタリ=bitを立てない
        dp[d][1] = -1; // つまり変更なし
      }
      else if (dp[d-1][1] == -1 && (K & (1LL << (44-d)))){
        // はじめてKのd桁目のbitが立った
        dp[d][0] = dp[d-1][0] + (N-turns[d]) * llpow(2, 44-d); // 未満でない=ピッタリ=bitを立てる
        dp[d][1] = turns[d] * llpow(2, 44-d); // 未満である = bitを立てない
      }
      else if (!(K & (1LL << (44-d)))){
        // 今までにbitが立ったことがあり、今回はKのbitが立っていない
        dp[d][0] = dp[d-1][0] + turns[d] * llpow(2, 44-d); // 未満でない=ピッタリ=bitを立てない
        dp[d][1] = dp[d-1][1] + max(turns[d] * llpow(2, 44-d), (N-turns[d]) * llpow(2, 44-d)); // 未満である=前回の時点で未満でない場合(dp[d-1][0])からは取れない（今回で未満にできないから）。前回の時点で未満になっていたところからなら、自由に取れる（今回立てても立てなくてもいい）
      }
      else if (K & (1LL << (44-d))){
        // 今までにbitが立ったことがあり、今回もKのbitが立った
        dp[d][0] = dp[d-1][0] + (N-turns[d]) * llpow(2, 44-d); // 未満でない=ピッタリ=bitを立てる
        dp[d][1] = max(dp[d-1][0] + turns[d] * llpow(2, 44-d) , max(dp[d-1][1] + (N-turns[d]) * llpow(2, 44-d) , dp[d-1][1] + turns[d] * llpow(2, 44-d) ) ); // 未満である=前回の時点で未満でない場合から取ってbitを立てないか、前回の時点で未満になっていたところから自由に取るか。よって3択となる
      }
    }
  }
