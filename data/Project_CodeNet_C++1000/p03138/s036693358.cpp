...
int main(){
   ...
   for(int d=0;d<MAX_DIGIT;++d){
      ...
      if(dp[d][1]!=-1){
         chmax(dp[d+1][1],dp[d][1]+max(cost0,cost1));//smaller->smaller
      }
      if(dp[d][0]!=-1){
         if(K &mask){
            chmax(dp[d+1][1],dp[d][0]+cost0);//same->smaller
            chmax(dp[d+1][0],dp[d][0]+cost1);//same->same
         }
         else{
            chmax(dp[d+1][0],dp[d][0]+cost0);//same->same(両方0)
         }
      }
   }
