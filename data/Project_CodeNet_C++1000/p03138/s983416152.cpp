...
int main()
{
  ...
  vector<vector<ll>> dp(cnt+1,vector<ll>(2,-1));
  //0→jまで一致。1→jまでに不一致。
  dp[cnt][0] = power(2,cnt)*(v[cnt]);
  
  for(int j=cnt-1; j>=0; j--){
    if((K>>j)%2==0){//0。
      dp[j][0] = max(dp[j][0],dp[j+1][0]+power(2,j)*v[j]);//本来は1の方が最大値を取る。
      if(dp[j+1][1] > -1){
        dp[j][1] = max(dp[j][1],dp[j+1][1]+power(2,j)*max(v[j],N-v[j]) );
      }
    }else{//1
      dp[j][0] = max(dp[j][0],dp[j+1][0]+power(2,j)*(N-v[j]) );
      dp[j][1] = max(dp[j][1],dp[j+1][0]+power(2,j)*v[j] );
      if(dp[j+1][1] > -1){
       dp[j][1] = max(dp[j][1],dp[j+1][1]+power(2,j)*max(v[j],N-v[j]) ); 
      }
    }
