...
signed main() {
    ...
    for (int d = 0; d < MAX_DIGIT; d++) {
        int mask = 1LL << (MAX_DIGIT - d - 1); // 2^(MAX_DIGIT-d-1)
        ...
        int point_when_x_0 = mask * original_bits;
        int point_when_x_1 = mask * (n - original_bits);

        // smaller -> smaller
        if (dp[d][1] != -1) {
            chmax(dp[d + 1][1], dp[d][1] + max(point_when_x_0, point_when_x_1));
        }

        // exact -> smaller
        if (dp[d][0] != -1) {
            if (k & mask) { // kのd桁目1
                chmax(dp[d + 1][1], dp[d][0] + point_when_x_0);
            }
        }
        ...
        if (dp[d][0] != -1) {
            if (k & mask) { // kのd桁目1
                chmax(dp[d + 1][0], dp[d][0] + point_when_x_1);
            } else {
                chmax(dp[d + 1][0], dp[d][0] + point_when_x_0);
            }
        }
    }
