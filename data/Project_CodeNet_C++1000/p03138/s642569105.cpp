...
int main() {
	cin >> N >> K; vl one(48);
	for (j = 0; j < 48; j++) one[j] = 0;
	for (i = 0; i < N; i++) {
		ll a; cin >> a; vl B = bitList(2, 48, a);
		for (j = 0; j < 48; j++) one[j]+=B[j];
	}
	vl P = bitList(2, 48, K); vll dp(49, vl(2, 0));
	reverse(P.begin(), P.end()); reverse(one.begin(), one.end());
	for (i = 0; i < 49; i++) for (j = 0; j < 2; j++) dp[i][j] = -1;
	dp[0][0] = 0;
	for (i = 0; i < 48; i++) {
