...
void solve(){
  auto f = [&](ll k, int d)->ll{
    return ( k/(1LL<<(d+1))*(1LL<<d) + max<ll>(0, k%(1LL<<(d+1))-(1LL<<d) ) ) ;
    
  };
  int n; ll k; cin >> n >> k;
  V<ll> a(n); cin >> a;
  V<ll> dp(2,-LINF), dppre(2);//  {?, ok}
  dp[0] = 0;
  IFOR(i,0,50){
    FOR(x1,0,2)  { dppre[x1] = dp[x1]; dp[x1] = -LINF;}
    ll c = 1LL&(k>>i);
    //ll num1 = f(k,i);
    //ll num0 = (k+1)-num1;couts(num0); coutn(num1);
    ll num0 = 0, num1 = 0;
    FOR(j,0,n){ if((a[j]>>i)&1) ++num1; else num0++;}
    if(dppre[1]>-LINF+100000) dp[1] = dppre[1] + max(num1,num0)*(1LL<<i);
    if(c==1){
      dp[0] = dppre[0] + num0 * (1LL<<i);
      dp[1] = max<ll>(dp[1], dppre[0] + num1*(1LL<<i) );
    }
    else{
      dp[0] = dppre[0] + num1 * (1LL<<i);
    }
    //coutn(dp);
    
  }
  coutn( max<ll>(dp[0], dp[1]) );
