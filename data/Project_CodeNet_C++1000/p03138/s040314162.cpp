...
int main(void) {
    ...
    FOR(i, -1, 42) {
        ...
        map<LL, LL> xcnt;
        for(auto x : kcnt) {
            if(idx < i) xcnt[x.first] = 1;
            if(idx == i) xcnt[x.first] = 0;
            if(idx > i) xcnt[x.first] = x.second;
            idx++;
        }
        LL x = 0;
        for(auto p : xcnt) x += p.first * p.second;
        if(x >= k) continue;
        // cerr << "===" << endl;
        // mprint(xcnt);
        // cerr << x << endl;
        LL tmp = 0;
        for(auto p : xcnt) {
            if(p.second) tmp += p.first * max(cnt[p.first], n - cnt[p.first]);
            else tmp += p.first * cnt[p.first];
        }
        ans = max(ans, tmp);
    }
    cout << ans << endl;
