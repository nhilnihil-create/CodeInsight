...
// ここから編集しろ
long long p[210000];
long long dp[210000][2][4];
int main(){
	int a;scanf("%d",&a);
	for(int i=0;i<a;i++)scanf("%lld",p+i);
	for(int i=0;i<=a+1;i++)for(int j=0;j<2;j++)for(int k=0;k<3;k++){
		dp[i][j][k]=-inf;
	}
	dp[0][0][0]=0;
	for(int i=0;i<=a;i++){
		for(int j=0;j<2;j++){
			for(int k=0;k<3;k++){
				if(j==1){
					...
					dp[i+1][1][k]=max(dp[i+1][1][k],dp[i][j][k]+p[i]);
					if(k+1<3)dp[i+1][0][k+1]=max(dp[i+1][0][k+1],dp[i][j][k]);
				}
			}
		}
	}
