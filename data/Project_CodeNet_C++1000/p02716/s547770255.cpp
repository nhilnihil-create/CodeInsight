...
ll solve(ll in, ll f1, ll f2, ll f3) {
    if(in > n) {
        if(f1) return 0;
        return -INF;
    }
    if(dp[in][f1][f2][f3] != -1) return dp[in][f1][f2][f3];

    ll t, res(-INF);

    if(f1 && f2) {
        t = a[in] + solve(in+2, 1, 1, f3);
        res = max(res, t);
    }
    else if(f1 && !f2) {
        t = a[in] + solve(in+2, 1, 0, f3);
        res = max(res, t);
        if(in+3 <= n) {
            t = a[in] + solve(in+3, 1, 1, f3);
            res = max(res, t);
        }
    }
    else {
        t = a[in] + solve(in+2, 0, 0, f3);
        res = max(res, t);

        if(in+3 <= n) {
            t = a[in] + solve(in+3, 1, 0, 0);
            res = max(res, t);
        }

        if(f3 && in+4 <= n) {
            t = a[in] + solve(in+4, 1, 1, 0);
            res = max(res, t);
        }
    }

    return dp[in][f1][f2][f3] = res;
}
