...
ll odd(int N, vi& a){
    ll NEG = -1000000000000000LL;
    vector<vll> dp(N+1, vll(3, NEG));
    dp[0][0]=0;
    dp[1][0]=a[0];
    dp[2][1]=a[1];
    dp[3][2]=a[2];
    rep(i,N){
        if (i-1 >= 0){
            amax(dp[i+1][0], dp[i-1][0] + a[i]);
            amax(dp[i+1][1], dp[i-1][1] + a[i]);
            amax(dp[i+1][2], dp[i-1][2] + a[i]);
        } else {
            ;
        }
        if (i-2 >= 0){
            amax(dp[i+1][1], dp[i-2][0] + a[i]);
            amax(dp[i+1][2], dp[i-2][1] + a[i]);
        } else {
            ;
        }
        if (i-3 >= 0){
            amax(dp[i+1][2], dp[i-3][0] + a[i]);
        } else {
            ;
        }
    }
    return max({dp[N][2], dp[N-1][1], dp[N-2][0]});
}
ll even(int N, vi& a){
    int H=N/2;
    vll lacc(H+1,0), racc(H+1,0);
    rep(i,H){
        lacc[i+1] = lacc[i] + a[i*2];
    }
    rep(i,H){
        racc[i+1] = racc[i] + a[N-1 -i*2];
    }
    #ifdef DEBUG
    cerr << N << a << endl;
    cerr << lacc << endl;
...
ll solve(int N, vi& a) {
    if (N % 2) return odd(N,a);
    else return even(N,a);
}
