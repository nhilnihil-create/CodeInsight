...
signed main()
{
    ...
    reverse(all(a));
    a.push_back(0);
    for (int i = 0; i < 300000; i++) for (int j = 0; j < 3; j++) dp[i][j][0] = dp[i][j][1] = -INFLL;
    dp[0][0][0] = 0;
    for (int i = 1; i <= n + 1; i++)
    {
        for (int h = 0; h < 3; h++)
        {
            dp[i][h][0] = max(dp[i][h][0], dp[i - 1][h][1]);
            dp[i][h][1] = max(dp[i][h][1], dp[i - 1][h][0] + a[i]);
            if (h + 1 < 3) dp[i][h + 1][0] = max(dp[i][h + 1][0], dp[i - 1][h][0]);
        }
    }
    cout << dp[n + 1][1 + (n % 2)][0] << endl;
}
