...
signed main()
{
    ...
    dp[0][0] = 0;
    dp[1][0] = a.at(0);
    dp[0][1] = max(a.at(0), a.at(1));
    rep(i, 2, n)
    {
        if (i % 2 == 1)
        {
            dp[0][i] = max(dp[1][i - 1], dp[0][i - 2] + a.at(i));
        }
        else
        {
            dp[1][i] = dp[1][i - 2] + a.at(i);
            dp[0][i] = max(dp[1][i - 2], max(dp[0][i - 1], dp[0][i - 2] + a.at(i)));
        }
    }
    cout << dp[0][n - 1] << endl;
    // rep(n) cout << dp[0][i] << " ";
