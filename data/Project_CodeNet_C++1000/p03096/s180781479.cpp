...
//    readint(test_cases);
    ...
    forr(t, 1, test_cases) {
        ...
        vector<int> c;
        forr(i,0,n) {
            readint(kk); c.push_back(kk);
        }

        vector<int> last_same_color;
        map<int,int> temp;
        forr(i,0,n) {
            if (set_map_includes(temp, c[i])) {
                last_same_color.push_back(temp[c[i]]);
            } else {
                last_same_color.push_back(-1);
            }
            temp[c[i]] = i;
        }

        vector<ll> count_the_ways(n);
        count_the_ways[0] = 1;
        forr(i,1,n-1) {
            if (c[i]==c[i-1]) {
                count_the_ways[i] = count_the_ways[i-1];
            } else {
                count_the_ways[i] = count_the_ways[i-1];
                if (last_same_color[i] >= 0) {
                  count_the_ways[i] += count_the_ways[last_same_color[i]];
                }
                count_the_ways[i] = count_the_ways[i] % modd;
            }
        }

        cout << count_the_ways[n-1] << endl;
