...
//vector<vr> g,mat;
vector<vr> dp;
vr height,beauty,fenwick ;
ll n;
/*ll go(ll index,ll prev,ll parity)
{
	//debug3(index,sum,prev);
	if(index==n+1)
	{
		return 0;
	}
	parity=0;
	if(height[prev]>height[index])
	{
		parity=1;
	}
	...
	dp[index][0]=max(beauty[index]+ go(index+1,max(index,prev),0),go(index+1,prev,0));
	dp[index][1]=go(index+1,prev,0);
	return dp[index][parity];
}*/
void update(ll index,ll val)
{
	while(index<=n)
	{
		fenwick[index]=max(val,fenwick[index]);
		index+=index&(-index);
	}
}
ll query(ll index)
...
signed main()
{
	...
	{
        	cin>>n;
				  height.resize(n+1,0),beauty.resize(n+1);
					f(1,n)
					{
						cin>>height[i];
					}
					...
					for(ll i =n;i>=1;i--)
					{
						ll mx=query(n-height[i]+1);
						update(n-height[i]+1,mx+beauty[i]);
					}
					ll ans=0;
					f(1,n)
					{
