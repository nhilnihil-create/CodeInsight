...
const int siz=200003;
vector <ll> h(siz),cst(siz);
vector <ll> segt(4*siz,0);
ll query(ll sind,ll left,ll right,ll qleft,ll qright)
{
   // cout<<left <<" "<<right <<" "<<qleft<<" "<<qright<<endl;
    ...
    if(qleft<=left && right<=qright)
        return segt[sind];
    ll mid=(left+right)/2;
    return max(query(2*sind+1,left,mid,qleft,qright),query(2*sind+2,mid+1,right,qleft,qright));
}
void update(ll sind,ll left,ll right,ll pos,ll val)
{
    if(left==right)
        {segt[sind]=val;return;}
    ll mid=(left+right)/2;
    if(pos<=mid)
        update(2*sind+1,left,mid,pos,val);
    else
        update(2*sind+2,mid+1,right,pos,val);
    segt[sind]=max(segt[2*sind+1],segt[2*sind+2]);
}
int main()
{
    #ifndef ONLINE_JUDGE
   ...
   // cin>>test;
    while(test--)
    {
            ...
            fo(i,n)
            {
                ll ht=h[i];
                ll cos=cst[i];
               /// t[ht]=cos+ *(max_element(t.begin(),t.begin()+ht));
                ll val=cos+query(0,0,n-1,0,ht);
                //cout<<val<<endl;
               // exit(0);
                update(0,0,n-1,ht,val);
            }
            cout<<segt[0]<<endl;
            //fo(i,n)
