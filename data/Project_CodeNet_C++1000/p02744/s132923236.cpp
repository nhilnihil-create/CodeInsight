...
int main(void){
  ...
  while(q.size()!=0){
    pair<char, string> now = q.front();
    q.pop();

   if((now.second).size()==n){
     cout<<now.second<<endl;
   }else{
     char max = 'a';
     REP(i, (now.second).size()){
       //cout<<max<<" ";
       if(now.second[i]>max) max = now.second[i];
     }
    for(char X='a';X<=max+1;X++){
      q.push(make_pair(X, now.second+X));
    }
   }
  }
