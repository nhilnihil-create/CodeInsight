...
vector<int> TopologicalSort(set<pair<int,int>> edges,int n){
  //nは頂点数
  //0_index
  vector<int> results;
  set<int> notins;//入力辺を持たない辺の集合
  vector<set<int>> ins(n);//入力辺集合
  //notinsを構築する
  for(auto i:edges){
    ins[i.second].insert(i.first);
  }
  for(int i=0;i<n;i++){
    if(ins[i].size()==0){
      notins.insert(i);
  ...
  //notins構築完了
  vector<set<int>> nexts(n);//各頂点に対する辺情報
  for(auto i:edges){
    nexts[i.first].insert(i.second);
  }
  //辺入力完了
  //本計算

  while(notins.size()>0){
    int nownode=*notins.begin();
    notins.erase(nownode);
    results.push_back(nownode);
    set<int>tmp=nexts[nownode];
    for(auto i:tmp){
      int nownext=i;
      nexts[nownode].erase(nownext);
      edges.erase({nownode,nownext});
      ins[nownext].erase(nownode);
      if(ins[nownext].size()==0){
        notins.insert(nownext);
      }
    }
  }
  if(edges.size()>0){
...
int main(void){
  ...
  rep(i,n-1+m){
    a[i]--;b[i]--;
    //insertree_unidire(1,a[i],b[i],i);
  }
  vi inn(n,0);
  vi outn(n,0);
  rep(i,n-1+m){
    inn[b[i]]++;//その頂点に入ってくる数
    outn[a[i]]++;//その頂点から出ていく数
  }
set<pair<int,int>> edges;
  rep(i,n-1+m){
    edges.insert({a[i],b[i]});
  }
  vi result = TopologicalSort(edges,n);
  vi ans(n,-1);
  vector<set<int>> ins(n);//入力辺集合
  for(auto i:edges){
    ins[i.second].insert(i.first);
  ...
  vector<set<int>> in_locs(n);
  rep(i,n){
    for(auto j:ins[i]){
      in_locs[i].insert(locs[j]);//入ってくる矢印の場所の記録
    }
  }
  rep(i,n-1){
    cout<<endl;
    int aa;
    auto itr=in_locs[result[i+1]].lower_bound(i+1);
    itr--;
    aa=*itr;
    ans[result[i+1]]=result[aa];
  }
  rep(i,n){
    cout<<ans[i]+1<<endl;
  }
