...
//Library-End
int n, m, a, b, groot, ans[100004],fukasa[100004],basho[100004];
bool itta[100004];
vector<int> children[100004], par[100004],toporojikaru;
void solve(int v) {
    itta[v] = 1;
    rep(i, children[v].size()) {
        if (itta[children[v][i]]) {
            continue;
        }
        solve(children[v][i]);
    }
    toporojikaru.push_back(v);
}
...
signed main() {
    ...
    reverse(toporojikaru.begin(), toporojikaru.end());
    rep(i, n) {
        basho[toporojikaru[i]] = i;
    }
    rep(i, n) {
        if (!basho[i])continue;
        //cout << 'p' << endl;
        int ka = 0;
        int now = 0 - inf;
        rep(j, (int)par[i].size()) {
            if (now < basho[par[i][j]]) {
                ka = par[i][j]+1;
                now = basho[par[i][j]];
            }
        }
