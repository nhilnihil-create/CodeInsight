...
ll n,m,a[110000],b[110000],k[110000]={},u[110000]={},ans[110000]={},l;
vector<vector<ll>>p(110000);
vector<vector<ll>>r(110000);
vector<ll>q;

void dfs(ll x){
  if(k[x])return;
  k[x]=1;
  rep(i,p[x].size())dfs(p[x][i]);
  q.push_back(x);
}
...
int main(){
  ...
  rep(i,n)if(r[i].size()==0)dfs(i);
  rep(i,n)u[q[i]]=i;
  rep(i,n){
    l=INF;
    rep(j,r[i].size())l=min(u[r[i][j]],l);
    rep(j,r[i].size())if(l==u[r[i][j]])ans[i]=r[i][j]+1;
  }
  rep(i,n)printf("%lld\n",ans[i]);
}