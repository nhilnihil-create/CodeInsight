...
SINT64 top;
vector<SINT64> root;
vector<vector<SINT64>> up(100005);
vector<vector<SINT64>> down(100005);
...
void dfs2(SINT64 x,SINT64 cnt) {
	root[x] = MAX(cnt,root[x]);
	cnt++;
	rep(i,0,down[x].size()) {
		dfs2(down[x][i],cnt);
	}
...
int main() {
	...
	while(q.size() != 0) {
		...
		buf.F++;
		rep(i,0,down[buf.S].size()) {
			ccc[down[buf.S][i]]--;
			if (ccc[down[buf.S][i]] == 0){
				q.push(Pll(buf.F,down[buf.S][i]));
			}
		}
	}
