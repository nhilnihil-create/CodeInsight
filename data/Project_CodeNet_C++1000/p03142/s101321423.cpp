...
//---------------------------------------------------------
// トポロジカルソート
void tsort_dfs(ll n, vvll& to, vector<bool>& visited, vll& result)
{
    if (!visited[n]) {
        visited[n] = true;
        for (auto t : to[n]) {
            tsort_dfs(t, to, visited, result);
        }
        result.push_back(n);
    }
}
void tsort(ll N, vvll& to, vll& result)
{
    vector<bool> visited(N, false);
    for1 (i, N) {
        tsort_dfs(i, to, visited, result);
    }
    reverse(all(result));
}
//---------------------------------------------------------
