...
int32_t main()
{
	...
	__int128 le = x - k * d, ri = x + k * d;
	__int128 ans = linf;
	if(ri < 0){
		return cout << (ll)abs(ri) << endl, 0; 
	}
	if(le > 0){
		cout << (ll)abs(le) << endl;
		return 0;
	...
	ll kp = k - stp;
	if(kp & 1){
		if(x>0)write(abs(x - (stp+1) * d));
		else write(abs(x + (stp+1) * d));
	}else{
		if(x>0)write(abs(x - stp * d));
		else write(abs(x+stp*d));
	}
    return 0;
}