...
void solve(){
    ...
    vi inds_of_R, inds_of_W;

    int curr_streak = 0;

    for(int i=0;i<N;i++){
        if(c[i] == 'W'){
            inds_of_W.push_back(i);
        }else{
            // then is R
            inds_of_R.push_back(i);
        }
    }

    if(inds_of_W.size() == 0 || inds_of_R.size() == 0){
        cout<<0<<endl;
        return;
    }

//    reverse(inds_of_R.begin(), inds_of_R.end());

    int tot_moves_made = 0;
    int j = 0;
    for(int i=inds_of_R.size()-1;i>=0;i--){
        // Try fill streak
        int curr_R_ind = inds_of_R[i];
        int curr_W_ind = inds_of_W[j];

        if(curr_R_ind<curr_W_ind){
            break;
        }
