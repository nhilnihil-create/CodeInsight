...
int32_t main()
{
	...
	// now I have generated the map i,j -> ith shop jth day mai konse konse time khuli thi..
	// now what can we do is calculate number of shops opened in
	int ans = -INF;
	for (int mask = 1; mask < (1 << 10ll); mask++)
	{
		map<int, int>m;
		int cnt = 0;
		zip(i, 0, n)
		{
			int c = 0;
			zip(j, 0, 10) if (mask & (1 << j) && f[i][j])c++;
			cnt += p[i][c];
		}
		// zip(i, 0, 10)
		// {
		// 	if (mask & (1 << i)) //  i+1 day pe dukan khuli ho ..
		// 	{
		// 		cnt++;
		// 		zip(j, 0, n)
		// 		{
