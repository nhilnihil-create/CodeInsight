...
int main()
{
  ...
  ll n; cin >> n;
  vvll open(n,vll(10));
  FOR(i,n){ FOR(j,10){ cin >> open[i][j]; } }
  vvll prof(n,vll(10+1));
  FOR(i,n){ FOR(j,10+1){ cin >> prof[i][j]; } }

  //main
  ll ans = -1e18;
  for(int i=1; i<power(2,10); i++){
    ll tmp = 0;
    vll bothOpen(n,0);
    for(int j=0; j<10; j++){
      if((i>>j)%2 == 1){//j番目に自身が営業しているとき。
        for(int k=0; k<n; k++){//j番目に他が営業しているとき。
          if(open[k][j]){
            bothOpen[k] ++;
          }
        }
