...
int main() {
    int N;cin >> N;
    vector<vector<long long>> open(N,vector<long long>(10));
    vector<vector<long long>> money(N,vector<long long>(11));
    for(int i = 0;i<N;i++){
        for(int j = 0;j<10;j++){
            cin >> open.at(i).at(j);
        }
    ...
    for(int i = 1;i<(1 << 10);i++){
        bitset<11> BIT(i);
        long long nowprofit = 0;
        for(int j = 0;j<N;j++){
            int dupli = 0;
            for(int k = 0;k<11;k++){
                if(BIT.test(k) == 1 && open.at(j).at(k) == 1) dupli++;
            }
            nowprofit += money.at(j).at(dupli);
        }
        ans = max(ans,nowprofit);
    }
    cout << ans << endl;
}
