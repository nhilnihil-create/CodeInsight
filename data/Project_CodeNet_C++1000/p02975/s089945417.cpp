...
signed main(){
	...
	rep(0,i,n-1){
		if(a[i]!=a[i+1]){
			diff++;
			if(henka1==0)henka1=i+1;
			else henka2=i+1;
		}
	}
	if(diff==0&&a[0]==0)c("Yes")
	else if(diff==1&&a[0]==0&&henka1==n/3)c("Yes")
	else if(diff==2){
		if(henka1*3==n&&n/3*2==henka2){
			if(XOR(a[0],a[n/3])==a[n*2/3]&&XOR(a[0],a[n*2/3])==a[n/3]&&XOR(a[n*2/3],a[n/3])==a[0]){
				c("Yes")
				return 0;
			}
		}
