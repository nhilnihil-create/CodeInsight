...
int main(void){
  ...
  map< ll,ll,greater<ll> > mRest;
  map< ll,ll,greater<ll> > mCur;
  map< ll,ll,greater<ll> > mAdd;
  mCur[s[0]]=1;
  for(ll i=1;i<num;i++) mRest[ s[i] ]++;

  for(ll i=0;i< n ; i++ ){
    mAdd.clear();
    for(auto p:mCur){
      for(int j=0;j<p.second;j++){
        auto itr = mRest.lower_bound(p.first);
        if(itr==end(mRest)){
          cout << "No" << endl;
          return 0;
        }
        if(itr->first == p.first) itr++;
        if(itr==end(mRest)){
          cout << "No" << endl;
          return 0;
        }
        ...
        mRest[key]--;
        if(mRest[key]==0) mRest.erase(key);
        mAdd[key]++;
      }
      
    }
    for(auto p:mAdd){
      mCur[p.first]+=p.second;
    }
    //cout << endl;
