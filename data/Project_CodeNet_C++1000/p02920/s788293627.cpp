...
int main()
{
	...
	vector<ll> comp_count(N + 1);
	comp_count[N] = 1;
	bool can_make = true;
	for (auto iter = S.begin(); iter != S.end(); ++iter)
	{
		vector<ll> comp_add(N + 1);
		auto elem_count = iter->second;
		if (elem_count > accumulate(comp_count.begin(), comp_count.end(), 0)) { can_make = false; break; }
		ll pos = N;
		for (; elem_count > 0; --elem_count)
		{
			for (; pos >= 0 && comp_count[pos] == 0; --pos) {}
			--comp_count[pos];
			for (ll i = 0; i < pos; ++i)
			{
				++comp_add[i];
