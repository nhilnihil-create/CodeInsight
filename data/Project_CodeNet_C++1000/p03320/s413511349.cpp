...
double getScore(ll n)
{
	...
	return static_cast<double>(n) / static_cast<double>(s);
}

ll nextSnukeNumber(ll n)
{
	ll res = n;
	double bestScore = getScore(n);

	ll tenPower = 1LL;
	while ( n / tenPower >= 1)
	{
		tenPower *= 10LL;
		ll cand = tenPower * (n / tenPower + 1) - 1;
