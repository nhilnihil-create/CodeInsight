...
int main()
{
	...
	for(int i=0;i<=15;i++)
	{
		...
		while(1)
		{
			if(n==0)break;
			fo++;
			if(1.0*(pow(10LL,1LL*i)*(fo)-1)/(9LL*i+(fo-1)%10+(fo-1)%100/10+(fo-1)%1000/100+(fo-1)%10000/1000)<1.0*(pow(10LL,1LL*i)*(fo+1)-1)/(9LL*i+(fo)%10+(fo)%100/10+(fo)%1000/100+(fo)%10000/1000)+1e-6)
				cout<<(INT)pow(10LL,1LL*i)*fo-1<<endl;
			else
				break;
			n--;
		}
