...
vector<ll> G[MAX]; //グラフ
ll color[MAX]; // 頂点iの色(1 or -1)
ll b = 0, w = 0;
// 頂点を1と-1で塗っていく
bool dfs(ll v, ll c) {
	color[v] = c; // 頂点vをcで塗る
...
void solve(long long N, long long M, vector<long long> A, vector<long long> B){
	if (!dfs(0, 1))	cout << max(N*(N-1)/2-M,0LL) << endl;
	else cout << max(b*w-M,0LL)<< endl; 
	//頂点0を1で塗って始めた時、2部グラフでなかったら全頂点間を結べるのでN*(N-1)/2-M
	//2部グラフだったら黒と白の頂点数の積-M
}
...
int main(){
	...
	for(int i = 0 ; i <= M-1 ; i++){
		cin >> A[i];
		cin >> B[i];
		G[A[i]-1].push_back(B[i]-1); //グラフを作る
		G[B[i]-1].push_back(A[i]-1); //グラフを作る
	}
	solve(N, M, A, B);
	return 0;
}