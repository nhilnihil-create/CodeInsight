...
string i2b(int n) { string f = ""; while(n) {f += (n%2)+'0';n /= 2;}while(f.size() != 30) f += '0';reverse(all(f));return f;}

int b2i(string s) {int ans = 0;for(int i=0;i<s.size();i++) {ans += (s[i]=='1')*((1<<(s.size()-i-1)));}return ans;}
