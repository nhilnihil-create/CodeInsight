...
int M = 3000;
ll dp[55][55][7000+1];
// dp[i][j][k]: x[i]までのうち j 枚を使って値を k にするような場合の数

int main(){
  int n, a; cin >> n >> a;
  vi x(n); rep(i,n) cin >> x[i];
  auto id = [&](int num){return num + M;};
  dp[0][0][id(0)] = 1;
  rep(i,n) rep(j,n) for(int k = -M; k <= M; k++) {
  // printf("i:%d j:%d\n", i, j);
    dp[i+1][j][id(k)] += dp[i][j][id(k)];
    dp[i+1][j+1][id(k+x[i])] += dp[i][j][id(k)];
  }
  ll ans = 0;
  for(int j = 1; j < n+1; j++) {
    ans += dp[n][j][id(j*a)];
