...
// dp[i][j][k] := i-1個目まで見てj個選んで、合計をkにできるか
ll dp[52][52][2503];
...
int main(){
    ...
    V<> a(n);
    cin >> a;
    repn(i, n) repn(j, n) repn(k, 2500) dp[i][j][k] = 0;
    dp[0][0][0] = 1;
    rep(i, n) repn(j, n) repn(k, 2500){
        // a[i]を使わない
        dp[i+1][j][k] += dp[i][j][k];
        // a[i]を使う
    ...
    ll ans = 0;
    srepn(i, 1, n) repn(j, 2500) {
        if(j / i == k && j % i == 0) ans += dp[n][i][j];
    }
    cout << ans << endl;
}