...
ll pura,mai,zero;
ll flag;
ll dp[51][3000];//カードをiまい使って合計をjにする
...
int main(){
    ...
    dp[0][0]=1;
    for(ll i=0;i<n;i++){//i枚目において使うか
      vector<P> oppai;
      for(ll j=0;j<n;j++){
        for(ll k=0;k<2500;k++){
          if(dp[j][k]!=0){
            oppai.push_back(P(j,k));
          }
        }
      }
      sort(oppai.begin(),oppai.end(),greater<P>());
      for(ll j=0;j<oppai.size();j++){
        dp[oppai[j].first+1][oppai[j].second+a[i]]+=dp[oppai[j].first][oppai[j].second];
      }
    }
    for(ll i=1;i<=n;i++){
      ans+=dp[i][i*k];
