...
constexpr ll K_MAX = 5000;
constexpr ll Q_MAX = 5000;
constexpr ll D_I_MAX = 1e9;
constexpr ll N_I_MAX = 1e9;
constexpr ll X_I_MAX = 1e9;
constexpr ll M_I_MAX = 1e9;
//constexpr ll MOD = z;
...
void solve() {
	...
	x.assign(Q, 0);
	m.assign(Q, 0);
	rep(i, Q) { scanf("%lld%lld%lld", &n[i], &x[i], &m[i]); }
	
	vector<ll> ansl(Q);
	rep(q,Q){
		vector<ll> modd(K);
		rep(i,K)modd[i]=d[i]%m[q];
		...
		//s: a[x]>a[x+1]の個数
		ll s;
		ll sum=accumulate(all(modd),0LL);
		ll a_n_1=x[q]+(n[q]-1)/K*sum;
		rep(i,(n[q]-1)%K){
			a_n_1+=modd[i];
		}
		ll q_n_1=a_n_1/m[q];
