...
int main(){
  ...
  REP(q,Q){
    ...
    vll zero_count(K, 0);
    vll d_sum(K, 0);
    if(d[0]%m == 0) zero_count[0] = 1;
    FOR(k,1,K){
      zero_count[k] = zero_count[k-1];
      if(d[k]%m==0) zero_count[k]+=1;
    }
    d_sum[0] = d[0]%m;
    FOR(k,1,K){
      d_sum[k] += d_sum[k-1] + d[k]%m;
    }

    //vec_print(zero_count);

    ll num_of_zero = 0;
    num_of_zero += zero_count[K-1]*((n-2)/K);
    num_of_zero += zero_count[(n-2)%K];

    debug_print(num_of_zero);

    ll a_min = x;
    ll a_max = x + d_sum[K-1]*((n-2)/K);
    a_max += d_sum[(n-2)%K];

    debug_print(a_min, a_max);
    ...
    debug_print(b_min, b_max);

    ll m_count = b_max-b_min;
    */
