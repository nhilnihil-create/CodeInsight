...
signed main() {
  ...
  REP(i, q) {
    ...
    REP(j, k) {
      modD[j] = d[j] % m;
      cnt0 += modD[j] == 0;
      if (j < (n - 1) % k) cnt1 += modD[j] == 0;
    }
    int shuki = accumulate(modD, modD + k, 0ll);
    //終わりの項
    int v0 = x + shuki * ((n - 1) / k) + accumulate(modD, modD + (n - 1) % k, 0ll);
    int v1 = v0 / m - x / m;
    int ret = n - 1 - v1 - cnt0 * ((n - 1) / k) - cnt1;
    print(ret);
  }
}
