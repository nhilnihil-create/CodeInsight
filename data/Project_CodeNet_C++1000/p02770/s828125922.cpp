...
signed main(){
    ...
    REP(_,q){
        I(n); I(x); I(m);
        // m-進法でみたとき、いわゆる繰り上がりが起こらない回数に等しい
        // あと m-進で 0 もアウト
        vi dmod;
        REP(i,k) dmod.pb(d[i] % m);
        int sumd = accumulate(all(dmod), 0LL);
        x %= m;
        n--;
        
        int a_n = x + sumd * (n/k);
        REP(i,n%k) a_n += dmod[i];
        int subst = a_n / m;
        
        int zeros = 0;
        REP(i,k) zeros += dmod[i] == 0;
        zeros *= (n/k);
        REP(i,n%k) zeros += dmod[i] == 0;

        cout << (n - subst - zeros) << endl;
    }
}