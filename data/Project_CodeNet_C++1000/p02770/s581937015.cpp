...
int main()
{
    ...
    REP(q, Q) {
        ...
        REP(i, K) {
            ...
            count_zero[i+1] = count_zero[i] + (d[i] % m == 0);
        }
        ...
        ll sum = (x%m) + cumsum[K] * (n / K) + cumsum[n % K];
        ll zero = count_zero[K] * (n / K) + count_zero[n % K];

        println(n - sum/m - zero);
