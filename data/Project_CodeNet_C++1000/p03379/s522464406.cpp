...
int main() {
    int N;cin >> N;
    vector<P> retu(N);
    for(int i = 0;i<N;i++) cin >> retu.at(i).first,retu.at(i).second = i;
    vector<int> ans(N);
    sort(retu.begin(),retu.end());
    int lesshalf = retu.at(N/2).first;
    int morehalf = retu.at(N/2-1).first;
    for(int i = 0;i<N;i++){
        if(i < N/2){
            ans.at(retu.at(i).second) = lesshalf;
        }
        else{
            ans.at(retu.at(i).second) = morehalf;
        }
    }
