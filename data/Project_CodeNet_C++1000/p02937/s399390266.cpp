...
int main(void) {
    string s, t;
    cin >> s >> t;
    vector<pair<char, LL>> vs(s.size());
    REP(i, s.size()) {
        vs[i] = make_pair(s[i], i + 1);
    }
    sort(ALL(vs));
    // vpprint(vs);
    LL cnt = 0;
    ...
    LL n = s.size();
    REP(i, t.size()) {
        auto it = lower_bound(ALL(vs), make_pair(t[i], prev + 1));
        if(it->first != t[i]) {
            it = lower_bound(ALL(vs), make_pair(t[i], 0LL));
            if(it->first != t[i]) {
                cout << -1 << endl;
                return 0;
            }
