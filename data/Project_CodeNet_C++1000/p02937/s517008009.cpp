...
int main()
{
  ...
  vector<vector<ll>> v(26,vector<ll>(0)); 
  for(int i=0; i<N; i++){ v[s[i]-'a'].emplace_back(i); }
  
  ll ans = 0;
  char lastchar = 'a';
  if(t[0] == 'a'){ lastchar = 'b'; }
  vector<vector<ll>::iterator> lastitr(26);
  vector<ll> lastans(26,0);
  FOR(i,26){ lastitr[i] = v[i].begin(); }
  ...
  for(int i=0; i<M; i++){
    ...
    if(t[i] != lastchar){
      if(ans/N > lastans[index]/N){
        itr = lower_bound(v[index].begin(),v[index].end(),ans%N);  
      }else{
        itr = lower_bound(lastitr[index],v[index].end(),ans%N);
      }
    }else{
      if(ans/N > lastans[index]/N){
        itr = lower_bound(v[index].begin(),v[index].end(),ans%N+1); 
      }else{
        itr = lower_bound(lastitr[index],v[index].end(),ans%N+1); 
      }
    }
    ...
    if(itr != v[index].end()){
      ...
      itr = lower_bound(v[index].begin(),v[index].end(),0);
      ans = (ans/N+1)*N+*itr;
    }
