...
{
    vector<vector<ll>> charl(26);

    for (int i = 0; i < s.size(); i++)
    {
        charl[s[i] - 'a'].push_back(i + 1);
    }
    ll ans = 0;
    ll loop = 0;
    for (int i = 0; i < t.size(); i++)
    {
        if (charl[t[i] - 'a'].size() == 0)
        {
            cout << -1 << endl;
            return;
        }
        auto ite = upper_bound(charl[t[i] - 'a'].begin(), charl[t[i] - 'a'].end(), ans);
        if (ite - charl[t[i] - 'a'].end() == 0 || *ite < ans)
        {
            loop++;
            ans = charl[t[i] - 'a'][0];
        }
