...
int main(int argc, char const *argv[])
{
    ...
    s += s;
    ll len = s.length();
    dp.resize(26, Vector(len, INFTY));

    // dp 設定
    // 一番右側は全部 INFTY
    // 今を見て次を決める。一番右側から2番目スタート
    for(int j = len - 1; j > 0; j--){   // 行
        rep(i, 26){ // 列
            ll c = s[j] - 'a';
            if(i == c){
                dp[c][j - 1] = 1;
            }else if(dp[i][j] != INFTY){
                dp[i][j - 1] = dp[i][j] + 1;
            }
        }
    }
    ...
    // 測定開始
    ll start = 0, s_p = 0, ans = 0;
    if(s[0] == t[0])    start++;
    FOR(i, start, t.length()){
        ll now_tc = t[i] - 'a';
        ll add = dp[now_tc][s_p];
        // cout << s_p << " " << add << endl;
        if(add == INFTY){
            cout << -1 << endl;
            return 0;
