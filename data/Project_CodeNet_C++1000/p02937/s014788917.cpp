...
signed main(int argc, char* argv[]) {
  ...
  string s, t;
  In(s, t);
  vector<vector<int64_t>> ids(26);

  rep(i, s.size()) { ids[s[i] - 'a'].emplace_back(i); }
  ...
  for (auto c : t) {
    auto it = ALL(lower_bound, ids[c - 'a'], i + 1);

    if (it == ids[c - 'a'].end()) {
      cc++;
      it = ALL(lower_bound, ids[c - 'a'], 0);
    }

    i = *it;
  }

  cout << int64_t(s.size()) * cc + i + 1 << endl;
