...
int main(){
    ...
    long long i=0,count=-1,rock1;
    while(i!=(long long)T.size()){
        count++;
        long long key=mp[T.at(i)].at(0);
        //cout<<i<<" "<<key<<endl;
        while(true){
            i++;
            if(i==(long long)T.size()){
                //cout<<i<<" "<<key<<endl;
                rock1=key+1;
                break;
            }
            long long left=0,right=(long long)mp[T.at(i)].size()-1,ans=MOD;
            while(left<=right){
                long long mid=left+(right-left)/2;
                if(mp[T.at(i)].at(mid)>key){
                    ans=min(ans,mid);
                    right=mid-1;
                }
                else{
    ...
    long long answer=count*(long long)S.size()+(long long)rock1;
    cout<<answer<<endl;
}