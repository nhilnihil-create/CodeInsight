...
int main(){
  ...
  for(ll i=0;i<t.size();i++){
    auto itr=upper_bound(ss[t[i]-'a'].begin(),ss[t[i]-'a'].end(),ind);
    if(itr!=ss[t[i]-'a'].end()){
      ind=*itr;
    }else{
      loop++;
      ind=-1;
      itr=upper_bound(ss[t[i]-'a'].begin(),ss[t[i]-'a'].end(),ind);
      if(itr==ss[t[i]-'a'].end()){
        cout<<-1<<endl;
        return 0;
      }
  ...
  cout<<s.size()*loop+lmax(0,ind+1)<<endl;;
}
