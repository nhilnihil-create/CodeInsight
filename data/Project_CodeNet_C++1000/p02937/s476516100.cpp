...
{
	string s, t; cin >> s >> t;
	vvt<ll> sindex(CtoL('z') + 1);
	each(i, e, s) sindex[CtoL(e)].push_back(i);

	each(i, e, t)
	{
		if (Size(sindex[CtoL(e)]) == 0) output(-1);
	}

	vt<pair<ll, ll>> ans; // sindex, sumindex
	ans.push_back({ sindex[CtoL(t[0])].front(), sindex[CtoL(t[0])].front() + 1 });
	rep(i, 1, Size(t))
	{
		auto it = upper_bound(all(sindex[CtoL(t[i])]), ans[i - 1].first);
		if (it == sindex[CtoL(t[i])].end())
			ans.push_back({ sindex[CtoL(t[i])].front(), ans[i - 1].second + Size(s) - ans[i - 1].first + sindex[CtoL(t[i])].front() });
		else
			ans.push_back({ *it, ans[i - 1].second + *it - ans[i - 1].first });
	}
