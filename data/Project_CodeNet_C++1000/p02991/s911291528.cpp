...
long long int d[3][100010];

void dijkstra(int s){
   typedef pair<long long int, P> T; //pair<頂点までの距離, 状態, 頂点>
   ...
   while(!que.empty()){
       ...
       if(state == 2){
           for(int i = 0; i < graph[v].size(); i++){
                edge e = graph[v][i];
                if(d[0][e.to] > dist + e.cost){
                    d[0][e.to] = dist + e.cost;
           ...
           for(int i = 0; i < graph[v].size(); i++){
                edge e = graph[v][i];
                if(d[state+1][e.to] > dist + e.cost){
                    d[state+1][e.to] = dist + e.cost;
                    que.push(T(d[state+1][e.to], P(state+1, e.to)));
                }
           }
       }
   }
