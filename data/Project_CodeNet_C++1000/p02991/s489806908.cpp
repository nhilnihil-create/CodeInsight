...
namespace NyaGadget
{
	...
	struct NyaaBFS
	{
		...
		**/
		void Run(long long start, const std::vector<std::vector<long long>>& g, std::vector<vector<long long>>& res)
		{	// 初期化
			queue<pair<long long, long long>> q;
			q.push({ start, 0 }); // first=v, second=mod
			res[start][0] = 0;

			while (!q.empty())
			{
				pair<ll, ll> from = q.front(); q.pop();
				for (auto&& to : g[from.first])
				{	// 頂点fromから頂点toへの処理
					ll toMod = (from.second + 1) % 3;
					if (res[to][toMod] != LLONG_MAX) continue;
					res[to][toMod] = res[from.first][from.second] + 1;
					q.push({ to,toMod });
				}
			}
		}
...
int main(void)
{
	...
	NyaaBFS bfs;
	vector<vector<long long>> res; MakeVVT(N, 3, res, LLONG_MAX);
	ll S, T; cin >> S >> T; S--, T--;
	bfs.Run(S, lg, res);

	if (res[T][0] == LLONG_MAX) cout << -1;
	else cout << res[T][0] / 3;
	return 0;
}