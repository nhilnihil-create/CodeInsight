...
int main() {
    ...
    vector<vector<PII>> channels(C);
    for (int i = 0; i < N; i++) {
        int s, t, c;
        cin >> s >> t >> c;
        channels[c - 1].push_back(make_pair(s, t));
    }
    for (int i = 0; i < C; i++) sort(channels[i].begin(), channels[i].end());

    vector<PII> new_channels;
    for (int i = 0; i < C; i++) {
        PII p(-1, -1);
        for (int j = 0; j < channels[i].size(); j++) {
            if (j == 0) p = channels[i][j];
            else {
                if (p.second == channels[i][j].first) p.second = channels[i][j].second;
                else {
                    new_channels.push_back(p);
                    p = channels[i][j];
                }
            }
        }
        if (p != make_pair(-1, -1)) new_channels.push_back(p);
    }

    Imos<int> imos(300000);
    for (auto x: new_channels) {
        imos.add(x.first, x.second , 1);
    }
    imos.build();
    int ans = 0;
    for (int i = 0; i < 300000; i++) ans = max(ans, imos.get(i));
    cout << ans << endl;
