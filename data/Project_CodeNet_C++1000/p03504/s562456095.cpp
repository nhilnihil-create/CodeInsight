...
signed main(){cin.tie(0);cout.tie(0);ios::sync_with_stdio(false);cout<<fixed<<setprecision(15);
    ...
    vector<Tiii> ust(n);
    REP(i,0,n) ust[i]=mt(u[i],s[i],t[i]);
    sort(ALL(ust));

    VPii sts;
    REP(i,0,n){
        if(i==n-1) sts.pb(mp(get<1>(ust[i]),get<2>(ust[i])));
        else{
            if(get<0>(ust[i])==get<0>(ust[i+1]) && get<2>(ust[i])==get<1>(ust[i+1])){
                ust[i+1]=mt(get<0>(ust[i]),get<1>(ust[i]),get<2>(ust[i+1]));
            }else{
                sts.pb(mp(get<1>(ust[i]),get<2>(ust[i])));
            }
        }
    }

    // REP(i,0,sts.size()){
    //     cout<<sts[i].first<<" "<<sts[i].second<<endl;
    // }
    ...
    Vi imoss = accum(imos);
    cout<<*max_element(ALL(imoss))<<endl;
    return 0;
}