...
int main(){
    ...
    ll N,C;
    cin >> N >> C;
    vector<ll> from(N),to(N),chanel(N);
    rep(i,N){
        cin >> from[i] >> to[i] >> chanel[i];
        chanel[i]--;
    }
    const ll MAX = 100005;
    vector<vector<ll>> imos(C,vector<ll>(MAX));
    rep(i,N){
        imos[chanel[i]][from[i]]++;
        imos[chanel[i]][to[i]]--;
    }
    rep(i,C)
    rep(j,MAX - 1)imos[i][j + 1] += imos[i][j];
    rep(i,C){
        rep(j,MAX - 1)if(imos[i][j + 1])imos[i][j] = 1;
    }
    ll ans = 0;
    rep(i,MAX){
        ll cnt = 0;
