...
constexpr int inf = (100000)*2 + 1;
std::array<std::array<int, inf>, 31> jikokuhyo{};
...
void solve(void)
{
  ...
  for (int time = 1; time < inf; ++time) {
    for (int index = 0; index < C; ++index) {
      jikokuhyo[index][time] += jikokuhyo[index][time-1];
    }
  }
  vector<int> result(inf, 0);
  for (int t = 0; t < inf; ++t) {
    int res{};
    for (int i = 0; i < C; ++i) {
      if (jikokuhyo[i][t] > 0) {
  ...
  auto ans = boost::max_element(result);
  cout << *ans << CRLF;
  return;
}
