...
int main(){
  ...
  for(ll i=0;i<m;i++){
    ...
    for(ll j=0;j<a[i].size();j++){
      if(seiri.empty()||seiri.back().second<a[i][j].first){
        seiri.push_back(a[i][j]);
      }else{// ==はつなぐ
        seiri.back().second=a[i][j].second;
      }
    }
    for(ll j=0;j<seiri.size();j++){
      imos[seiri[j].first]++;
      imos[seiri[j].second+1]--;
    }
  }
  l=0;
