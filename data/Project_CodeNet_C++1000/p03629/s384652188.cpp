...
void solve() {
  string s; cin >> s;
  int n; n = s.size();
  vector<int> bas(26, n + 1);
  dp = vector<int>(n + 2, -1);
  nxt = vector<vector<int>>(n + 1, vector<int>(26));
  for (int i = n; i >= 0; i--) {
    rep (j, 26) nxt[i][j] = bas[j];
    if (i != 0) bas[s[i-1]-'a'] = i;
  }
  dp[n+1] = 0;
  rep (i, n + 1) memo(i);
  int pos = 0;
  ...
  {
    char a = 'a';
    rep (i, 26) aru.insert(a), ++a;
  }
  while (pos != n + 1) {
    rep (i, 26) {
      if (dp[pos] == dp[nxt[pos][i]] + 1) {
        if (nxt[pos][i] != n + 1) ans += s[nxt[pos][i]-1];
        else {
          FOR (i, pos, n) aru.erase(s[i]);
          ans += *aru.begin();
        }
        pos = nxt[pos][i];
        break;
