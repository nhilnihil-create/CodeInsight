...
// Problem : E - Don't Be a Subsequence
// Contest : AtCoder - AtCoder Regular Contest 081
...
int32_t main()
{
	...
	dp[n]=1; //shortest string not a subsequence of empty string
	
	for(ll i=n-1;i>=0;i--){
		dp[i]=mod;
		f(j,26){
			if(dp[nx[i][j]+1]+1<dp[i]){ par[i]=j,to[i]=nx[i][j]+1;  }
