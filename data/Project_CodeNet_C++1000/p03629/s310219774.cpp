...
int main() {
	...
	IO >> s;
	const auto n = s.size();
	std::array<uint32, 26> a;
	std::vector<uint32> ac(n, 1000000),c(n);
	for (auto &e : a) e = n;
	for (const auto i : irange<>(0, n).rev()) {
		uint32 temp;
		for (const auto j : irange<>(0,26)) {
			if (smini(ac[i], a[j] == n ? 0 : ac[a[j]])) {
				temp = j;
			}
		}
		++ac[i];
	...
	uint32 h = 1000000, index = 0;
	for (const auto i : irange<>(0, 26))
		if (smini(h, a[i] == n ? 0 : ac[a[i]]))
			index = i;
	char next = index + 'a';
	std::string ans;
