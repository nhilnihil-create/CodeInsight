...
void solve(int TEST_CASE){
    ...
    fr(i,3005) dp[i][0]=0, dp[0][i]=0;
    fr(i,ns){
        fr(j,nt){
            if(s[i]==t[j]) dp[i+1][j+1] = max(max(dp[i+1][j], dp[i][j+1]), dp[i][j]+1);
            else dp[i+1][j+1]=max(max(dp[i+1][j], dp[i][j+1]), dp[i][j]);
        }
    }
    dbg{
        fr(i,ns+1){
    ...
    //fr(i,nt) if(dp[ns][i+1]!=dp[ns][i]) cout << t[i];
    stack<char> st;
    int cn=dp[ns][nt], si=ns, ti=nt;
    while(cn){
        dout << "original cn="<<cn << ", si="<<si << ", ti="<<ti<<endl;
        while(dp[si-1][ti]==dp[si][ti]) si--;
        dout << "after si reduction, si="<<si<<endl;
        while(dp[si][ti-1]==dp[si][ti]) ti--;
        dout << "after ti reduction, ti="<<ti<<endl;
        st.push(s[si-1]);
        si--; ti--;
        cn = dp[si][ti];
    }
