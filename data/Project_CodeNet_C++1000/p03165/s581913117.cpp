...
int getans(int n,int m)
{
     ...
     if(s[n-1]==t[m-1])
	{
		 ans=1+getans(n-1,m-1);

	}
	else
	{
		 ans=max(getans(n-1,m),getans(n,m-1));
	}
	return dp[n][m]=ans;
}
string getstring(int n,int m)
{
	...
	if(s[n-1]==t[m-1])
	{
          ans=s[n-1]+getstring(n-1,m-1);
	}
	else
	{
          if(dp[n-1][m]>dp[n][m-1])ans+=getstring(n-1,m);
          else ans+=getstring(n,m-1);
	}
	return ans;
}
main()
{
	...
	getans(len1,len2);
	string ans=getstring(len1,len2);
	reverse(all(ans));
	cout<<ans;
	cout<<endl;
