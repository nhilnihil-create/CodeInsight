...
int main()
{
    ...
    int n, x, y, h, hmax{0}; cin >> n;
    VVI pos_list(3, VI(n));
    bool ok{false};
    rep(i, n)
    {
        cin >> pos_list[0][i] >> pos_list[1][i] >> pos_list[2][i];
    }
    hmax = *max_element(allpt_c(pos_list[2]));

    rep(i, 101) rep(j, 101) rep2(k, hmax, hmax + 203)
    {
        ok = true;
        rep(p, n)
        {
            if (max(k - abs(pos_list[0][p] - i) - abs(pos_list[1][p] - j), 0) != pos_list[2][p])
                ok = false;
        }
        if (ok)
            cout << i << wsp << j << wsp << k << rt;
    }
