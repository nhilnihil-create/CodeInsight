...
int main()
{
    ...
    rep(i, N) {
        cin >> x[i] >> y[i] >> h[i];
        pair<ll,ll> a = make_pair(x[i], y[i]);
        pa.push_back(make_pair(h[i], a));
    }
    ...
    rep(i, 101) {
        rep(j, 101) {
            ll H = pa[N - 1].first + abs(pa[N - 1].second.first - i) + abs(pa[N - 1].second.second - j);
            ll jj = 0;
            rep(k, N) {
                if (max(H - abs(i - x[k]) - abs(j - y[k]), (ll)0) == h[k]) {
                    ++jj;
                }
            }
            if (jj == N) {
