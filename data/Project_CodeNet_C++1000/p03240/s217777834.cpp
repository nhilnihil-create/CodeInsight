...
int main() {
  ...
  REP(i,N) cin >> z.at(i).x >> z.at(i).y >> z.at(i).h;
  
  sort(ALL(z),[](prmd a, prmd b){return a.h > b.h;});
  
  REP(i,101)REP(j,101){
    
    LL H = z.at(0).h + abs(z.at(0).x-i) + abs(z.at(0).y-j);
    
    bool fl = true;
    REP(k,N){
      if(max(H-abs(z.at(k).x-i)-abs(z.at(k).y-j), 0LL) !=  z.at(k).h){
        fl = false;
        break;
      }
    }
