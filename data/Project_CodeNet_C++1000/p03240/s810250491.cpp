...
void solve(){
  int n; cin >> n;
  V< pair< ll, pair<int,int> > >  info(n);
  FOR(i,0,n) cin >> info[i].sd >> info[i].fs;
  sort( RALL(info) );
  ...
  FOR(Cx,0,101){
    FOR(Cy,0,101){
      ...
      if( info[0].fs > 0){
        ll H = ( abs(Cx-info[0].sd.fs)+abs(Cy-info[0].sd.sd) ) + info[0].fs;
        bool ok = true;
        FOR(i,1,n){
          if( info[i].fs != max<ll>(0, H - ( abs(Cx-info[i].sd.fs)+abs(Cy-info[i].sd.sd) ))  ){
            ok = false;
            break;
          }
        }
        if(ok){
          couts(Cx); couts(Cy); couts(H);
          return;
        }
        ...
        ll H = 1'000'000'000;
        FOR(i,0,n){
          if(Cx==info[i].sd.fs && Cy==info[i].sd.sd) {ok = false;break;}
        }
        if(ok){
          couts(Cx); couts(Cy); couts(1);
          return;
        }
      }
