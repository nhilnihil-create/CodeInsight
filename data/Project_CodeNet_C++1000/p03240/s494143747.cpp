...
int main(){
  ...
  for(int X =0;X<=100;X++){
    for(int Y=0;Y<=100;Y++){
      ...
      for(int i=0;i<N;i++){
        if(h[i]==0){
          max_h=min(max_h,abs(x[i]-X)+abs(y[i]-Y));
        }
        else{          
          H.insert(h[i]+abs(x[i]-X)+abs(y[i]-Y));
        }
      }
      if(H.size()==1 &&*begin(H)>=1 &&ok==false){
        cout<<X<<" "<<Y<<" "<<*begin(H)<<endl;
        return 0;
      }
       else if(H.size()==1 &&*begin(H)>=1&&ok&&max_h>=*begin(H)){
        cout<<X<<" "<<Y<<" "<<*begin(H)<<endl;
        return 0;
      }
