...
int main(){
    ...
    ll N;
    cin >> N;
    vector<tuple<ll,ll,ll>> ten(N);
    ll x,y,h;
    rep(i,N){
        cin >> x >> y >> h;
        ten[i] = make_tuple(h,x,y);
    }
    sort(all(ten));
    reverse(all(ten));
    /*rep(i,N){
    ...
    rep(i,102LL){
        rep(j,102LL){
            tie(h,x,y) = ten[0];
            bool ok = true;
            ll hei = h + abs(x - i) + abs(y - j);
            //cout << "x:" << i << ",y:" << j << " height:" << hei << "\n";
            rep(k,N){
                tie(h,x,y) = ten[k];
                ll takasa = hei;
                takasa -= abs(i - x);takasa -= abs(j - y);
                if(max(takasa,0LL) != h){
                    ok = false;
                    break;
                }
