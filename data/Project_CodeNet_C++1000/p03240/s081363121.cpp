...
int main(void) {
    ...
    rep(i, N) cin >> get<0>(points[i]) >> get<1>(points[i]) >> get<2>(points[i]);
    repi(cx, 100) {
        repi(cy, 100) {
            set<ll> s;
            iter(p, points) {
                if(get<2>(p) != 0) {
                    ll temp = get<2>(p) + abs(cx - get<0>(p)) + abs(cy - get<1>(p));
                    if(temp >= 1) s.insert(temp);
                }
            }
            if(s.size() == 1) {
                iter(p, points) {
                    if(max(0LL, *s.begin() - abs(cx - get<0>(p)) - abs(cy - get<1>(p))) != get<2>(p)) goto end;
                }
