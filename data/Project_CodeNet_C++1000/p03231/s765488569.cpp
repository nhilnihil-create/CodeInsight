#include <bits/stdc++.h>
using namespace std;

// using ll = long long;
#define int long long

#define LOG(variable) cout << #variable":\t" << (variable) << endl
#define LOGCON(i, container) for(int (i) = 0; (i) < (container).size(); ++(i)) cout << (i) << ":\t" << (container)[(i)] << endl
#define REP(i, n) for (int i = 0; i < (n); ++i)
#define REPS(i, r, n) for (int i = (r); i < (n); ++i)
#define REPR(i, n) for(int i = (n); i >= 0; --i) // from n to 0
#define REPRS(i, r, n) for(int i = (n); i >= (r); --i) // from n to r
#define REPOBJ(itr, obj) for(auto itr = (obj).begin(); itr != (obj).end() ; ++itr)
#define REPROBJ(itr, obj) for(auto itr = (obj).rbegin(), e = (obj).rend(); itr != e; ++itr)
#define COUTB(x) cout << (x) << "\n"
#define COUTS(x) cout << (x) << " "
#define PB push_back
#define SORT(obj) sort((obj).begin(), (obj).end())
#define SORTR(obj) sort((obj).begin(), (obj).end(), greater<>())
#define ALL(obj) (obj).begin(), (obj).end()
#define MOD 1000000007
#define PI (acos(-1))
int lcm(int a, int b) { return a / __gcd(a,b) * b;}
/***** MAIN *****/
signed main() {
  int N, M;
  string S, T;
  cin >> N >> M;
  cin >> S >> T;
  int L = lcm(N,M);
  string longer_s, shorter_s;
  int longer_l, shorter_l;
  if(N == M) {
    if(S == T) {
      cout << N << endl;
      return 0;
    } else {
      cout << -1 << endl;
      return 0;
    }
  }
  if(N > M) {
    longer_s = S, shorter_s = T;
    longer_l = N, shorter_l = M;
  } else {
    longer_s = T, shorter_s = S;
    longer_l = M, shorter_l = N;
  }
  int i = 0;
  bool possible = true;
  int longer_l_div = L / longer_l;
  int shorter_l_div = L / shorter_l;
  while(longer_l_div*i <= L) {
    if((longer_l_div*i) % shorter_l_div == 0) {
      int shorter_i = longer_l_div*i / shorter_l_div;
      if(longer_s[i] != shorter_s[shorter_i]) {
        possible = false;
        break;
      }
    }
    ++i;
  }
  if(possible) {
    cout << L;
  } else {
    cout << -1;
  }
  cout << "\n";
  return 0;
}
/***** MAIN *****/
