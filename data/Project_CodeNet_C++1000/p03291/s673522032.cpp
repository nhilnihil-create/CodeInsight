...
ll n;
ll a[SZ], c[SZ], qp[SZ], qs[SZ], pw3[SZ];

int main() {
    ll cs = 0, tc = 1, x, y, z, i, j, k, g, p, q, ans = 0, sum = 0, t = 0;
//    ll a, b, d;
    string s, s1, s2;

    pw3[0] = 1;
    for1(i, SZ-2) pw3[i] = (pw3[i-1] * 3) % mod;
    ...
    for1(i, n) {
        if(s[i-1] == 'A' || s[i-1] == 'C') continue;

        t = qp[i-1] + qs[i+1];

        x = (a[i-1] * c[i+1]) % mod;
        ans = (ans + (x * pw3[t])%mod) % mod;

//        if(t-1 >= 0) {
            x = (a[i-1] * qs[i+1]) % mod;
            ans = (ans + (x * pw3[t-1])%mod) % mod;
