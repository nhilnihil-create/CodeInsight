...
struct initializer {
	initializer() {
		std::cout << fixed << setprecision(11);
	}
};
initializer _____;
ll gcd(ll a, ll b) {
...
signed main() {
	...
	int R = Mpow(3, Q, MOD);
	int ans = 0;
	vector< int > accA(N + 1, 0), accC(N + 1, 0), accQ(N + 1, 0);
	rep(i, N) {
		accA[i + 1] = accA[i] + (S[i] == 'A' ? 1 : 0);
		accC[i + 1] = accC[i] + (S[i] == 'C' ? 1 : 0);
		accQ[i + 1] = accQ[i] + (S[i] == '?' ? 1 : 0);
	}
	for (int i = 1; i < N - 1; i++) {
		if (S[i] == 'B' || S[i] == '?') {
			int a = accA[i] * Mpow(3, accQ[i], MOD) % MOD;
			a += accQ[i] * Mpow(3, accQ[i] - 1, MOD) % MOD;
			a %= MOD;
			int c = (accC[N] - accC[i + 1]) * Mpow(3, (accQ[N] - accQ[i + 1]), MOD) % MOD;
			c += (accQ[N] - accQ[i + 1]) * Mpow(3, (accQ[N] - accQ[i + 1]) - 1, MOD) % MOD;
			c %= MOD;
			ans = ((ans + a * c % MOD) % MOD);
			//std::cout << a << " " << c << " " << ans << endl;
		}
	}
	//std::cout << accA << accC << accQ;
	std::cout << ans << endl;
	return 0;
}