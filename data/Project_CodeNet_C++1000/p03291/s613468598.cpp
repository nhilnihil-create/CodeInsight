...
void solve()
{
    ...
    cin>>s;
    int n=s.size();
    vector<vector<vector<int>>> dp(n, vector<vector<int>>(3,vector<int>(4)));
    ...
    int ans = 0;
    for(int i= 0;i< min((int)4, vacant+1);i++)
    {
        ans = (ans + power(3, vacant - i, mod)*(cnt[2][i]%mod))%mod;
    }
    cout<<ans<<endl;
}
