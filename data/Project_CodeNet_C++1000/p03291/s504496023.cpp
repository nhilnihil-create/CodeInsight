...
void solve(){
  ...
  ll ans = 0;
  V<ll> num1(4), num2(4);
  for(auto &x:s){ if(x=='?') num2[3]++; else num2[x-'A']++;}
  FOR(i,0,n){
    if(s[i]=='?') num2[3]--; else num2[s[i]-'A']--;
    //couts(num1); coutn(num2);
    if(s[i]=='B' || s[i]=='?'){
      (ans += ((num1[0]*modpow(3,num1[3])+num1[3]*modpow(3,num1[3]-1) )%mod  ) * ( (num2[2]*modpow(3,num2[3])+num2[3]*modpow(3,num2[3]-1)) % mod )%mod    ) %= mod;
    }
    if(s[i]=='?') num1[3]++; else num1[s[i]-'A']++;
  }
  couts( ans );
}
