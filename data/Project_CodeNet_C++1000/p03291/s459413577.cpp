...
vec f(string s)
{
  ...
  REP(i, n)
  {
    ...
    if (c == '?')
    {
      dpNew[0] = mod_add(mod_mul(dp[0], 3LL), qp(3LL, cnt));
      dpNew[1] = mod_add(mod_mul(dp[1], 3LL), dp[0]);
      dpNew[2] = mod_add(mod_mul(dp[2], 3LL), dp[1]);
      cnt++;
    }
    else if (c == 'A')
    {

      dpNew[0] = mod_add(dp[0], qp(3LL, cnt));
      dpNew[1] = dp[1];
      dpNew[2] = dp[2];
    }
    else if (c == 'B')
