...
void solve() {
    ...
    fr(i, 0, n - 1) {
        pre[i + 1][0] = pre[i][0] + (s[i] == 'A');
        pre[i + 1][1] = pre[i][1] + (s[i] == 'C');
        pre[i + 1][2] = pre[i][2] + (s[i] == '?');
    }
    ll ans = 0;
    fr(i, 1, n) {
        if(s[i - 1] == 'B' || s[i - 1] == '?') {
            for(int j : {0, 2}) {
                for(int k : {1, 2}) {
                    int c1 = pre[i - 1][j], c2 = pre[n][k] - pre[i][k];
                    if(!c1 || !c2) continue;
                    int rem = pre[n][2] - (s[i - 1] == '?') - (j == 2) - (k == 2);
                    int ways = (pw[rem] * 1LL * c1) % mod;
                    ways = (ways * 1LL * c2) % mod;
                    ans = (ans + ways) % mod;
                }
            }
        }
