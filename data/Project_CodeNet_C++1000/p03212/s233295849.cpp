...
signed main() {
	...
	queue<int> que;
	rep(i, 3) que.push(d.at(i));

	int res = 0;
	bool flag = false;
	while (true) {
		int tempo = que.front(); que.pop();
		//cout << "tempo: " << tempo << endl;
		deque<signed> v;
		ll_to_vector(10, tempo, v);
		
		rep(i, 3) {
			int tempo2 = tempo*10 + d.at(i);
			...
			deque<signed> v2;
			ll_to_vector(10, tempo2, v2);
			vector<bool> judge(3, false);
			rep(j, v2.size()) {
				rep(k, 3) {
					if (v2.at(j) == d.at(k)) judge.at(k) = true;
				}
			}

			if (judge.at(0) && judge.at(1) && judge.at(2)) {
				res++;
				//cout << tempo2 << endl;
			}
