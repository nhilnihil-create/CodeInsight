...
bool check(string s, int N){
	...
	vector<int> n(10,0);
    for(char c : s) n[c-'0']++;// あれここって計算量O(N)で10^9だけどACしたわ
	if(stoi(s) > N) return false;
    return n[3] && n[5] && n[7]; //https://atcoder.jp/contests/abc114/submissions/10378605
	
	// bool is_3 = false, is_5 = false, is_7 = false;
	// for(char c : s){// あれここって計算量O(N)で10^9だけどACしたわ
	// 	if(c == '3') is_3 = true;
	// 	else if(c == '5') is_5 = true;
	// 	else if(c == '7') is_7 = true;
	// }
	// if(stoi(s) > N) return false;
	// return is_3 && is_5 && is_7;
}

void dfs(string s, bool is_zero, int size, int N){
	if(size == (int)s.size()){
		if(check(s, N)) ans++;
	...
	s.push_back('3');
	dfs(s, false, size, N);
	s.pop_back(); s.push_back('5');
	dfs(s, false, size, N);
	s.pop_back(); s.push_back('7');
	dfs(s, false, size, N);
	if(is_zero){
