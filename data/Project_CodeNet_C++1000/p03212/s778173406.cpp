...
signed main() {
  ...
  ll tens = 1000;
  while(tens<=10000000000){
    ll tmp;
    ll size = vals.size();
    for(int i=0;i<size;i++){
      ll v = vals[i];
      for(ll i=1;i<=tens;i*=10){
        tmp = (v/i)*(i*10) + 3*i + v%i; vals.push_back(tmp);
        tmp = (v/i)*(i*10) + 5*i + v%i; vals.push_back(tmp);
        tmp = (v/i)*(i*10) + 7*i + v%i; vals.push_back(tmp);
        if(v/i==0)break;
      }
    }
    tens *= 10;
