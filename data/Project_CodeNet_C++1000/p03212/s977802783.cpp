...
void dfs(ll x,ll maxx, int* count)
{
	if (x > maxx)return;
	...
	dfs(x * 10 + 3,maxx, count);
	dfs(x * 10 + 5,maxx, count);
	dfs(x * 10 + 7,maxx, count);
}

int main()
{
	//can be dfs'ed because somehting much smaller than 3^9 + 3^8... etc will be all u get
