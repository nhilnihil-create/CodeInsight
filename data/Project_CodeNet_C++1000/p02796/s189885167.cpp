...
int main()
{
    ...
    std::vector<std::pair<long long, long long>> left_right(n);
    for (int i = 0; i < n; i++) {
        long long x, l;
        std::cin >> x >> l;

        left_right[i].first = x - l;
        left_right[i].second = x + l;
    }

    std::sort(left_right.begin(), left_right.end(), std::greater<std::pair<long long, long long>>());

    long long current_left = left_right[0].first;
    long long count = 1;
    for (int i = 1; i < n; i++) {
        if (left_right[i].second <= current_left) {
            count++;
            current_left = left_right[i].first;
        }
    }
