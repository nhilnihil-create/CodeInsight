#include <bits/stdc++.h>
#define vsort(v) sort(v.begin(), v.end())
#define rvsort(v) sort(v.begin(), v.end(), greater<ll>())
#define vfind(v, target) find(v.begin(), v.end(), target)
#define notFound(v, target) (vfind(v, target) == v.end())
#define maxIndex(v) max_element(v.begin(), v.end()) - v.begin()
#define maxElement(v) *max_element(v.begin(), v.end())
#define minIndex(v) min_element(v.begin(), v.end()) - v.begin()
#define minElement(v) *min_element(v.begin(), v.end())
#define ll long long
#define rep(i, s, g) for (ll i = (s); i < (g); i++)
#define MOD 1000000007
using namespace std;

int main()
{
  ll sx, sy, tx, ty;
  cin >> sx >> sy >> tx >> ty;
  ll dis_x = tx - sx;
  ll dis_y = ty - sy;
  string route = "";
  rep(i, 0, dis_y) route.push_back('U');
  rep(i, 0, dis_x) route.push_back('R');
  rep(i, 0, dis_y) route.push_back('D');
  rep(i, 0, dis_x + 1) route.push_back('L');
  rep(i, 0, dis_y + 1) route.push_back('U');
  rep(i, 0, dis_x + 1) route.push_back('R');
  route += "DR";
  rep(i, 0, dis_y + 1) route.push_back('D');
  rep(i, 0, dis_x + 1) route.push_back('L');
  route += "U";
  cout << route << endl;
}