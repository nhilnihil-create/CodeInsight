...
int main(){
 ...
 // dp[i][j] represents the number of ways to match i men with the set of women indicated by the mask j
 dp[0][0]=1;  // As there is only one way to select from 0 men and 0 women - select none
 for(int i=0;i<n;i++){
  for(auto mask=0;mask<(1<<n);mask++){ // Iterates over all possible sets of women
   if(i==__builtin_popcountll(mask)){  // We can't proceed unless this condition holds as the no. of men matched must equal no. of women matched
    for(int j=0;j<n;j++){  // j-> denotes j-th woman in the set denoted by mask
     if(match[i][j] && !(mask & (1<<j))){  //It is necessary that !(mask & (1<<j)) holds, as (mask & (1<<j))==1 means that the j-th woman in the set has alredy been paired, so can't be matched again
      dp[i+1][mask|(1<<j)]=(dp[i+1][mask|(1<<j)]%MOD+dp[i][mask]%MOD)%MOD;  
     } // Above line denotes that we selected the jth woman in the set for the (i+1)th man, and counts the total no. of ways of doing this.
    }
   }
  }
 }
 cout<<dp[n][(1<<n)-1]<<"\n"; // As dp[i][j] denotes no. of ways to match i men with the set of women represented by j, thus, the required
                              // answer must be dp[n][(1<<n)-1] -> no. of ways of matching n men with the set representing all the women.
