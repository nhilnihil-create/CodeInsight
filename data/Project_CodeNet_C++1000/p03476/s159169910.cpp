...
int main() {
    ...
    // like_number かチェック
    vector<bool>  is_like_number(LIMIT + 1, false);
    for(ll i = 2; i <= LIMIT; i++){
      if(is_prime[i] && is_prime[(i + 1) / 2]){
        is_like_number[i] = true;
      }
    }

    // like_numberの累積和を作る
    vector<ll>  cum_sum(LIMIT + 2,0);
    for(ll i = 0; i <= LIMIT; i++){
      cum_sum[i + 1] = cum_sum[i] + (is_like_number[i + 1] ? 1 : 0);
    }
    ...
    rep(i,q){
      ll l,r;
      cin >> l >> r;
      ll ans = cum_sum[r + 1] - cum_sum[l - 1];
      cout << ans << endl;
    }
