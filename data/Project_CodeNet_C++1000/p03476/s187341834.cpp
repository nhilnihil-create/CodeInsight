...
void solve(){
  ...
  int amax = 100'030;
  {
    FOR(i,2,amax){
      ...
      if(ifprime) primes.eb(i);
    }
    for(auto x: primes){
      auto it = lb(ALL(primes), (x+1)/2 );
      if(it != primes.end() && *it== (x+1)/2 ) nums.eb(x);
    }
  }
  
  FOR(_,0,q){
    int l, r; cin >> l >> r;
    auto it1 = lb(ALL(nums), l);
    auto it2 = lb(ALL(nums), r+1);
    coutn( (nums.end()-it1) - (nums.end()-it2) );
  }
}
