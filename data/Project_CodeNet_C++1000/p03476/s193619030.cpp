...
vector<long long> prime_all(long long n) {
...
void solve() {
    ...
    static int like2017[_max+10];
    static int imos[_max+10];
    vec prime = prime_all(_max);
    sort(all(prime));
    for(auto v: prime){
        ll tmp = (v+1)/2;
        auto it = lower_bound(all(prime),tmp);
        if(tmp==*it)imos[v]++;
    }
    rep1(i,_max+1){
        like2017[i] = like2017[i-1]+ imos[i];
    }

    int Q; cin >> Q;
    rep(i,Q){
        int l,r; cin >> l >> r;
        int res = like2017[r] - like2017[l-1];
        cout << res << endl;
    }
