...
int main() {
    ...
    vector<bool> primes = Eratosthenes(100000);

    vector<ll> cum_primes(100001, 0);
    ll temp = 0;
    for(ll i = 0; i < cum_primes.size(); i++) {
        // cout << i << ":" << primes[i] <<  primes[(i+1)/2] << endl;
        cum_primes[i] = primes[i] && primes[(i+1)/2] ? ++temp : temp;
    }

    // for(ll i = 0; i < 100; i++) {
    //     cout << i << ":" << cum_primes[i] << endl;
    // }
    ...
    for(ll i = 0; i < Q; i++) {
        ...
        cout << cum_primes[r] - cum_primes[l-1] << endl;
    }
