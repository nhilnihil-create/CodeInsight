...
/*-----8<-----library-----8<-----*/
vector<ll> S;
void dfs(vector<set<ll>> &g, ll v, ll p, vector<bool> &seen, vector<vector<ll>> &ab ){
	for(ll nextv:g[v]){
		ab[v][S[nextv]]++;
	}
	seen[v] = true;
	for(ll nextv : g[v]){
		...
		dfs(g, nextv, v, seen, ab);
	}
	
}

bool ok0=false,ok1=false;
void dfs2(vector<set<ll>> &g, ll v, ll p, vector<bool> &seen,vector<bool> &valid) {
	if(S[v]==0)ok0=true;
	else ok1=true;
	seen[v] = true;
	for(ll nextv : g[v]){
		if(seen[nextv]||!valid[nextv]) continue;
		dfs2(g, nextv, v, seen,valid);
	}
...
signed main() {
	...
	vector<vector<ll>> ab(N,vector<ll>(2,0));
	...
	while(!q.empty()){
		...
		q.pop();
		if(!valid[i])continue;
		if(ab[i][0]>0&&ab[i][1]>0)continue;
		valid[i]=false;
		for(ll nextv : g[i]){
			if(!valid[nextv])continue;
			ab[nextv][S[i]]--;
			q.push(nextv);
		}
	}
