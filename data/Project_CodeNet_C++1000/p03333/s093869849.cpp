...
bool solve() {
    int n; cin >> n;
    vector<int> l(n), r(n);
    rep (i, n) cin >> l[i] >> r[i], r[i] = -r[i];
    sort(rall(l)), sort(rall(r));
    vector<int> suml(n + 1), sumr(n + 1);
    rep (i, n) suml[i+1] = suml[i] + l[i], sumr[i+1] = sumr[i] + r[i];
    int ans = 0;
    rep (i, n + 1) {
        if (i != 0) chmax(ans, suml[i] + sumr[i-1]);
        chmax(ans, suml[i] + sumr[i]);
        if (i != n) chmax(ans, suml[i] + sumr[i+1]);
    }
    cout << ans * 2 << endl;
}
signed main() {
