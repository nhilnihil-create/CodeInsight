...
vector<vector<ll>> g;
pair<ll,bool> isOK(){
	for(ll nextv:g[path[path.size()-1]]){
		if(s.count(nextv)==0)return {nextv,false};
	}
	for(ll nextv:g[path[0]]){
		if(s.count(nextv)==0)return {nextv,true};
	}
	return {-1,false};
}
...
void solve() {
	...
	while(1){
		...
		path.push_back(nextv.first);
		s.insert(nextv.first);
		debug(path);
	}
}
