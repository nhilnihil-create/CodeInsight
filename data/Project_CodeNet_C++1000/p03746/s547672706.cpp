...
int main() {
	...
	while (true) {
		ll newleft = -1;
		for (i = 0; i < path[left].size(); i++) {
			if (!flag[path[left][i]]) {
				L.push_back(path[left][i]);
				flag[path[left][i]] = 1;
				newleft = path[left][i];
				break;
			}
	...
	while (true) {
		ll newright = -1;
		for (i = 0; i < path[right].size(); i++) {
			if (!flag[path[right][i]]) {
				R.push_back(path[right][i]);
				flag[path[right][i]] = 1;
				newright = path[right][i];
				break;
			}
		}
		if (newright == -1) break;
