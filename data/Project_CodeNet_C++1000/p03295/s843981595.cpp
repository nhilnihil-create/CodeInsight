...
int main() {
    ...
    auto comPair = [](P p1, P p2) {
        int f1 = p1.first, f2 = p1.second;
        int t1 = p2.first, t2 = p2.second;
        if (f1 == t1)
            return f2 > t2;
        else
            return f1 < t1;
    ...
    sort(all(bridge), comPair);
    int lmax = 0;
    int rmin = INF;
    int ans = 0;
    rep(i, m) {
        chmax(lmax, bridge[i].first);
        chmin(rmin, bridge[i].second);
        if (rmin < lmax + 1) {
            ans++;
            lmax = bridge[i].first, rmin = bridge[i].second;
            i--;
        } else if (i == m - 1) {
            ans++;
        }
