...
int main(){
  ...
  rep(i, M){
    ...
    conflicts.p_b(m_p(l, r));
  }

  // 各争いを、左側の島をキーとして昇順ソート
  sort(begin_end(conflicts));
  ...
  rep(i, M){
    ll l = conflicts[i].first;
    ll r = conflicts[i].second;

    if (!rs.empty() && l >= rs.top()){
      // これより前で切らないと駄目
      ans++;
      // 空にする
      rs =  priority_queue<ll, vector<ll>, greater<ll> >();
    }
