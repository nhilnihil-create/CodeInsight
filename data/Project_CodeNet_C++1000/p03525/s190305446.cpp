...
signed main() {
  ...
  rep(i,0,n){
    cin >> qw;
    if(qw == 0) {cout << 0 << endl;return 0;}
    mono[qw]++;
  }
  
  //Verr(mono);
  
  if(mono[0] >= 2 || mono[12] >= 2) {cout << 0 << endl;return 0;}
  
  rep(i,0,13) {
    if(mono[i] >= 3) {cout << 0 << endl;return 0;}
  }
  ...
  rep(i,0,13){
    ...
    if(i==12) {
      ...
      if(mono[i]>=2) {cout << 0 << endl;return 0;}
      break;
    }
    
    else if(mono[i]==1) one.pb(i);
    else if(mono[i]==2) two.pb(i);
  }
  ...
  rep(i,0,ct){
    ...
    rep(j,0,qw){
      if(ch[j]==1) nowtab[one[j]]++;
      else nowtab[24-one[j]]++;
    }
    
    rep(j,0,25){
      if(nowtab[j]==1) {
        nowmin = min(nowmin,nowct);
        nowct = 1;
      }
      else nowct++;
    }
