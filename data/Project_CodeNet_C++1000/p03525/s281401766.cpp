   ...
   // cout<<bits<<endl;
    ...
    for(int i=0;i<13;i++){
      for(int j=i+1;j<13;j++){
        if(!(d_num[i]+ d_num[j] >=2 && d_num[i]>0 && d_num[j]>0))continue;
        int num1,num2;
        if(bits[i]==0){
          num1=-i;
        }else{
        ...
        int nowmin=min(abs((num1-num2+24)%24),24-abs((num1-num2+24)%24));
        tmpans=min(tmpans,nowmin);
        if(d_num[i]>=3 ||d_num[j]>=3){
          tmpans=0;
        }
        if(d_num[i]==2){
    ...
    for(int i=0;i<13;i++){
      if(d_num[i]==2){
        tmpans=min(tmpans,min(i*2,24-(i*2)));
      }
    }
    if(tmpans!=intMAX){
      ans=max(ans,tmpans);
    }
    ull now=bits.to_ullong();
    now++;
    bits=now;
  }
  cout<<ans<<endl;
