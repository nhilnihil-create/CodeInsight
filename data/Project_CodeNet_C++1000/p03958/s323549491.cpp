...
signed main() {
	...
	rep(i, T) memo.push(make_pair(a.at(i), i));
	...
	while (!memo.empty()) {
		pair<int, int> temp_pair = memo.top(); memo.pop();
		
		if (last_cake != temp_pair.second) {
			temp_pair.first--;
			last_cake = temp_pair.second;
			if (temp_pair.first > 0) memo.push(temp_pair);
			//eaten.push_back(temp_pair.second);
		}
		else {
			if (!memo.empty()) {
				pair<int, int> temp_pair2 = memo.top(); memo.pop();
				temp_pair2.first--;
				last_cake = temp_pair2.second;
				if (temp_pair2.first > 0) memo.push(temp_pair2);
				memo.push(temp_pair);
				//eaten.push_back(temp_pair2.second);
			}
			else {
				res += temp_pair.first;
				//rep(i, temp_pair.first) eaten.push_back(temp_pair.second);
			}
	...
	cout << endl;
	//rep(i, eaten.size()) cout << eaten.at(i) << endl;
}