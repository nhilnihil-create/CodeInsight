...
// in(vec) works for size-defined type vectors
	...
	while (t--) {
		...
		dp[1] = n;
		forn(i, 2, min(1LL * 200005, n) + 1) {
			ll multiple = (n - i + mod) % mod;
			multiple = (multiple + 1) % mod;
			dp[i] = ((dp[i - 1] * multiple) % mod);
		...
		sum = (sum + 1) % mod;
		//o(sum)
		ll ans = (power(2, n, mod) - sum + mod) % mod;
		o(ans)
...
//Credits: Reference of Fermat's little theroem mentioned in the editorial 