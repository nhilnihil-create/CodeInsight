...
/*
	...
	■出力
	あかりさんが作ることのできる花束の種類数を (1e9+7) で割った余りを出力せよ。(そのような花束がないときは 0 と出力せよ。)
...
int main()
{
	...
	int ans = powmod(2, n) - 1 - nCr(n, a) - nCr(n, b);
	ans %= MOD;

	(ans < 0) ? cout << (ans + MOD) << endl : cout << (ans) % MOD << endl;
