...
int main(){
  long long n,a,b;
  cin >> n >> a >> b;
  long long ans = ruizyou(2,n) - 1;
  long long bunsi = n-a+1;
  for(long long i=n-a+2;i<=n;i++)bunsi = bunsi * i % MOD;
  long long bunbo = kaizyou(a);
  ans -= bunsi * ruizyou(bunbo,MOD-2) % MOD;
  bunsi = n-b+1;
  for(long long i=n-b+2;i<=n;i++)bunsi = bunsi * i % MOD;
  bunbo = kaizyou(b);
  ans -= bunsi * ruizyou(bunbo,MOD-2) % MOD;
  ans %= MOD;
  if(ans<0)ans+=MOD;
