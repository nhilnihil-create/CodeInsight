...
{
	ll N; cin >> N;
	vt<ll> town(N); each(i, e, town) e = i;
	vt<pll> grid(N); each(i, e, grid) cin >> e.first >> e.second;

	ld ans = 0;
	do
	{
		rep(i, 1, N) ans += hypot(grid[town[i]].first - grid[town[i - 1]].first, grid[town[i]].second - grid[town[i - 1]].second);
	} while (next_permutation(all(town)));
	coutld << ans / (ld)NT_Counting<ll>::P(N, N);
	return 0;
}
