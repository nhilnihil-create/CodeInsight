...
int main(){
  ...
  rep(i,n){
    rep(j,n){
      ...
      sq+=pow(vec.at(i).at(0),2);
      sq+=pow(vec.at(j).at(0),2);
      sq+=pow(vec.at(i).at(1),2);
      sq+=pow(vec.at(j).at(1),2);
      sq-=2*vec.at(i).at(0)*vec.at(j).at(0);
      sq-=2*vec.at(i).at(1)*vec.at(j).at(1);
      //cout<<(double)(sq)<<endl;
      d+=sqrt((double)(sq));
    }
  }
