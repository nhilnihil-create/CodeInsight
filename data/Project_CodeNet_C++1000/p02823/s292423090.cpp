...
int main() {
    ...
    long long left = max(A-1,B-1);
    long long right = max(N-A,N-B);
    long long L = min(A-1,B-1);
    long long R = min(N-A,N-B);
    //Xは二人とも端っこ行く時の最小試合数
    long long X = min(left,right);
    //Yは片方端っこについてから差が偶数になって(1手待つ)から二人が逆に動く
    long long sa = B-A;
    long long LR = min(L,R);
    long long Y = (sa-1)/2+LR + 1;
