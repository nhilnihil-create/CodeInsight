...
void solve() {
    string s, t; cin >> s >> t;
    int q; cin >> q;
    vector<pair<int, int>> sum1(s.size() + 1);
    vector<pair<int, int>> sum2(t.size() + 1);
    rep (i, s.size()) {
        sum1[i + 1] = sum1[i];
        if (s[i] == 'A') {
            sum1[i + 1].first++;
    ...
    rep (i, q) {
        ...
        int num1 = (sum1[b].first - sum1[a - 1].first) + (sum1[b].second - sum1[a - 1].second) * 2;
        int num2 = (sum2[d].first - sum2[c - 1].first) + (sum2[d].second - sum2[c - 1].second) * 2;
        YESNO(num1 % 3 == num2 % 3);
    }
    return;
}
