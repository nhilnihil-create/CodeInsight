...
signed main(){cin.tie(0);cout.tie(0);ios::sync_with_stdio(false);cout<<fixed<<setprecision(15);
    ...
    Vi ssum(sn+1),tsum(tn+1);
    REP(i,0,sn){
        if(s[i]=='A') ssum[i+1]=ssum[i]+1;
        if(s[i]=='B') ssum[i+1]=ssum[i]+2;
    }
    REP(i,0,tn){
        if(t[i]=='A') tsum[i+1]=tsum[i]+1;
        if(t[i]=='B') tsum[i+1]=tsum[i]+2;
    }

    REP(i,0,q){
        int sq=ssum[b[i]]-ssum[a[i]-1];
        int tq=tsum[d[i]]-tsum[c[i]-1];
        if(sq%3==tq%3) cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
    }
    return 0;
}