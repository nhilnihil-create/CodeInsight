...
signed main() {
	...
	vector<ll> ruiS(S.size()+1,0);
	rep(i,S.size())ruiS[i+1]=ruiS[i] + (S[i]=='A' ? 1 : 2);
	vector<ll> ruiT(T.size()+1,0);
	rep(i,T.size())ruiT[i+1]=ruiT[i] + (T[i]=='A' ? 1 : 2);
	...
	while(N--){
		...
		ll t=ruiT[d]-ruiT[c];
		debug(s,t);
		if(s%3==t%3)ans.push_back(true);
		else ans.push_back(false);
	}

	rep(i,ans.size()) if(ans[i])p("YES"); else p("NO");
	return 0;
}
