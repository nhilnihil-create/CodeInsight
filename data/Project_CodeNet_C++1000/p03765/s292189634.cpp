...
int main() {
	...
	string S; cin >> S;
	string T; cin >> T;
	Seg<ll> seg1; seg1.init(S.size());
	Seg<ll> seg2; seg2.init(S.size());

	F0R(i, S.size()) seg1.update(i, 2 - S[i] + 'A');
	F0R(i, S.size()) seg2.update(i, 2 - T[i] + 'A');

	int Q; cin >> Q;
	F0R(i, Q) {
		int s1, s2, t1, t2; cin >> s1 >> s2 >> t1 >> t2;
		--s1; --s2; --t1; --t2;
		if (abs(seg1.query(s1, s2) - seg2.query(t1, t2))  % 3 == 0) {
			print("YES");
		} else {
			print("NO");
		}
