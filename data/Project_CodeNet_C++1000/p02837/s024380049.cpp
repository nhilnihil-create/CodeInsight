...
int main()
{
    ...
    std::vector<int> a(n);
    std::vector<std::vector<std::pair<int, int>>> x_y(n);
    for (int i = 0; i < n; i++) {
        std::cin >> a[i];

        std::vector<std::pair<int, int>> x_y_temp(a[i]);
        for (int j = 0; j < a[i]; j++) {
            std::cin >> x_y_temp[j].first >> x_y_temp[j].second;
            x_y_temp[j].first--;
        }
    ...
    for (unsigned i = 0; i < (1U << n); i++) {
        for (int j = 0; j < n; j++) {
            ...
            for (int k = 0; k < a[j]; k++) {
                if (((i >> x_y[j][k].first) & 1U) != (unsigned)x_y[j][k].second) {
                    goto err;
                }
            }
        }
