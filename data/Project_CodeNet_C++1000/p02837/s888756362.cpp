...
signed main(){
    ...
    cin >> n;
    intl a[n];
    vector<vector<pair<intl, intl> > > v(n);
    rep(i,n){
        cin >> a[i];
        v[i].resize(a[i]);
        rep(j,a[i]){
    ...
    for(intl i = 0; i < (1 << n); i++){
        ...
        rep(j,n){
            ...
            //debug(j);
            rep(k,a[j]){
                if(!!(i & (1 << (v[j][k].first - 1))) != v[j][k].second){
                    //cout << bitset<4>(i) << " "<< j << " " << k <<endl;
                    //cout << v[j][k].first - 1 << ":" << v[j][k].second << endl;
                    //cout << bitset<3>(i) << " " << bitset<3>(1 << (v[j][k].first - 1)) << " " << v[j][k].second << " " << !!(i & (1 << (v[j][k].first - 1))) <<  endl;
                    f = false;
                }
            }
        }
        intl cnt = __builtin_popcount(i);
        if(f)mi = max(mi,cnt);
        //cout << bitset<8>(i) << ":" << f << endl;
    }
    cout << mi << endl;
    return 0;
}
