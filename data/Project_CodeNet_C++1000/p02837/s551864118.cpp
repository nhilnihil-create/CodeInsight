...
int main() {
    ...
    vector<map<long long, bool>>  testimony(n);
    ...
    // bit全探索する
    for(ll i = 0; i < (1 << n); i++){
      bool isNoContradiction = true;
      for(ll bit = 0; bit < n; bit++){
        if(i & (1 << bit)){

          // bit 人目が正直者の場合、
          for(auto it : testimony[bit]){
            ll No = it.first;

            bool isHonest = (i & (1 << No));
            if( !( isHonest == it.second /* = 証言と全探索の決め打ちが合致しているか？ */))
            {
              isNoContradiction = false;
            }
          }
        }
        if(!isNoContradiction) break;
      }
