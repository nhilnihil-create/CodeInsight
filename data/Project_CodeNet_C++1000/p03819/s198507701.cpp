...
struct lcatree {
	...
	int lca(int u, int v) {
		if(h[u] < h[v]) swap(u, v);
		for(int i = 0; i < lg; i++) if(((h[u]-h[v])>>i)&1) u = p[u][i];
		if(u == v) return u;
		for(int i = lg; i--;) if(p[u][i]!=p[v][i]) u = p[u][i], v = p[v][i];
		return p[u][0];
	}
...
ll get(int l, int r, int ql, int qr, pnode rt) {
