...
void solve() {
	...
	for (ll left = 1; left <= M; ++left) {
		while (right < N && (a[right].second-a[right].first<left)) {
			// 実際に right を 集合に含める
			// ex: sum += a[right];
			bit.add(a[right].first, a[right].second, 1);
			++right;
		}
	
		// break した状態で right / [left, right) は条件を満たす最大なので、何かする
		// ex: res += (right - left);
		ll res=N-right;
		for(ll i=left; i<=M; i+=left){
			ll t=bit.sum(i,i+1);
			res+=t;
		}
