...
vector<pi> in; vector<pair<int, pi>> seg; vector<int> ret;
int main() {
	setIO();
    int N, M; cin >> N >> M; int mxInd = 0;
    fax(i,N){
      int l,r; cin >> l >> r;
      in.pb({l,r}); seg.pb({r-l+1, {l,r}});
      ckmax(mxInd, r);
    }
    trav(curr, in){
      if(curr.s == mxInd){
    ...
    int p = sz(seg)-1;
    sort(all(seg)); int ans = 0;
    f0xd(i,1,M+1){
    	// cout << i << " " << seg[p].f << endl;
      while(seg[p].f >= i){
      	// cout << seg[p].f << " " << i << " " << p << endl;
      	++ans;
        update(seg[p].s.f, -1); 
        if(seg[p].s.s != mxInd){
          update(seg[p].s.s+1, 1);
        }
        p--;
        if(p < 0){
          break;
