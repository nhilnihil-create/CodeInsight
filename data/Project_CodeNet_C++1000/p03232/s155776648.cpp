...
int main() {
    ...
    for(auto& i : A)cin >> i;
    partial_sum(weight.begin(), weight.end(), weight.begin(), [&MOD](auto a, auto b){return (a + b) % MOD;});
    unsigned long ans{0};
    for(unsigned long i = 0; i < N; ++i)ans += A[i] * (weight[i] + weight[N - i - 1] + MOD - weight[0]) % MOD;
    cout << ans % MOD << endl;
    return 0;
}