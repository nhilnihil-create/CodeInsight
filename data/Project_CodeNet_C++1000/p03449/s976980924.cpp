...
llong dfs(llong x, llong y, vector<vector<llong> > &A){
    if(x<0 || x>=A[0].size() || y<0 || y>=A.size()){
        return 0;
    }
    llong ans;

    ans = max(dfs(x-1,y,A)+A[y][x], dfs(x,y-1,A)+A[y][x]);
    return ans;
}
...
int main(){
    llong N;
    cin >> N;
    vector<vector<llong> > A(2, vector<llong>(N, 0));
    for(int y=0; y<A.size(); y++){
        for(int x=0; x<A[0].size(); x++){
            cin >> A[y][x];
        }
    }
    cout << dfs(N-1, 1, A) << endl;
    return 0;
}
