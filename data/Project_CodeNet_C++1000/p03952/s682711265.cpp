...
void pushHeap(vector<ll>& h,int n){
  int i=h.size();
  h.push_back(n);
...
bool area(pll x,pll y,pll z){
  ll ax=x.first-z.first;
  ll ay=x.second-z.second;
  ll bx=y.first-z.first;
  ll by=y.second-z.second;
  ll s=abs1(ax*by-bx*ay);
  if(s==0)return 0;
  if(s%2==0)return 1;
  return 0;
}
