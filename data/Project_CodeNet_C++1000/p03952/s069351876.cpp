...
void zenntann_checker(ll n, ll x) {
	vector<long long> index(n * 2 - 1);
	for (int i = 0; i < n * 2 - 1; i++) index[i] = i + 1;
	do {
		cout << index << endl;
		vll a;
		for (auto p : index)a.pb(p);
		rep(_, n - 1) {
			vll v(2 * n - ((_ + 1) * 2 + 1));
			REP(i, 1, v.size() + 1) {
				vll b(3);
