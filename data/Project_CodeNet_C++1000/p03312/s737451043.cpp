...
int main(){
	lin(n);
	vin(lint,a,n);
	liv s(n+1);rep(n)s[i+1]=s[i]+a[i];
	lint ans=linf;
	rep(c,1,n){
		lint mn,mx;
		lint sl=s[c]-s[0],sr=s[n]-s[c];
		lint cl=lower_bound(s.begin(),s.begin()+c,sl/2)-s.begin(),
			cr=lower_bound(s.begin()+c,s.end(),s[c]+sr/2)-s.begin();
		lint tl,tr;
		rep(b,4){
			mn=linf,mx=-linf;
			tl=max(0ll,cl-(b&1)),tr=max(0ll,cr-!!(b&2));
			mn=min(mn,s[tl]-s[0]),mx=max(mx,s[tl]-s[0]);
			mn=min(mn,s[c]-s[tl]),mx=max(mx,s[c]-s[tl]);
			mn=min(mn,s[tr]-s[c]),mx=max(mx,s[tr]-s[c]);
			mn=min(mn,s[n]-s[tr]),mx=max(mx,s[n]-s[tr]);
			ans=min(ans,mx-mn);
		}
	}cout<<ans<<endl;
}
