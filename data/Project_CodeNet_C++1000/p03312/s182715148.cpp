...
void meowmeow321()
{
    ...
    int ans=1ll*hell*hell;
    int i=1;
    int j=3;
    for (int m = 2; m <=n-2; ++m) {
    	// solve first half 
    	while(abs(pref[i] - (pref[m]-pref[i]))>=abs(pref[i+1] - (pref[m]-pref[i+1])))i++;
    	// solve second half
    	while(abs(pref[j] - pref[m] - (pref[n]-pref[j]))>=abs(pref[j+1]-pref[m]-(pref[n]-pref[j+1])))j++;
    	// upd answer
    	int wen=max({pref[i],pref[m]-pref[i],pref[j]-pref[m],pref[n]-pref[j]})-min({pref[i],pref[m]-pref[i],pref[j]-pref[m],pref[n]-pref[j]});
    	amin(ans,wen);
    }
    cout<<ans;
}
