...
int main()
{
    ...
    /*
    int sl = 2, sr = n - 2, m = (sl+sr) / 2;
    for (; sl < sr - 1; m = (sl + sr) / 2)
    {
        int l, r;
        getlr(n, m, l, r);
    ...
    //if (n > 4)
    {
        int l1, r1;
        getlr(n, sl, l1, r1);
        ll min1 = min(min(a[l1], a[m] - a[l1]), min(a[r1] - a[m], a[n] - a[r1]));
        ll max1 = max(max(a[l1], a[m] - a[l1]), max(a[r1] - a[m], a[n] - a[r1]));
        int l2, r2;
        getlr(n, sr, l2, r2);
        ll min2 = min(min(a[l2], a[m] - a[l2]), min(a[r2] - a[m], a[n] - a[r2]));
        ll max2 = max(max(a[l2], a[m] - a[l2]), max(a[r2] - a[m], a[n] - a[r2]));
