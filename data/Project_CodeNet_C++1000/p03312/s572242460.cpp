...
void getlr(int n, int m, int& l, int& r)
{
...
int main()
{
    ...
    //*
    int sl = 2, sr = n - 2;
    for (int m = (sl + sr) / 2; sl < sr - 1; m = (sl + sr) / 2)
    {
        int l, r;
        getlr(n, m, l, r);
        ll minl = min(a[l], a[m] - a[l]);
        ll minr = min(a[r] - a[m], a[n] - a[r]);
        ll maxl = max(a[l], a[m] - a[l]);
        ll maxr = max(a[r] - a[m], a[n] - a[r]);
    ...
    //    int l1, r1;
    //    getlr(n, sl, l1, r1);
    //    ll min1 = min(min(a[l1], a[sl] - a[l1]), min(a[r1] - a[sl], a[n] - a[r1]));
    //    ll max1 = max(max(a[l1], a[sl] - a[l1]), max(a[r1] - a[sl], a[n] - a[r1]));
    //    int l2, r2;
    //    getlr(n, sr, l2, r2);
    //    ll min2 = min(min(a[l2], a[sr] - a[l2]), min(a[r2] - a[sr], a[n] - a[r2]));
    //    ll max2 = max(max(a[l2], a[sr] - a[l2]), max(a[r2] - a[sr], a[n] - a[r2]));
    //    PT(min(max1 - min1, max2 - min2));
    //}

    int l = 1, r = 3; ll ret = a[n];
    int ml = sl, mr = sr + 1;
    int c = max(2, (int)(lower_bound(a + 1, a + n, a[n] / 2) - a));
    //ml = min(n - 2, c - 1000);
    //mr = min(n - 1, c + 1000);
    //l = max(1, (int)(lower_bound(a + 1, a + ml - 1, a[ml] / 2) - a) - 1);
    //r = max(ml + 1, (int)(lower_bound(a + ml + 1, a + n, (a[n] - a[ml]) / 2 + a[ml]) - a) - 1);
    //l = 1; r = ml + 1;
    FORi(i, ml, mr)
    {
        //for (; l < i - 1; ++l)
