...
int main(){
    ...
    REPR(k,31){
        ...
        REP(i,n){
            ll t = 1LL<<k;
            dist+=lower_bound(ALL(b),2*t-a[i])-lower_bound(ALL(b),t-a[i]);
            dist+=lower_bound(ALL(b),4*t-a[i])-lower_bound(ALL(b),3*t-a[i]);
            /*
            dist += distance(b.begin(),lower_bound(ALL(b),4*t-a[i]));
            dist -= distance(b.begin(),lower_bound(ALL(b),3*t-a[i]));

            int dist2 = distance(b.begin(),lower_bound(ALL(b),2*t-a[i]));
            dist2 -= distance(b.begin(),lower_bound(ALL(b),t-a[i]));

            dist += dist2;
            */
        }
        //dump(dist%2);dump(1<<k);
        ans|=(dist%2)*(1<<k);
    }
    COUT(ans);
