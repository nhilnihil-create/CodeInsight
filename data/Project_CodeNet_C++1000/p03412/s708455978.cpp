...
int A[30][200020],
	 B[30][200020];//X[i]:=x mod 2^i

int main(){
	lin(n);
	vin(int,a,n);vin(int,b,n);
	rep(bit,1,30){
		rep(i,n){
			A[bit][i]=a[i]%(1<<bit);
			B[bit][i]=b[i]%(1<<bit);
	...
	rep(n){
		rep(bit,1,30){
			if( (lower_bound(B[bit],B[bit]+n,(1<<(bit-1))-A[bit][i])
				-lower_bound(B[bit],B[bit]+n,(1<<(bit  ))-A[bit][i])
				+lower_bound(B[bit],B[bit]+n,3*(1<<(bit-1))-A[bit][i])
				-lower_bound(B[bit],B[bit]+n,4*(1<<(bit-1))-A[bit][i]))&1)ans^=(1<<(bit-1));
		}
	}cout<<ans<<endl;
}
