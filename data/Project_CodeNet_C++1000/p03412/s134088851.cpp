...
void solve(){
  ...
  for(ll i = 0; i < 29; i++){
    ll add = 0;
    vector<ll> D(N);
    rep(j, N) D[j] = B[j] % (1<<(i+1));
    sort(D.begin(), D.end());
    rep(j, N){
      vector<ll> cnt(4);
      ll a = A[j] % (1<<(i+1));
      cnt[3] = N - (lower_bound(D.begin(), D.end(), (1<<(i+1)) + (1<<i) - a) - D.begin());
      cnt[2] = N - (lower_bound(D.begin(), D.end(), (1<<(i+1)) - a) - D.begin());
      cnt[1] = N - (lower_bound(D.begin(), D.end(), (1<<i) - a) - D.begin());
      add += cnt[3] - cnt[2] + cnt[1];
      add %= 2;
    }
    ans += (1<<i) * add;
  }
