...
ll ans, n, a[222222], b[222222];
ll c[222222], d[222222];

bool isok(int x)
{
	ll i, cut=(1LL<<(x+1)), cnt=0, lef, rig;
	for(i=1 ; i<=n ; i++) c[i]=a[i]%cut;
	for(i=1 ; i<=n ; i++) d[i]=b[i]%cut;
	d[0]=-1e18; sort(d+1,d+n+1); d[n+1]=1e18;
	for(i=1 ; i<=n ; i++)
	{
		lef=(1LL<<x)-c[i];
		rig=2LL*(1LL<<x)-c[i]-1;
		cnt+=upper_bound(d,d+n+2,rig)-lower_bound(d,d+n+2,lef);
		lef=3LL*(1LL<<x)-c[i];
		rig=4LL*(1LL<<x)-c[i]-1;
		cnt+=upper_bound(d,d+n+2,rig)-lower_bound(d,d+n+2,lef);
	}
...
int main(void)
{
	...
	for(i=0 ; i<=28 ; i++) if(isok(i)) ans+=(1LL<<i);
	cout<<ans; return 0;
}
