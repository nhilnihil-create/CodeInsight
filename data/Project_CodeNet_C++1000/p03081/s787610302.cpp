...
int n, m;
string s;
pair<char, char>p[214514];
signed main() {
	cin >> n >> m >> s;
	rep(i, m)cin >> p[i].first >> p[i].second;
	int left = -1, right = n;
	for (int i = m - 1; i >= 0; i--) {
		if (left < n - 1 && s[left + 1] == p[i].first && p[i].second == 'L')left++;
		else if (left >= 0 && s[left] == p[i].first && p[i].second == 'R')left--;
		if (right && s[right - 1] == p[i].first && p[i].second == 'R')right--;
		else if (right <= n - 1 && s[right] == p[i].first && p[i].second == 'L')right++;
	}
