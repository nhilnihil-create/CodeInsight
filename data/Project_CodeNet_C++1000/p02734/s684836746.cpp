...
signed main()
{
	...
	rngi(i, n) {
		rngi(t, s + 1) {
			(dp[i + 1][t][0] += dp[i][t][0]) %= mod;
			(dp[i + 1][t][1] += dp[i][t][0] + dp[i][t][1]) %= mod;
			(dp[i + 1][t][2] += dp[i][t][0] + dp[i][t][1] + dp[i][t][2]) %= mod;
			(dp[i + 1][t + a[i]][1] += dp[i][t][0] + dp[i][t][1]) %= mod;
			(dp[i + 1][t + a[i]][2] += dp[i][t][0] + dp[i][t][1]) %= mod;
		}
	}
	cout << dp[n][s][2] << '\n';
}