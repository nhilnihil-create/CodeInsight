...
int main(void){
    ...
    rep(i,N) rep(j,3001){
        ...
        if(j+A[i]<=S) dp[1][i+1][j+A[i]]=(dp[1][i+1][j+A[i]]+dp[1][i][j]+dp[0][i][j])%MOD;
        dp[2][i+1][j]=(dp[2][i+1][j]+dp[1][i][j]+dp[0][i][j])%MOD;
        if(j+A[i]<=S) dp[2][i+1][j+A[i]]=(dp[2][i+1][j+A[i]]+dp[1][i][j]+dp[0][i][j])%MOD;
