    ...
    */
    ...
    ld dp[3000][3000];
    ld a[3000];
    ld solve(int indx, int tail)
    {
        if(indx==n)
            return tail<=(n/2);
        if(dp[indx][tail]!=-1)return dp[indx][tail];
        ld op1 = solve(indx+1,tail+1)*(1.0 - a[indx]);
        ld op2 = solve(indx+1,tail)*a[indx];
        return dp[indx][tail]=op1+op2;
    }
    int main()
    {
        bizarre
        ...
        cout << fixed << setprecision(12) << solve(0,0);
        return 0;
    }
