...
public:
	...
	// 1-indexed
	void add(int a, T w) {
		for (int x = a; x <= size; x += (x & -x)) vec[x] += w;
	}

	// [1, a)
	T sum(int a) const {
		T res = T();
		for (int x = a - 1; x > 0; x -= (x & -x)) res += vec[x];
		return res;
	}

	// [a, b)
	inline T sum(int a, int b) const {
		return sum(b) - sum(a);
	}
...
int main() {
	...
	binary_indexed_tree<lint>bit(n);
