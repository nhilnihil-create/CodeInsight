...
void build(int now, int lo ,int hi) {
    ...
    node[now] = gcd(node[now*2], node[(now*2)+1]);
}

int queary(int now, int lo, int hi, int l, int r) {
    if(lo == l && hi == r) return node[now];

    int mid = (lo + hi) / 2;
    if(r <= mid) return queary(now*2, lo, mid, l, r);
    else if(l > mid) return queary((now*2)+1, mid+1, hi, l, r);
    else{
        return gcd(queary(now*2, lo, mid, l, mid), queary((now*2)+1, mid+1, hi, mid+1, r));
    }
}
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        if(i == 1) {
            int x = queary(1, 1, n, i+1, n);
            mx = max(mx, x);
        }
        else if(i == n) {
            int x = queary(1, 1, n, 1, i-1);
            mx = max(mx, x);
        }
        else{
            int x = queary(1, 1, n, 1, i-1), y = queary(1, 1, n, i+1, n);
            mx = max(mx, gcd(queary(1, 1, n, 1, i-1), queary(1, 1, n, i+1, n)));
        }
    }
    cout << mx << "\n";
