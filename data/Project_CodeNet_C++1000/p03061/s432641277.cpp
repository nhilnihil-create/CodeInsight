...
signed main() {
    ...
    vector<int> a(n);
    rep(i, n) cin >> a[i];
    auto f = [](int a, int b) -> int { return GCD(a, b); };
    SegmentTree<int> segtree(a, f, -1);
    int res = segtree.query(0, n);
    res = max(segtree.query(1,n),segtree.query(0,n-1));
    for (int i = 1; i < n; i++)
        chmax(res, GCD(segtree.query(0, i), segtree.query(i + 1, n)));
    cout << res << endl;
}