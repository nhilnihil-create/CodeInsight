...
void _main(){ 
    int N,M,Q;IN(N,M,Q);
    VI mHn(M+N-1);REP(i,M+N-1) mHn[i]=i;
    VI A(N,1);
    VVI qs(Q,VI(4));REP(i,Q)REP(j,4) IN(qs[i][j]);
    int out = 0;
    do{
        int beforeindex=-1;
        int beforeA=1;
        REP(i,N){
            A[i]=beforeA + mHn[i]-beforeindex-1;
            beforeindex=mHn[i];
            beforeA=A[i];
        }
        int r=0;
        for(auto q:qs){
            if(A[q[1]-1]-A[q[0]-1]==q[2]) r+= q[3];
        }
        out = max(out , r);
    }while(next_combination(mHn.begin(),mHn.begin()+N,mHn.end()));
    OUT_L(out);
    return;
}
int main() {
