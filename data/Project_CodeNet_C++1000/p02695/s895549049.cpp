...
int bestsum = 0;
int q = 0;
void dfs(int* data, int k, int targetn, int val, int maxval)
{
	if (k == targetn)
	{
		//
		...
		repi(q)
		{
			int diff = data[b[1][i]] - data[b[0][i]];
			if (diff == b[2][i]) currentsum += b[3][i];
		}
	...
	data[k] = val;
	dfs(data, k + 1, targetn, val, maxval);
	dfs(data, k, targetn, val + 1, maxval);
}
