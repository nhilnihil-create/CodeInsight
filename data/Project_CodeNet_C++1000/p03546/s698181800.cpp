...
namespace NyaGadget
{
	...
	{	// 結果を格納する変数
		...
		存在しない経路のコストは「LLONG_MAX」を返す。
		負の閉路がある場合falseを返す。
		**/
		bool Run(const std::vector<std::vector<long long>>& mg, long long inf)
		{
			const long long INF = LLONG_MAX;
			long long vmax = (long long)mg.size();

			res = mg;
			for (auto i = 0LL; i < vmax; ++i) for (auto j = 0LL; j < vmax; ++j)
			{
				if (i == j) res[i][j] = 0;
			...
			for (auto i = 0LL; i < vmax; i++) if (res[i][i] < 0) return false;
			return true;
		}
	};
}
...
{
	ll H, W; cin >> H >> W;
	vvt<ll> c; InputVVT(10, 10, c);
	vvt<ll> A; InputVVT(H, W, A);

	GT_WarshallFloyd wf;
	wf.Run(c, -1);

	ll ans = 0;
	rep(y, 0, H) rep(x, 0, W) if (A[y][x] != -1) ans += wf.res[A[y][x]][1];
	cout << ans;
	return 0;
}
