...
int main() {
    ...
    for(int i = 0;i<N;i++){
        ...
        if(now == 'R' && past == 'L'){
            Leftcount = ren-Rightcount;
            past = 'R';
            if(Leftcount % 2 == 0){
                ans.at(Leftindex) = ren/2;
                ans.at(Rightindex) = ren-ans.at(Leftindex);
            }
            else{
                ans.at(Rightindex) = ren/2;
                ans.at(Leftindex) = ren-ans.at(Rightindex);
            }
            //cout << Leftindex << " " << Rightindex << " " << ren << endl;
            ren = 0;
        }
        ren++;
        if(i == N-1){
