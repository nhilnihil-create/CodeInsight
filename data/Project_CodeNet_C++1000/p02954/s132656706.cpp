...
int main(){
    ...
    cin >> S;
    ll s = S.size();
    //vector<ll> vec(s);//Rを数えたい累積和
    vector<ll> sakai(1);//境目の左
    sakai[0] = -2;
    /*rep(i,s){
        if(S[i] == 'R')vec[i]++;
    }
    rep(i,s - 1)vec[i + 1] += vec[i];*/
    rep(i,s - 1){
        if(S[i] == 'R' && S[i + 1] == 'L')sakai.emplace_back(i);
    }
    sakai.emplace_back(s + 1);
    vector<ll> ans(s);
    ll t = sakai.size();
    REP(i,t - 2){
        ll le = 0,ri = 0;
        for(ll j = sakai[i];j > sakai[i - 1] + 1;j--){
            if(S[j] == 'R'){
                if((sakai[i] - j) % 2 == 1){
                    ri++;
                }
        ...
        for(ll j = sakai[i] + 1;j < sakai[i + 1];j++){
            if(S[j] == 'L'){
                if((j - sakai[i] - 1) % 2 == 1){
                    le++;
                }
        ...
        //cout << le << " "<< ri << "\n";
        ans[sakai[i]] += le;ans[sakai[i] + 1] += ri;
        //vecout(ans);
    }
    //vecout(sakai);
    vecout(ans);
