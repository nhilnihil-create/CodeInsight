...
int to_f(int x, int y) {
    return 1000 * x + y;
}

constexpr int MAX = 1001000;
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < a.size() - 1; j++) {
            int f = to_f(a[j].first, a[j].second);
            int to = to_f(a[j + 1].first, a[j + 1].second);
            if (find(G[f].begin(), G[f].end(), to) == G[f].end())
                G[f].push_back(to), in[to]++;
        }
    }
