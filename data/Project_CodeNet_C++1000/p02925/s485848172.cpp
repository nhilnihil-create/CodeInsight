...
ll dfs(graph &G,int i,vector<int> &seen,vector<ll> &d){
    if(seen[i] == 2)return d[i];
    seen[i] = 1;
    for (int j = 0; j < G[i].size(); ++j) {
        auto next = G[i][j];
        if(seen[next] == 1){
        ...
        d[i] = max(d[i],dfs(G,next,seen,d) + 1);
    }
    seen[i] = 2;
    return d[i];
...
int main() {
    ll N;
    cin >> N;
    auto A = InputVec2d(N,N-1);
    graph G(N * (N-1)/2 + 1);
    vector<ll> d(N * (N-1)/2 + 1,0);
    vector<int> seen(N * (N-1)/2 + 1,0);
    for (ll yi = 0; yi < N; ++yi) {
        for (ll xi = 0; xi < N-1; ++xi) {
            ll currentm = 0;
            ll currentM = 0;
            if(xi != 0){
                currentm = min(yi,A[yi][xi-1]-1);
                currentM = max(yi,A[yi][xi-1]-1);
                ll nextm = min(yi,A[yi][xi]-1);
                ll nextM = max(yi,A[yi][xi]-1);
                G[currentM * (currentM - 1) /2 + currentm + 1].push_back(nextM * (nextM - 1) /2 + nextm + 1);
            }else{
                ll nextm = min(yi,A[yi][xi]-1);
                ll nextM = max(yi,A[yi][xi]-1);
                G[0].push_back(nextM * (nextM - 1) /2 + nextm + 1);
            }
        }
    }
    cout << dfs(G,0,seen,d) << endl;
