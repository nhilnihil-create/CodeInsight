...
template <typename T>
vi topological_sort(const vector<vector<T>> &g) {
  const int N = g.size();
  vi deg(N);
  rep(i, N) each(to, g[i])++ deg[to];
  stack<int> st;
  ...
  while (!st.empty()) {
    ...
    st.pop();
    ord.emplace_back(p);
    each(to, g[p]) if (--deg[to] == 0) st.emplace(to);
  }
  return ord;
}
// DAG <=> sz(ord) == N
...
void solve(int n, vvi a) {
  int m = 0;
  vvi _id(n);
  rep(i, n)rep(_, i)_id[i].push_back(m++);
  function<int(int, int)> id = [&](int i, int j) {
    return i > j ? _id[i][j] : _id[j][i];
  };
  vvi g(m);
  rep(i, n)rep(j, n - 2) g[id(i, a[i][j])].push_back(id(i, a[i][j + 1]));
  vi ts = topological_sort(g);
  if(sz(ts) < m) {
    COUT(-1);
    return;
  }
  vi d(m, 1);
  each(i, ts)each(x, g[i])amax(d[x], d[i] + 1);
  COUT(max_of(d));
}
