...
void solve() {
  int N;
  cin >> N;
  vector<vi32> A(N, vi32(N - 1));
  cin >> A;
  rep(i, N) rep(j, N - 1) {
    A[i][j]--;
  }
  vector<vi32> indices(N, vi32(N, -1));
  rep(i, N) rep(j, N - 1) {
    indices[i][A[i][j]] = j;
  }
  vector<vi32> dp(N, vi32(N + 1, -1));
  vector<vi32> lo(N, vi32(N + 1, 0));
  auto rec = [&](auto rec, int i, int j) -> int {
    if (j == 0) return 0;
    if (dp[i][j] != -1) return dp[i][j];
    if (lo[i][j]) return 1e9;
    lo[i][j] = 1;
    int ret = 0;
    amax(ret, rec(rec, i, j - 1));
    amax(ret, rec(rec, A[i][j - 1], indices[A[i][j - 1]][i]));
    return dp[i][j] = ret == 1e9 ? 1e9 : ret + 1;
  };
  int ans = 0;
  rep(i, N) amax(ans, rec(rec, i, N - 1));
  if (ans == 1e9) ans = -1;
  cout << ans << endl;
}
