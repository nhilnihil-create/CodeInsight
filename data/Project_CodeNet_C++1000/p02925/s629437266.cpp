...
int dfs(VVI &G, int i, VI &dp){
  if(dp.at(i) >= 0) return dp.at(i);
  int res = 0;
  for(auto e : G.at(i)){
    chmax(res,dfs(G,e,dp)+1);
  }
  return dp.at(i) = res;
}
...
int main() {
  ...
  REP(i,N)REP(j,N-2){
    int a = i, b = i;
    int now = A.at(i).at(j);
    int nec = A.at(i).at(j+1);
    if(a > now) swap(a,now);
    if(b > nec) swap(b,nec);
    G.at(f(a,now,N)).PB(f(b,nec,N));
//    COUT2(f(a,now,N),f(b,nec,N));
  };
