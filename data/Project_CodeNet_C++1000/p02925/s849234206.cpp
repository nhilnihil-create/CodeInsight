...
int main(){
 ...
 /*for(ll i=0;i<n;i++){
    for(ll j=0;j<n-1;j++){
      ...
      match[i][j]=make_pair(p1,p2);
    }
  }
*/
  for(ll i=0;i<n;i++){
    for(ll j=1;j<n-1;j++){
      //match[i][j-1] id->match[i][j] id
      graph[id[i][a[i][j-1]]].push_back(id[i][a[i][j]]);
      //cout<<id[match[i][j-1].first][match[i][j-1].second]<<id[match[i][j].first][match[i][j].second]<<endl;
    }
  }
  //DAGの最長路
  for(ll i=0;i<=idnum;i++){
    if(d[i]==-1){
      dfs(i);
    }
  }
