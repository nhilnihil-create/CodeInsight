...
#define frrep(i,m,n) for(int (i)=(n); (i)>(int)(m); (i)--)
...
int main() {
    int n, day = 0, game = 0;
    cin >> n;
    vector<queue<int>> a(1005);
    vector<int> last(1005, 0);
    rep(i, n) {
        rep(j, n-1) {
            int x;
            cin >> x;
    ...
    while(!q.empty()) {
        ...
        rep(i, vol) {
            int x = q.front();
            q.pop();
            if(a[x].size() != 0 && last[x] <= day && last[a[x].front()] <= day && a[a[x].front()].front() == x) {
                last[x] = (day + 1);
                last[a[x].front()] = (day + 1);
                q.push(x);
                q.push(a[x].front());
                a[a[x].front()].pop();
                a[x].pop();
