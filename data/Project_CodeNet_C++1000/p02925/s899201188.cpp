#include <bits/stdc++.h>
using namespace std;
// types
typedef long long ll; 
typedef unsigned long long ull;
typedef vector<int> vi;
typedef pair<int, int> pii;
// IO
#define fo(i, n) for (int i = 0; i < n; i++)
#define fo1(i, n) for (int i = 1; i <= n; i++)
#define sc(n) scanf("%d", &n) 
#define sca(a, n) fo(i, n) sc(a[i])
#define pd(a) printf("%d ", a)
#define pld(a) printf("%d\n", a)
#define pda(a, n) fo(i, n) pd(a[i])
#define pl printf("\n")
#define pb(a) push_back(a)
#define mp(a, b) make_pair(a, b) 
void Yes() {printf("Yes\n");}
void No() {printf("No\n");}
// math
const ll mod = 1e9+7;
template<typename T> T gcf(T a, T b) {return (b == 0) ? a : gcf(b, a%b);} 
template<typename T> T lcm(T a, T b) {return (a*b)/gcf(a, b);}
template<typename T> T sumTo(T x) {return x * (x+1) / 2;}
template<typename T> T sumSquaresTo(T x) {return x * (x+1) * (2*x+1) / 6;}
template<typename T> T ceilDiv(T n, T d) {return (n+d-1)/d;}

const int MAXN = 1005;
int n;
queue<int> orders[MAXN];


bool done() {
    fo1(i, n) if (!orders[i].empty()) return false;
    return true;
}

void solve() {
    sc(n); int m;
    fo1(i, n) {
        fo(j, n-1) {
            sc(m);
            orders[i].push(m);
        }
    }
    int days = 0;
    set<int> tp1;
    fo1(i, n) tp1.insert(i);
    while (!done()) {
        set<int> tp2;
        for(int i: tp1) if (!orders[i].empty()) {
            int j = orders[i].front();
            if (!orders[j].empty() && orders[j].front() == i) {
                tp2.insert(i);
                tp2.insert(j);
            }
        } 
        if (tp2.empty()) {
            pd(-1);
            return;
        }
        for (int x: tp2) orders[x].pop();
        tp1 = tp2;
        days++;
    }
    pd(days);
}

int main() {
    int t = 1;
    //scanf("%d", &t);
    while (t--) {
        solve();
    }
    return 0;
}