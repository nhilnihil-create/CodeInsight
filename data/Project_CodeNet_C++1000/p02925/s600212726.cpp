...
signed main(){
    ...
    while (que.size()) {
		...
		st[p.second].pop();
        if (st[p.first].size()) {
            if (st[st[p.first].front()].front()==p.first) {
                que.push({p.first,st[p.first].front()});
                //cout<<"p "<<p.first<<' '<<st[p.first].front()<<endl;
			}
        }
        if(st[p.second].size()){
			if(st[st[p.second].front()].front()==p.second){
				que.push({p.second,st[p.second].front()});
				//cout<<'q'<<endl;
			}
		}
    }
