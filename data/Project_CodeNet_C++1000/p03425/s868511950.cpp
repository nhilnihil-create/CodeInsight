...
signed main() {
  ...
  for(int i=0;i<marchStr.length();i++){
    //cout << marchStr[i] << ":" << iCount[marchStr[i]] << endl;
  }
  // MARCH
  ll count = 0;
  for(int i=0;i<marchStr.length()-2;i++){
  	for(int j=i+1;j<marchStr.length()-1;j++){
  	  for(int k=j+1;k<marchStr.length();k++){
        //cout << marchStr[i] << marchStr[j] << marchStr[k] << ":" << iCount[marchStr[i]]*iCount[marchStr[j]]*iCount[marchStr[k]] << endl;;
        count += (ll)iCount[marchStr[i]]*(ll)iCount[marchStr[j]]*(ll)iCount[marchStr[k]];
      }
    }
  }
  cout << count << endl;
