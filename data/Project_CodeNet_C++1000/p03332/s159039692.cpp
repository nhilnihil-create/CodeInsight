...
int c(int a,int b)
{
	return divide(fac[a],mult(fac[a-b],fac[b]));
}
...
int solve()
{
	...
	{
		ll sc=i*1ll*a;
		if (sc>k || (k-sc)%b) continue;
		ll z=(k-sc)/b;
		if (z>n) continue;
		ans=add(ans,mult(c(n,i),c(n,z)));
	}
	return ans;
}
...
int sB()
{
	...
	{
		int sc=0;
		for (int i=0;i<n;++i) if (m&(1<<i)) sc+=a;
		for (int i=n;i<2*n;++i) if (m&(1<<i)) sc+=b;
		if (sc==k) ++ans;
	}
	return ans;
}
...
void test()
{
	while (1)
	{
		...
		if (solve()!=sB())
		{
			cout<<n<<' '<<a<<' '<<b<<' '<<k<<endl<<solve()<<' '<<sB()<<endl;
			break;
		}
	}
}
