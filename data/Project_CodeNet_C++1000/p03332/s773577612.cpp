...
int main() {
    ...
    cin >> N >> A >> B >> K;
    ll MOD = 998244353;
    auto fact = Factorial(N+5,MOD);
    auto invFact =InvFact(N+5,MOD);
    ll count = 0;
    for (ll i = 0; i <= N; ++i) {
        if((K - A * i) >= 0 && (K - A * i) % B == 0){
            ll j = (K - A * i) / B;
            //cout << Combination(N,i,fact,invFact,MOD) << "," << Combination(N,j,fact,invFact,MOD) << endl;
            count += Combination(N,i,fact,invFact,MOD) * Combination(N,j,fact,invFact,MOD);
            count %= MOD;
        }
    }
    cout << count << endl;
