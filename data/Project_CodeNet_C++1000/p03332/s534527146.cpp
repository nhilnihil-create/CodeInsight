...
const int mod = 998244353, mod2 = 998244353, mod3 = 1e9 + 9;
const ll inf = 1e18;
...
int main()
{
    ...
    ll a, b, k, ans{0};
    cin >> n >> a >> b >> k;
    VL frac_inv(n + 10, 1), frac(n + 10, 1);
    make_frac_tables(n + 5, frac, frac_inv);

    rep(i, n + 1)
    {
        if ((k - a * i) % b == 0 && n >= (k - a * i) / b && (k - a * i) / b >= 0)
        {
            ans += (comb(n, i, frac, frac_inv) * comb(n, (k - a * i) / b, frac, frac_inv)) % mod;
            ans %= mod;
        }
    }
    cout << ans << rt;
