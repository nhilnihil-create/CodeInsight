...
int main() {
	...
	vllint fact(n + 1, 1);
	vllint invfact(n + 1, 1);
	rep(i, n) {
		fact[i + 1] = fact[i] * ((llint)i + 1);
		fact[i + 1] %= MOD;
	}
	rep(i, n + 1) {
	...
	rep(re, n + 1) {
		...
		if ((k - re * a) % b == 0) {
			int bl = (k - re * a) / b;
			if (re > n || bl > n)continue;
			llint now = (((fact[n] * invfact[re]) % MOD) * invfact[n - re]) % MOD;
			now *= (((fact[n] * invfact[bl]) % MOD) * invfact[n - bl]) % MOD;
			now %= MOD;
			ans += now;
			ans %= MOD;
		}
