...
int main(void){
    ...
    For(i, 4){
        ...
        for(int j = 1; j < (int)bi.size(); j ++) bi[j] += bi[j-1];
    }
    ll ans = 0;
    For(i0, (int)b[0].size()) For(i1, (int)b[1].size()) For(i2, (int)b[2].size()) For(i3, (int)b[3].size()){
        if((w0+0)*i0 + (w0+1)*i1 + (w0+2)*i2 + (w0+3)*i3 <= wm) ans = max(ans, (ll)b[0][i0] + b[1][i1] + b[2][i2] + b[3][i3]);
    }
    Cout(ans);
}
