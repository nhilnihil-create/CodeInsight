...
int main()
{
	...
	int n, w; cin >> n >> w;
	vl w_(n), v(n), x(n);
	rep(i, n)cin >> w_[i] >> v[i];
	rep(i, n)x[i] = w_[i] - w_[0];
	ll dp[110][110][350] = {};
	rep(i, n)rep(j, i + 2)rep(k, 3 * n + 1)
	{
		if (j > 0 && k >= x[i])
		{
			chmax(dp[i + 1][j][k], dp[i][j][k]);
			chmax(dp[i + 1][j][k], dp[i][j - 1][k - x[i]] + v[i]);
		}
		else
		{
			chmax(dp[i + 1][j][k], dp[i][j][k]);
	...
	ll ans = 0;
	rep(j, n + 1)rep(k, 3 * n + 1)
	{
		if (j * w_[0] + k <= w)chmax(ans, dp[n][j][k]);
	}
