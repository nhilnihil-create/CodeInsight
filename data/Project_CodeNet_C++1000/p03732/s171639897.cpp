...
int32_t main()
{
	...
	sort(rall(v3));
	sort(rall(v4));
	zip(i, 1, sz(v1))  v1[i] += v1[i - 1];
	zip(i, 1, sz(v2))  v2[i] += v2[i - 1];
	zip(i, 1, sz(v3))  v3[i] += v3[i - 1];
	zip(i, 1, sz(v4))  v4[i] += v4[i - 1];
	zip(i, 0, sz(v1)) a[i + 1] = v1[i];
	zip(i, 0, sz(v2)) b[i + 1] = v2[i];
	zip(i, 0, sz(v3)) c[i + 1] = v3[i];
	zip(i, 0, sz(v4)) d[i + 1] = v4[i];
