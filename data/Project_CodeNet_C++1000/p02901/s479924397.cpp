...
vector<long> a(M);
vector<long> b(M);
vector<long> nList(S,MOD);
vector<vector<long>> c(M,vector<long>(N));
for(long i=0;i<M;i++){
    cin>>a.at(i)>>b.at(i);
    for(long j=0;j<b.at(i);j++){
        cin>>c.at(i).at(j);
        c.at(i).at(j)--;
    }
}
nList.at(0)=0;
	for(long i=0;i<M;i++){
        for(long j=0;j<S;j++){
            bitset<12> ss(j);
            for(long k=0;k<b.at(i);k++){
               ss.set(c.at(i).at(k));

            }
            long T=ss.to_ullong();
            if(nList.at(T)>nList.at(j)+a.at(i)){
                nList.at(T)=nList.at(j)+a.at(i);
            }
        }

    }

  if(nList.at(pow(2,N)-1)<MOD){
    cout<<nList.at(pow(2,N)-1)<<endl;
  }else{
    cout<<-1<<endl;
  }
