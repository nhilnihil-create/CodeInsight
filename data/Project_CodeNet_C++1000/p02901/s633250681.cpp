...
int main(){
  ...
  // 開けられる宝箱の番号は 0-indexed
  vector<ll> keys(M, 0);
  vector<ll> prices(M, 0);
  ...
  // dp[i][j] = 左から i 個の鍵まで使って、開き具合を j の状態にできる最小費用
  vector<vector<ll> > dp(M+1, vector<ll>((1<<N), INF));

  // 鍵を1つも使わず、一つも箱を開けない状態にする最小費用は0
  dp[0][0] = 0;

  // DPテーブルの更新
  REP(i, 1, M+1){
    // i : 1 ~ M (鍵の情報を取り出す際、1-indexed, 0-indexed の差異に注意)

    // 鍵[i-1]を使わない
    rep(j, 1<<N) dp[i][j] = dp[i-1][j];

    // 鍵[i-1]を使うことで鍵[i-1]を使わずに開けるより安ければ、使う
    rep(j, 1<<N) dp[i][j|keys[i-1]] = min(dp[i][j|keys[i-1]], dp[i-1][j] + prices[i-1]);
  }

  cout << (dp[M][(1<<N)-1] >= INF ? -1 : dp[M][(1<<N)-1]) << endk;
