...
signed main(int argc, char* argv[]) {
  ...
  int64_t kInf = numeric_limits<int64_t>::max() / 3;

  vector<vector<int64_t>> dp(M + 1, vector<int64_t>((1 << N), kInf));
  ...
  rep(i, M) {
    rep(S, (1 << N)) {
      ...
      bitset<12> bs = S;
      rep(j, b[i]) { bs.set(c[i][j] - 1); }
      ChMin(dp[i + 1][bs.to_ulong()], dp[i][S] + a[i]);
    }
  }

  int64_t ans = dp[M][(1 << N) - 1];
