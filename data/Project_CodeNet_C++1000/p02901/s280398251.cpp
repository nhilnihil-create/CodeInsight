...
signed main(){
	...
	rep(0,j,m+1)rep(0,i,minpow(2,n))dp[j][i]=inf;
	dp[0][0]=0;
	rep(0,i,m){
		rep(0,j,minpow(2,n)){
			int v[15];
			rep(0,l,n)v[l]=(j%minpow(2,l+1))/minpow(2,l);
			int k=0;
			int s=1;
			rep(0,l,n){
				if(d[i][l]==0&&v[l]==1)k+=s;
				s*=2;
			}
			dp[i+1][j]=min(dp[i][j],dp[i][k]+a[i]);
		}
	}
	int ans=dp[m][minpow(2,n)-1];
	if(ans>=inf)ans=-1;
	c(ans)
}	