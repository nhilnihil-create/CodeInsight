...
int main() {
  ...
  std::array<i64, 3> occurrences = {
    std::count(S.begin(), S.end(), 'a'),
    std::count(S.begin(), S.end(), 'b'),
    std::count(S.begin(), S.end(), 'c')
  };
  std::cout << (*std::max_element(occurrences.begin(), occurrences.end()) - *std::min_element(occurrences.begin(), occurrences.end()) <= 1 ? "YES" : "NO") << std::endl;
  return 0;
}