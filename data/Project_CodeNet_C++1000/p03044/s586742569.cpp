...
int n, a, b, c;
vector<int>vec2(100004,-1);
vector<vector<pair<int, int>>>vec(100004);
void color(int xx) {
	rep(i, vec[xx].size()) {
		if (vec2[vec[xx][i].first] != -1) {
			continue;
		}
		if (vec[xx][i].second % 2 == 1) {
			vec2[vec[xx][i].first] = 1 - vec2[xx];
			color(vec[xx][i].first);
		}
		else {
			vec2[vec[xx][i].first] = vec2[xx];
			color(vec[xx][i].first);
		}
	}
	return;
...
signed main() {
	...
	rep(i, n - 1) {
		...
		a--;
		b--;
		vec[a].push_back(m_p(b, c));
		vec[b].push_back(m_p(a, c));
	}
	vec2[0] = 0;
	color(0);
	rep(i, n) {
