...
int main() {
	...
	rep(i, N - 1) {
		...
		W[make_pair(Q[i].first-1, Q[i].second.first-1)] = Q[i].second.second;
		W[make_pair(Q[i].second.first-1, Q[i].first-1)] = Q[i].second.second;
	}
	G[0].cost = 0;
	G[0].p = -1;
	G[0].w = 0;
	...
	while (!Qu.empty()) {
		v = Qu.front();
		Qu.pop();
		G[v].cost = G[Qu2.front()].cost + W[make_pair(Qu2.front(),v)];
		Qu2.pop();
	...
	while (!Qu.empty()) {
		v = Qu.front();
		Qu.pop();
		if (labs(G[v].cost - G[Qu2.front()].cost) % 2 == 0) {
			if (G[Qu2.front()].col == 0) {
				G[v].col = 0;
			}
			else {
				G[v].col = 1;
