...
int32_t main()
{
	...
	int ans1 = t[0], ans2 = a[0];
	zip(i, 1, n)
	{
		int var = max(ans1, ans2);
		if (t[i] < ans1 && a[i] < ans2)
		{
			int y = min(t[i], a[i]);
			int x = (var + y - 1) / y;
			x = max((ans1 + t[i] - 1) / t[i], (ans2 + a[i] - 1) / a[i]);
			ans1 =  (x * t[i]);
			ans2 = (x * a[i]);
		}
		else if (t[i] >= ans1 && a[i] < ans2)
		{
			int x = (ans2 + a[i] - 1) / a[i];
			ans1 = (t[i] * x);
			ans2 = (a[i] * x);
		}
		else if (t[i] < ans1 && a[i] >= ans2)
		{
			int x = (ans1 + t[i] - 1) / t[i];
			ans1 = (t[i] * x);
			ans2 = (a[i] * x);
		}
		else
