...
//-----------------------ここから-----------
ll n, m;
pair<ll,ll> calc(vector<int> &removed, vector<vector<ll>> &a){

    vector<ll> cnt(m, 0);

    rep(i, n){
        rep(j, m){
            if(removed[a[i][j]]) continue;
            cnt[a[i][j]]++;
            break;
        }
    }
...
int main(void){
    ...
    pair<ll,ll> result = calc(removed, a);
    ans = min(ans, result.first);
    removed[result.second] = 1;

    while(check(removed)){
        result = calc(removed, a);
        ans = min(ans, result.first);
        removed[result.second] = 1;
    }
    cout << ans << endl;
