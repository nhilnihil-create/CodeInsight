...
int n, m, att[MAXN][MAXN], buk[MAXN], ans;
int choose[MAXN], p[MAXN];
/* functions */
int main(int argc, char *argv[]) {
    /*FILE_IN("data.in");*/ read(n); read(m); ans = n;
    FOR(i, 1, n, 1) FOR(j, 1, m, 1) read(att[i][j]);
    FOR(i, 1, m, 1) choose[i] = 1;
    FOR(i, 1, n, 1) p[i] = 1;
    FOR(k, 1, m, 1) {
        FOR(i, 1, m, 1) buk[i] = 0;
        FOR(i, 1, n, 1) {
            while(!choose[att[i][p[i]]]) ++p[i];
            ++buk[att[i][p[i]]];
        } int maxp = 0;
        FOR(j, 1, m, 1) if(buk[maxp] < buk[j]) maxp = j;
        if(ans > buk[maxp]) ans = buk[maxp];
        choose[maxp] = 0;
    } printf("%d\n", ans);
    return 0;
}
