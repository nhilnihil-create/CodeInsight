...
int main(){
  ...
  auto check = [&](int X){
    ...
    rep(i,n) rep(j,m) q[i].push(a[i][j]);
    // C[j]: 試合 j に参加する人数 
    vi C(m);
    rep(i,n) C[q[i].front()]++;
    vi cancel(m);
    ...
    while(cnt < m){
      // printf("cnt:%d\n", cnt);
      // pmax[最大参加人数, どのスポーツ] 試合
      pii pmax = mp(0,-1);
      rep(j,m) chmax(pmax, mp(C[j], j));
      // printf("j:%d C[j]:%d\n", pmax.second, pmax.first);
      if(pmax.first <= X) return true;
      cancel[pmax.second] = true;
      C[pmax.second] = 0;
      rep(i,n) {
        bool del = false;
        while(!q[i].empty() && cancel[q[i].front()]){
          q[i].pop();
          del = true;
        }
        if(del && !q[i].empty()){
  ...
  // にぶたんじゃなくても通った. 15 倍くらい遅くなるけど -> これそもそも X を決め打つ必要ないな…
  int ok = n, ng = 0;
  while (abs(ng-ok)>1){
    int mid = (ok+ng)/2;
    if(check(mid)) ok = mid;
