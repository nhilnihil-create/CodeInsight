...
/*-----8<-----template-----8<-----*/
// (0..n-1)のn個の要素について、nCkの組み合わせに対して関数fを適用する
template<typename Function> void foreach_comb(ll n, ll k, Function f) {
	vector<ll> indexes(k);
	auto recursive_comb = [&](auto recursive_comb, vector<ll> &indexes, ll s, ll rest, Function f){
		if (rest == 0) {
			f(indexes);
		} else {
			if (s < 0) return;
			recursive_comb(recursive_comb, indexes, s - 1, rest, f);
			indexes[rest - 1] = s;
			recursive_comb(recursive_comb, indexes, s - 1, rest - 1, f);
		}
	};
	recursive_comb(recursive_comb, indexes, n - 1, k, f);
}
/*-----8<-----library-----8<-----*/

constexpr ll N_MAX = 300;
constexpr ll M_MAX = 300;
...
void solve() {
	...
	ll minval=INF;
	vector<ll> kaisai(M,0);//1開催、-1非開催、0まだわからない
	vector<ll> nindex(N,0);//人がどのindexのスポーツに参加するか
	while(1){
		vector<vector<ll>> kaisaininzu(M);//どのスポーツに誰が参加するか
		rep(i,N){
			kaisaininzu[A[i][nindex[i]]].push_back(i);
		}
		ll maxval=0;
		ll sportsnum=-1;
		rep(i,M){
			if(chmax(maxval,(ll)kaisaininzu[i].size())){
				sportsnum=i;
			}
		}
		chmin(minval,maxval);
		...
		//全て探索終了なら終わる
		if([&]{
			rep(i,M){if(kaisai[i]!=-1)return false;}
			return true;
		}()){
			p(minval);return;
		}

		for(ll hito:kaisaininzu[sportsnum]){
			while(kaisai[A[hito][nindex[hito]]]==-1){
				nindex[hito]++;
			}
		}
		debug(kaisai);
