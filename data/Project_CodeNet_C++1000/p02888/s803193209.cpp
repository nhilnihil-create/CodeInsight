...
string k;
vct h;
int f[maxn];//pos:下标;val:更新的值;up:数组容量上限,一般是n,getsum(n+1)是错误的;
void update(int pos,int val,int up){ for(int i=pos;i<=up;i+=lowbit(i)) f[i] += val; }
...
void solve()
{
    ...
    fun(i)
    {
        ...
        {
            update(a[j],-1,3000);
            ans+=getsum(a[i]+a[j]-1)-getsum(a[j]-1);
        }
        fu(j,i+1,n)update(a[j],1,3000);
//        update(a[i],1,3000);
