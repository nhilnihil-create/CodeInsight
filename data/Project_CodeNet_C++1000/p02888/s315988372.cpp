...
/*
	■問題文
	高橋君は、互いに区別出来る N 本の棒を持っています。i 本目の棒の長さは Li です。
...
int main()
{
	...
	vector<int32_t> _v(n,0);
	_v[n - 3] = 1;
	_v[n - 2] = 2;
	_v[n - 1] = 3;

	vector<map<int32_t, int32_t>> mp;
	do {
		int32_t tri[3];
		int32_t i = 0;
		for (auto num : _v) {
		...
		if (tri[0] >= (tri[1] + tri[2])) { continue; }
		if (tri[1] >= (tri[2] + tri[0])) { continue; }
		if (tri[2] >= (tri[0] + tri[1])) { continue; }


		map<int32_t, int32_t> _mp;
		_mp[tri[0]]++;
		_mp[tri[1]]++;
		_mp[tri[2]]++;

		for (i = 0; i < mp.size(); i++) {
			if (mp[i] != _mp) { continue; }
			break;
		}
		if (i < mp.size()) { continue; }
		mp.push_back(_mp);
