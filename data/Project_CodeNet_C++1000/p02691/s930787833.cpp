...
int main() {
    int N;
    cin >> N;
    vector<pair<long long, long long>> participants;

    map<long long, long long> participantsIDs;

    for (int i = 1; i <= N; i++) {
        long long A;
        cin >> A;
        participants.push_back(make_pair(i, A));
        participantsIDs[i + A]++;
    }

    long long answer = 0;
    for (int i = N - 1; i >= 0; i--) {
        long long id = participants[i].first - participants[i].second;
        answer += participantsIDs[id];
    }
