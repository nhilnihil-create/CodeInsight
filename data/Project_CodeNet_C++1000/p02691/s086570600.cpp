...
int main(){
 ...
 rep(i,0,n){
  ll s=a[i]-i;
  if(!used1[s]&&!used2[0-s]){
   ans+=sub[s]*add[0-s];
   used1[s]=true;
   used2[0-s]=true;   
 //  cout<<1<<" i:"<<i<<" s:"<<s<<" sub[s]:"<<sub[s]<<" add[0-s]:"<<add[0-s]<<" ans:"<<ans<<endl;
  }
  ll t=a[i]+i;
  if(!used1[0-t]&&!used2[t]){
   ans+=sub[0-t]*add[t];
   used1[0-t]=true;
   used2[t]=true;  
 //  cout<<2<<" i:"<<i<<" s:"<<t<<" sub[0-s]:"<<sub[0-t]<<" add[s]:"<<add[t]<<" ans:"<<ans<<endl;      
  }
 }
 pr(ans);
return 0;}
