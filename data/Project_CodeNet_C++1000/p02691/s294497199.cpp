...
void solve(long long N, std::vector<long long> A) {
  // 考え方
  // WLOG, i<j. So condition is rewritten as i+A[i]==j-A[j]
  // Following this, an inverted index I is built.
  // for each person i, we search the index by a key i+A[i].
  // for each person j in the I[i+A[i]], if i<j, we add him to the result.
  unordered_map<ll, vec<ll>> I;
  rep(j,N){
    I[j-A[j]].push_back(j);
  }
  ll ret = 0;
  rep(i,N){
    if(I.find(i+A[i])!=I.end()){
      auto &II = I[i+A[i]];
      auto it = upper_bound(all(II), i);
      ret += II.size() - (it - II.begin());
    }
  }
  cout << ret << endl;
}
