...
int main()
{
    ...
    vector<vector<ll>> count(3, vector<ll>(C));
    loop(i, N)
    {
        loop(j, N)
        {
            count[(i + j) % 3][color[i][j]]++;
        }
    }
    ll ans = 1e18;
    loop(i, C)
    {
        loop(j, C)
        {
            loop(k, C)
            {
                if (i == j || j == k || k == i)
                    continue;
                //あまり0のマスをi,1のマスをj,2のマスをkにする
                ll preans = 0;
                loop(l, C) preans += count[0][l] * D[l][i];
                loop(l, C) preans += count[1][l] * D[l][j];
                loop(l, C) preans += count[2][l] * D[l][k];
                ans = min(ans, preans);
            }
        }
    }
