...
signed main() {
    ...
    vvll color_cnt(3, vll(C, 0));
    REP(i, N) REP(j, N) {
        color_cnt[(i+j)%3][c[i][j]]++;
    }
    ...
    REP(i, C) REP(j, C) REP(k, C) {
        ...
        subans = 0;
        REP(color, C) {
            subans += d[color][i] * color_cnt[0][color];
            subans += d[color][j] * color_cnt[1][color];
            subans += d[color][k] * color_cnt[2][color];
        }
        chmin(ans, subans);
    }
