...
int main(){
  ...
  // ds[ci][cj] := ci -> cj に色を変えるときの違和感
  // 色、マスともに 1-indexed
  vector<vector<ll> > ds(C, vector<ll>(C, 0));
  rep(i, C) rep(j, C){
    ds[i][j] = il();
  }

  // color_cnt[g][c] := グループg にある 色c のマスの数
  vector<vector<ll> > color_cnt(3, vector<ll>(C, 0));
  rep(i, N) rep(j, N){
    ll c = il() - 1; // 1-indexed
    color_cnt[(i+j)%3][c]++;
  }
  ...
  rep(i, C) rep(j, C) rep(k, C){
    ...
    // グループ0をi色、グループ1をj色、グループ2をk色で塗る
    ll tmp = 0;
    rep(c, C){
      tmp += ds[c][i] * color_cnt[0][c] + ds[c][j] * color_cnt[1][c] + ds[c][k] * color_cnt[2][c];
    }
    chmin(ans, tmp);
  }
