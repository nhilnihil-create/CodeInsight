...
const long long INF = 1000000000000; // 10^12:極めて大きい値,∞
...
int main() {
    ...
    int n, c;
    cin >> n >> c;
    vector<vector<int>> diff(c, vector<int>(c, 0));
    REP(i, c) {
        REP(j, c) { cin >> diff[i][j]; }
    }

    vector<vector<int>> given_col(n, vector<int>(n, 0));
    REP(i, n) {
        REP(j, n) {
            cin >> given_col[i][j];
            given_col[i][j]--;
    ...
    // i斜め行をj色にしたときのdiscomfort
    vector<vector<int>> pre_discomfort(3, vector<int>(c, 0));
    REP(ci, c) {
        REP(i, n) {
            REP(j, n) {
                pre_discomfort[(i + j) % 3][ci] += diff[given_col[i][j]][ci];
            }
        }
    }
        ...
        REP(c1, c) {
            REP(c2, c) {
                if (c0 == c1 || c0 == c2 || c1 == c2) {
                    ...
                    chmin(ans, (ll)pre_discomfort[0][c0] +
                                   pre_discomfort[1][c1] +
                                   pre_discomfort[2][c2]);
                }
            }
