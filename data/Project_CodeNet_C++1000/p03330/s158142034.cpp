...
signed main()
{
    ...
    forn(i, c) forn(j, c) cin >> dist[i][j];
    //forn(k, c) forn(i, c) forn(j, c) dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]);
    ...
    ll ans = INFLL;
    forn(x, c) forn(y, c) forn(z, c)
    {
        if (x == y || y == z || z == x) continue;
        ll res = 0;
        forn(t, c) res += cnt[0][t] * dist[t][x];
        forn(t, c) res += cnt[1][t] * dist[t][y];
        forn(t, c) res += cnt[2][t] * dist[t][z];
        ans = min(ans, res);
    }
    cout << ans;
}
