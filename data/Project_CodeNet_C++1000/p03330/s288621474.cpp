...
int main(){
  ...
  vector<vector<int64_t>> syukei(3,vector<int64_t>(c));
  rep(i,n){
    rep(j,n){
      int64_t hoge;
      cin >> hoge;
      syukei[(i+j+2)%3][hoge-1]++;
    }
  }
  int64_t miniwakan = 10000000000000000;
  rep(pi,c){
    rep(pj,c){
      rep(pk,c){
        int64_t iwakan = 0;
        ...
        rep(i,c){
          iwakan += syukei[0][i]*d[i][pi];
        }
        rep(i,c){
          iwakan += syukei[1][i]*d[i][pj];
        }
        rep(i,c){
          iwakan += syukei[2][i]*d[i][pk];
        }
        //cout << iwakan << endl;
        miniwakan = min(miniwakan,iwakan);
      }
    }
  }
  cout << miniwakan << endl;
