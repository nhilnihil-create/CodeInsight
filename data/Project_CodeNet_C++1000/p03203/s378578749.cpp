...
void solve() {
    ...
    rep (i, w) sort(all(ret[i]));
    
    int lim = 1;
    vector<int> lims(w);
    FOR (i, 1, w) {
        while (lim != h && *lower_bound(all(ret[i]), lim) == lim) lim++;
        lims[i] = lim++;
    }

    int ans = INF;
    rep (i, w) {
        if (upper_bound(all(ret[i]), lims[i]) == ret[i].end()) continue;
        chmin(ans, *upper_bound(all(ret[i]), lims[i]));
    }
