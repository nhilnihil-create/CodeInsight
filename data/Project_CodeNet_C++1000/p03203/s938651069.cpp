...
vl fact, rfact; void set_fact(int n, ll m){fact.pb(1);rfact.pb(1);FOR(i,1,n+1)fact.pb(fact[i-1]*i%m),rfact.pb(modpow(fact[i],m,m-2));}
...
main(){
  ...
  r = 0;
  FOR(i, 1, h){
    if(!m[i].empty() && *m[i].begin() <= r){
      cout << i << endl;
      return 0;
    }
    if(m[i].find(r+1) == m[i].end()) r++;
  }
  cout << h << endl;
  return 0;
}