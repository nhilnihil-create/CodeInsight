...
vector<vector<long long int>> combination_calc(2000,vector<long long int>(2000,-1));
long long int combination2(long long int n,long long int k){
    if(combination_calc[n][k]!=-1){
        ...
        combination_calc[n][k]=(combination2(n-1,k-1)+combination2(n-1,k))%MOD;
        return combination_calc[n][k];
    }else{
        return (combination2(n-1,k-1)+combination2(n-1,k))%MOD;
    }
}
...
int main(){
    ...
    ll ans=1;
    for(int i=1;i<=k;i++){
        ans=combination2(k-1,i-1);
        if(red>=i-1)ans*=combination2(red+1,i);
        else ans=0;
        ans%=MOD;
        out(ans);
    }
