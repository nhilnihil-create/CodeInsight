...
const int NM=100001;// size upto which u want
ll factmod[NM];
ll factmodinv[NM];
void ncrprecal()
{
    ...
    for(ll i=1;i<NM;i++)
    {
        factmod[i]=(i*factmod[i-1])%mo;
        factmodinv[i]=modinv(factmod[i]);
    }
}

ll ncrmp(ll n,ll r){if(n==r||r==0)return 1;if(n<r)return 0;return mmul(mmul(factmod[n],factmodinv[r]),factmodinv[n-r]);}
...
for(int i=1;i<=k;i++)
{
cout<<mmul(ncrmp(b,i),ncrmp(k-1,i-1))<<"\n";	
}
