...
int main(){
  ...
  box[0]=datas(v[0],1);
  rep1(i,N){
    if(v[i]==box[box.size()-1].first){
      box[box.size()-1].second++;
    }else{
      box.push_back(datas(v[i],1));
    }
  }
  N=box.size();
  x=1LL<<40;
  rep(i,N){
    if(!box[i].second)continue;
    while(box[i].first*2<x)x/=2;
    if(box[i].first*2==x){
      ans+=box[i].second/2;
      continue;
    }
    ...
    while(ng-ok>1){
      mid=(ok+ng)/2;
      if(box[mid].first<x-box[i].first){
        ng=mid;
      }else{
        ok=mid;
      }
    }
    if(box[ok].first==x-box[i].first&&box[ok].second){
      ans+=min(box[i].second,box[ok].second);
      box[ok].second-=min(box[i].second,box[ok].second);
    }
  }
  cout<<ans<<endl;
  return 0;
