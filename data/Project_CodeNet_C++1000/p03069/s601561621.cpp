...
void solve(void)
{
  ...
  // 右側累積和
  vector<int> shiroCnt(N+1);
  // 左側累積和
  vector<int> kuroCnt(N+1);

  shiroCnt[N] = 0;
  for (int i = N - 1; i >= 0; --i) {
    shiroCnt[i] = shiroCnt[i+1] + !S[i];
  }

  kuroCnt[0] = 0;
  for (int i = 1; i < N+1; ++i) {
    kuroCnt[i] = kuroCnt[i-1] + S[i-1];
  }

  int res = INF;
  for (int i = 0; i < N+1; ++i) {
    auto rightShiro = shiroCnt[i+1]; // 右の白を全部黒にする
    auto leftKuro = kuroCnt[i]; // 左の黒を全部白にする

    // 両方行えば題意を満たす
    auto m = rightShiro + leftKuro;
