...
signed main() {
	...
	vel sum(n + 1, 0);
	vel dsum(n + 1, 0);
	rep(i, n) { sum[i + 1] = sum[i] + d[i]; dsum[i + 1] = dsum[i] + d[i] * i; }
	int ans = 1000000000;
	ans *= 5; ans *= n;
	ans += x * n;
	for (int i = 1; i <= n;i++) {
		...
		while (i*j + 1 <= n) {
			...
			int to = (j + 2)*(j + 2) + 1;
			int diff = min(n - i * j, i);
			qans += get(st*(i - 1) + to, to - st, i*j, i*j + diff, sum, dsum);
			j++;
		}
		mmin(ans, qans + i * x);
	}
	int ret = ans + n * x;
	long long really_ret = ret;
	cout << really_ret << endl;
	return 0;
}
/*18:45:00*/