...
int main() {
	...
	for(ll i=1;i<=n;i++) {
		...
		if (temp > 0)step = 1;
		if (n / i+step <= 2) {
			chmin(min, (ull)(ans + dif + i * X));
			continue;
		}
		
		REP(j, n / i+step) {
			if (step == 1 && j == 0) {
				dif += (n / i - j - 2LL+step) *2LL* (x[temp] - x[0]);
			}
			else {
				if ((n / i - j - 2LL+step) < 0)break;
				dif += (n / i - j - 2LL+step) * 2LL*(x[ i*(j-step) + i+temp] - x[i*(j-step)+temp]);
			}
		}
		chmin(min, (ull)(ans + dif + i * X));
	}
