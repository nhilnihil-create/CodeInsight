...
int main(void){
    Cin(int, n);
    Cinm(int, n, a);
    int amin = min_element(Range(a))->first;
    int amax = max_element(Range(a))->first;

    if(amax - amin >= 2) Return("No");
    if(amax - amin == 1) Return((a[amin] < amax && n - a[amin] >= 2*(amax-a[amin])) ? "Yes" : "No");
    CoutYn(amin == n-1 || n % amin == 0);
}
