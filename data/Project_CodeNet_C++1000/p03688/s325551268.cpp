...
int main(void)
{
  ...
  for (ll i = 0; i < n; i++)
  {
    minHat = min(minHat, a[i]);
    maxHat = max(maxHat, a[i]);
  }
  if (maxHat - minHat > 1)
  {
    cout << "No" << endl;
  ...
  else if (maxHat == minHat)
  {
    if (maxHat * 2 <= n || n - 1 == maxHat)
    {
      cout << "Yes" << endl;
    }
    else
  ...
  else
  {
    ...
    if (um[maxHat] < 2 * k || um[maxHat] % k != 0 || um[maxHat] / k < 2)
    {
      cout << "No" << endl;
      return 0;
    }
