...
vl flag, color, D1, D2; vll path, path2; vl A; vl dp; vl Ans; ll cnt1 = 0, cnt2 = 0;
//***********//
int main() {
	cin >> N;
	vl A(N); for (i = 0; i < N; i++) cin >> A[i];
	sort(A.begin(), A.end());
	if (A[0] + 1 < A[N - 1]) cout << "No";
	else {
		if (A[0] == A[N - 1]) {
			if (A[0] <= N / 2 || A[0] == N - 1) cout << "Yes";
			else cout << "No";
		}
		else {
			ll pre= 0, pre2 = 0;
			...
			if (A[N - 1] >= pre + 1 && A[N - 1] <= pre + pre2 / 2) cout << "Yes";
			else cout << "No";
		}
	}
}