...
/*
	...
	'ACBD','ADBC','BCAD','BDAC','CADB','CBDA','DACB','DBCA' の 8 通りの並べ方があります。
*/
...
int main()
{
	...
	limit = _max(n, m);
	int64_t temp_max = temp_min;
	for (; i <= limit; i++) {
		temp_max *= i;
		temp_max %= mod;
	}

	int64_t result = temp_min * temp_max * ((m == n) ? 2 : 1) % mod;
	cout << result << endl;
	return 0;
}
