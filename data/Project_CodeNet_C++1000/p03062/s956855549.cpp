...
int main(){
  ...
  rep(i,n) sum += abs(a.at(i));

  //0が1つでも入っていれば すべての絶対値を足して終了
  rep(i,n) if(a.at(i) == 0)zeroflag = 1;
  if(zeroflag == 1){
    cout << sum << endl;
    return 0;
  }
  ...
  //0が1つもない場合。
  rep(i,n) if(a.at(i) < 0)micnt++; 
  
  //負の数が偶数個の場合→そのままsumを出力
  if(micnt %2 == 0){
    cout << sum << endl;
    return 0;
  }
  //負の数が奇数個の場合→その数の中での絶対値が1番小さいをsumから引く
  else{
    ll num = 1000000000000;
    rep(i,n){
        num = min(num,abs(a.at(i)) );
    }
//    cout << sum << endl;
//    cout << num << endl;
