...
int main()
{
  ...
  FOR(i,N){ cin >> A[i].first; A[i].second = i+1; }
  ...
  for(int n=1; n<=N; n++){
    for(int l=0; l<=n; l++){
      ll r = n-l;
      if(l>=1) dp[l][r] = max(dp[l][r], dp[l-1][r]+(A[n-1].first)*abs(A[n-1].second-l)); 
      if(r>=1) dp[l][r] = max(dp[l][r], dp[l][r-1]+(A[n-1].first)*abs(A[n-1].second-(N-r+1)));
    }
  }
