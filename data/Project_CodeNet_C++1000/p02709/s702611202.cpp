...
int main(void){
    ...
    sort(_all(ai), greater<AI>());

    vector<vll> dp(n+1, vll(n+1, -1LL));
    dp[0][0] = 0LL;

    rep(i, n){
        int a, c; tie(a, c) = ai[i];
        int ni = i + 1;
        ...
        rep(j, i + 1){
            ...
            int l = j;
            int nj = j + 1;
            chmax(dp[ni][nj], cur + 1LL * a * abs(l - c));

            // right
            int r = n - 1 - (i - j);
            nj = j;
            chmax(dp[ni][nj], cur + 1LL * a * abs(r - c));
        }
    }

    cout << *max_element(_all(dp[n])) << endl;
