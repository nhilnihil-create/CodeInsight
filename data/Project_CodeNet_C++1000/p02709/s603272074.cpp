...
int main() {
	...
	vector<vector<SINT64>> DP(N+1,vector<SINT64>(N+1,-1));
	...
	rep(i,0,N) {
		rep(j,0,N) {
			if (DP[i][j] != -1) {
				SINT64 l = N-1-(i-j);
				SINT64 r = j;
				DP[i+1][j] = MAX(DP[i+1][j], DP[i][j]+data[i].F*ABS(l-data[i].S));
				DP[i+1][j+1] = MAX(DP[i+1][j+1], DP[i][j]+data[i].F*ABS(r-data[i].S));
			} 
		}
	}
