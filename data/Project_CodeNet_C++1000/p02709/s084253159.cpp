...
int main() {
    ...
    ll n; cin>>n;
    vpint a(n); rep(i,n){ll x; cin>>x; a[i] = mp(x, i);}

    sort(ALL(a), greater<pint>());
    ...
    rep(i,n){
        rep(l, i+1){
            ll r = i - l;
            chmax(dp[r+1][l], dp[r][l] + a[i].first*abs(r-a[i].second));
            chmax(dp[r][l+1], dp[r][l] + a[i].first*abs((n-1) - l - a[i].second));
        }
    }
