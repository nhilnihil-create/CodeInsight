...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j <= i; j++) {
            if (dp[i][j] + A[i].first * (A[i].second - j) > dp[i + 1][j + 1]) {
                dp[i + 1][j + 1] = dp[i][j] + A[i].first * (A[i].second - j);
            }
            if (dp[i][j] + A[i].first * ((N-1) - (i-j) - A[i].second) > dp[i + 1][j]) {
                dp[i + 1][j] = dp[i][j] + A[i].first * ((N - 1) - (i - j) - A[i].second);
            }
        }
    }
