...
signed main(void) { cin.tie(0); ios::sync_with_stdio(false);
    ...
    rep(i, n){
        for(int j=0;j<=i;j++){
            int l = j;
            int r = n-1 - (i-j);
            //dump(i, l, r, dp[i][j], a[i], dp[i][j] + a[i].F * abs(a[i].S  - l), dp[i][j] + a[i].F * abs(a[i].S - r));
            chmax(dp[i+1][l+1], dp[i][j] + a[i].F * abs(a[i].S  - l));
            chmax(dp[i+1][l], dp[i][j] + a[i].F * abs(a[i].S - r));
        }
    }
    int ans = 0;
    rep(i, n){
