...
int32_t main(){
    ...
    for(int sum=1;sum<=n;++sum){
        int i=sum-1;
        int val=a[i].f,pos=a[i].s;
        for(int l=0;l<=sum;++l){
            int r=sum-l;
            int posl=l-1,posr=n-r;
            if(l)upx(dp[l][r],dp[l-1][r]+val*abs(pos-posl));
            if(r)upx(dp[l][r],dp[l][r-1]+val*abs(pos-posr));
        }
    }
    int ans=0;
    forn(i,n+1)upx(ans,dp[i][n-i]);
    cout<<ans<<'\n';
