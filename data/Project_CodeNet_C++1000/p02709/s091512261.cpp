...
int main() {
	...
	for (int j = 0; j < n; ++j) {
	    ...
	    //~ debug(arr[i + j].val, (n - j - arr[i + j].index));
	    dp[i][j + 1] = max(dp[i][j + 1], dp[i][j] + arr[i + j].val * (n - j - arr[i + j].index - 1));
	    dp[i + 1][j] = max(dp[i + 1][j], dp[i][j] + arr[i + j].val * (arr[i + j].index - i));
	}
	//~ cout << '\n';
    } 
    cout << ans;
