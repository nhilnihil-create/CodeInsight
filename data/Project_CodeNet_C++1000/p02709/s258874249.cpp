...
{ 
    ...
    dp[0][0] = 0;
    ve(pii) chiis;
    f(i,0,n) chiis.push_back({a[i],i+1});
    sort(all(chiis));
    reverse(all(chiis));
    f(i,0,n)
    {
        ...
        {
            ll y = su - x;
            dp[x+1][y] = max(dp[x+1][y] , dp[x][y] + chiis[i].first * (chiis[i].second - x - 1));
            dp[x][y+1] = max(dp[x][y+1] , dp[x][y] + chiis[i].first * (n - y -chiis[i].second  ));
        }
    }
    ll ans = 0;
    f(x,0,n+1) ans = max(ans, dp[x][n-x]);
    cout<<ans;
}
