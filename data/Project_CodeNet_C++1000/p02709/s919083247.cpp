...
ll solve(ll st, ll en, ll kitne)
{
	...
	if (dp[st][kitne] != -1)
		return dp[st][kitne];
	ll q1 = a[st].fst * abs(a[st].scd - kitne) + solve(st + 1, en, kitne + 1);
	ll q2 = a[st].fst * abs(a[st].scd - (en - 1 - (st - kitne))) + solve(st + 1, en, kitne);
	dp[st][kitne] = max(q1, q2);
	return max(q1, q2);
}
