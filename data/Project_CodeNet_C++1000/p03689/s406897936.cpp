/*
首先想到同样例1的构造方式。还是能过三十多个点的。
比如1 4 1 3，会输出无解（会填[1 1 -3 1]）。怎么改呢。对于一个点，它可以覆盖多个h*w的子矩形，只要对每个子矩形满足负权和，它们的和可以尽量大。
...
int main()
{
	...
	if(!(H%h) && !(W%w)) return puts("No"),0;//每个子矩形都是负的，拼一块还是负的 

	int r=h,c=w,tot=0,lr=2*(h-1),lc=2*(w-1);
//	while(1)
//	{
//		while(1)
//		{
