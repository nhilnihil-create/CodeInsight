...
int solve(int ind, int rem, int val){
	if(ind == n){
		if(rem == 5) return 1;
		else return 0;
	}
	int &res=  dp[ind][rem][val];
	if(res != -1) return res;
	res = 0;
	
	if(s[ind] != '?'){
		int k = mod_pow(10,ind,13);
		k = k * (s[ind] - '0');
		res += solve(ind + 1, (k%13 + rem % 13) % 13,(s[ind]-'0'));
		res %= mod;
	}else{
		int k = mod_pow(10,ind,13);
		for(int i = 0; i <= 9 ; i++){
			res += solve(ind + 1, ((k * i) % 13 + rem % 13) % 13,val);
			res %= mod;
		}
		res %= mod;
