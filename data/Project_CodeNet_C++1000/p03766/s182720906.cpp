...
int main()
{
  ll n,a[1200000]={},s[1200000]={},ans=0;
  cin >> n;
  a[0]=1;
  a[1]=1;
  s[0]=1;
  s[1]=2;
  FOR(i,2,1000001){
    a[i]=((s[i-1]-a[i-2])%MOD+MOD)%MOD;
    s[i]=((s[i-1]+a[i])%MOD+MOD)%MOD;
  }
  ans=((ans+s[n-2]*POW(n-1,2))%MOD+MOD)%MOD;
  ans=((ans+a[n-1]*(n-1)%MOD)%MOD+MOD)%MOD;
  ans=(ans+a[n])%MOD;
  rep(i,n-1)ans=((ans+a[i]*(i+1)%MOD)%MOD+MOD)%MOD;
  printf("%lld",ans);
}