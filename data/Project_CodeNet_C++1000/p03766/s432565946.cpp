...
ll f1(ll a, ll b) {return (a + b) % MOD;}
ll f2(ll a, ll b) {return (a + b) % MOD;}
ll f3(ll a, ll b, int l, int r) {return (a + (r - l) * b) % MOD;}

int main() {
  int n;
  cin >> n;

  LazySegmentTree<ll, ll> dp(vl(n + 1, 0), f1, f2, f3, 0, 0);

  dp.update(0, 1, 1, 0, 0, dp.n);
  rep (i, n) {
    ll now = dp.query(i, i + 1, 0, 0, dp.n);
    dp.update(i + 1, i + 2, now, 0, 0, dp.n);

    if (i + 3 <= n) {
      dp.update(i + 3, n + 1, now, 0, 0, dp.n);
      dp.update(n, n + 1, now * (i + 1) % MOD, 0, 0, dp.n);
    }
    else {
      dp.update(n, n + 1, now * (n - 1) % MOD, 0, 0, dp.n);
    }

    if (i != n - 1) {
      dp.update(n, n + 1, now * (n - 1) % MOD * (n - 1) % MOD, 0, 0, dp.n);
    }
    /*
    rep (j, n + 1) {
      cout << dp.query(j, j + 1, 0, 0, dp.n) << " ";
    }
    cout << endl;
    //*/
  }
  cout << dp.query(n, n + 1, 0, 0, dp.n) << endl;
}
