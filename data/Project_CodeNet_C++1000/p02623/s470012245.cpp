...
using namespace std;
int main(){fast;
	ll i,n,m,k,x,c=0;cin>>n>>m>>k;vector<ll>a(n+1,0),b(m+1,0);
	for(i=1;i<=n;i++) cin>>x,a[i]=x+a[i-1];
	for(i=1;i<=m;i++) cin>>x,b[i]=x+b[i-1];
	ll j=m;for(i=0;i<=n;i++){
		while(j&&b[j]>k-a[i]) j--;
		if(a[i]+b[j]<=k) c=max(c,i+j);
	}cout<<c<<endl;
}
