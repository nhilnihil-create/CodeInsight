...
int main() {
    ...
    std::vector<ll> A_sum(N + 1, 0);
    REP(acc_i, N) {
        A_sum[acc_i + 1] = A_sum[acc_i] + A[acc_i];
    }
    std::vector<ll> B_sum(M + 1, 0);
    REP(acc_i, M) {
        B_sum[acc_i + 1] = B_sum[acc_i] + B[acc_i];
    }
    ...
    REP(ai, N + 1) {
        auto v = K - A_sum[ai];
        if(0 <= v) {
            auto itr = upper_bound(ALL(B_sum), v) - 1;
            chmax(ans, ai + (int)distance(B_sum.begin(), itr));
        }
    }
