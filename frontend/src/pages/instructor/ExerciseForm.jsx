...
export default function ExerciseForm({ mode = 'create' }) {
  ...
  const fetchExercise = async () => {
    try {
      ...
      // Normalize test_cases to objects if they're strings
      let test_cases = data.test_cases || [];
      if (Array.isArray(test_cases) && test_cases.length > 0) {
        if (typeof test_cases[0] === 'string') {
      ...
      setFormData({
        ...
        track_ner: data.track_ner !== false,
        track_nrs: data.track_nrs !== false,
        track_nts: data.track_nts !== false,
        auto_alert: data.auto_alert !== false,
        test_cases: test_cases.length > 0 ? test_cases : [{ input: '', expected_output: '', hidden: false }]
      });
      setSelectedConcept(data.concept_name || null);
    } catch (err) {
      ...
      if (err.response?.status === 404) {
        ...
        alert('Failed to load exercise: ' + (err.response?.data?.message || err.message));
      }
      navigate('/instructor');
    }
  };
  ...
  const handleSubmit = async (e, isDraft = false) => {
    ...
    try {
      if (mode === 'edit') {
        await api.put(`/api/exercises/${id}`, {
          ...formData,
          is_draft: isDraft
        });
        alert('Exercise updated successfully!');
        navigate(`/instructor/sections/${formData.section_id}`);
      } else {
  ...
  const pageTitle = mode === 'edit' ? 'Edit Exercise' : 'Create New Exercise';
  const pageSubtitle = mode === 'edit' ? 'Update exercise details and test cases' : 'Tag a concept · Set test cases · Assign to class';
  const submitButtonText = mode === 'edit' ? 'Update Exercise' : 'Publish Exercise';
      ...
      <form onSubmit={(e) => handleSubmit(e, false)} style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.6fr', gap: '20px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            ...
            >
              {submitting ? (mode === 'edit' ? 'Updating...' : 'Publishing...') : submitButtonText}
            </button>
            <button 
              type="button"
