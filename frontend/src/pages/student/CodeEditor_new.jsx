        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '8px 14px', background: COLORS.surface2, borderBottom: `1px solid ${COLORS.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {!running ? (
                <button onClick={handleRun} disabled={running} style={{ padding: '6px 14px', borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: running ? 'not-allowed' : 'pointer', border: '1px solid #2a5a4a', background: '#1a4d3a', color: COLORS.success, fontFamily: "'DM Sans', sans-serif", transition: 'all 0.15s', opacity: running ? 0.6 : 1 }}>
                  ▶ Run
                </button>
              ) : (
              ...
              <button onClick={handleSubmit} disabled={submitting} style={{ padding: '6px 14px', borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', border: 'none', background: COLORS.teal, color: '#091a1a', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.15s', opacity: submitting ? 0.6 : 1 }}>
                {submitting ? '⏳...' : '✓ Submit'}
              </button>
            </div>
          </div>
                ...
                testResults ? (
                  <>
                    ...
                    {testResults.compilation_log && (
                      <div style={{ color: COLORS.muted, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                        {testResults.compilation_log}
                      </div>
                    )}
                    {!testResults.error && !testResults.compilation_log && (
                      <div style={{ color: COLORS.muted }}>(No compiler log)</div>
                    )}
                  </>
                ) : (
                  <div style={{ color: COLORS.muted }}>(No log yet)</div>
                )
              )}
            </div>
          </div>
            ...
            {attempts.length === 0 ? (
              ...
              attempts.map((attempt, idx) => (
                <div key={idx} style={{ background: COLORS.surface2, border: `1px solid ${attempt.passed ? COLORS.success : COLORS.error}`, borderLeft: `3px solid ${attempt.passed ? COLORS.success : COLORS.error}`, borderRadius: '8px', padding: '10px', marginBottom: '8px' }}>
                  ...
                  <div style={{ fontSize: '9px', color: COLORS.muted }}>{new Date(attempt.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
              ))
            )}
          </div>
